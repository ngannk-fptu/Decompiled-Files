...
extends StopwordAnalyzerBase {
    ...
    protected Analyzer.TokenStreamComponents createComponents(String fieldName, Reader reader) {
        ...
        StopFilter s = new StopFilter(this.matchVersion, (TokenStream)result, HYPHENATIONS);
        if (!this.matchVersion.onOrAfter(Version.LUCENE_44)) {
            s.setEnablePositionIncrements(false);
        }
        result = s;
        result = new ElisionFilter((TokenStream)result, DEFAULT_ARTICLES);
        result = new IrishLowerCaseFilter((TokenStream)result);
        ...
        result = new SnowballFilter((TokenStream)result, new IrishStemmer());
