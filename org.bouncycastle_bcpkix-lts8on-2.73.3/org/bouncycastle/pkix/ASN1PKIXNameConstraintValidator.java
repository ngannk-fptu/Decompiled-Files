...
class ASN1PKIXNameConstraintValidator
implements NameConstraintValidator {
    ...
    public void checkPermitted(GeneralName name) throws NameConstraintValidatorException {
        switch (name.getTagNo()) {
            case 0: {
                this.checkPermittedOtherName(this.permittedSubtreesOtherName, OtherName.getInstance((Object)name.getName()));
                break;
            }
            case 1: {
            ...
            case 2: {
                this.checkPermittedDNS(this.permittedSubtreesDNS, ASN1IA5String.getInstance((Object)name.getName()).getString());
                break;
            }
            case 4: {
                this.checkPermittedDN(X500Name.getInstance((Object)name.getName()));
                break;
            }
            case 6: {
                this.checkPermittedURI(this.permittedSubtreesURI, ASN1IA5String.getInstance((Object)name.getName()).getString());
                break;
            }
            case 7: {
                byte[] ip = ASN1OctetString.getInstance((Object)name.getName()).getOctets();
    ...
    public void checkExcluded(GeneralName name) throws NameConstraintValidatorException {
        switch (name.getTagNo()) {
            case 0: {
                this.checkExcludedOtherName(this.excludedSubtreesOtherName, OtherName.getInstance((Object)name.getName()));
                break;
            }
            case 1: {
            ...
            case 2: {
                this.checkExcludedDNS(this.excludedSubtreesDNS, ASN1IA5String.getInstance((Object)name.getName()).getString());
                break;
            }
            case 4: {
                this.checkExcludedDN(X500Name.getInstance((Object)name.getName()));
                break;
            }
            case 6: {
                this.checkExcludedURI(this.excludedSubtreesURI, ASN1IA5String.getInstance((Object)name.getName()).getString());
                break;
            }
            case 7: {
                byte[] ip = ASN1OctetString.getInstance((Object)name.getName()).getOctets();
    ...
    public void intersectPermittedSubtree(GeneralSubtree[] permitted) {
        ...
        for (int i = 0; i != permitted.length; ++i) {
            ...
            Integer tagNo = Integers.valueOf((int)subtree.getBase().getTagNo());
    ...
    public void addExcludedSubtree(GeneralSubtree subtree) {
        ...
        switch (base.getTagNo()) {
            case 0: {
                this.excludedSubtreesOtherName = this.unionOtherName(this.excludedSubtreesOtherName, OtherName.getInstance((Object)base.getName()));
                break;
            }
            case 1: {
            ...
            case 7: {
                this.excludedSubtreesIP = this.unionIP(this.excludedSubtreesIP, ASN1OctetString.getInstance((Object)base.getName()).getOctets());
                break;
            }
            default: {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof ASN1PKIXNameConstraintValidator)) {
            return false;
        }
        ASN1PKIXNameConstraintValidator constraintValidator = (ASN1PKIXNameConstraintValidator)o;
    ...
    public void checkPermittedDN(X500Name dns) throws NameConstraintValidatorException {
        this.checkPermittedDN(this.permittedSubtreesDN, ASN1Sequence.getInstance((Object)dns.toASN1Primitive()));
    }

    public void checkExcludedDN(X500Name dns) throws NameConstraintValidatorException {
        this.checkExcludedDN(this.excludedSubtreesDN, ASN1Sequence.getInstance((Object)dns));
    }
    ...
    private static boolean withinDNSubtree(ASN1Sequence dns, ASN1Sequence subtree) {
        ...
        int start = 0;
        RDN subtreeRdnStart = RDN.getInstance((Object)subtree.getObjectAt(0));
        for (j = 0; j < dns.size(); ++j) {
            start = j;
            RDN dnsRdn = RDN.getInstance((Object)dns.getObjectAt(j));
            if (dnsRdn.equals((Object)subtreeRdnStart)) break;
        }
        if (subtree.size() > dns.size() - start) {
            return false;
        }
        for (j = 0; j < subtree.size(); ++j) {
            RDN subtreeRdn = RDN.getInstance((Object)subtree.getObjectAt(j));
            RDN dnsRdn = RDN.getInstance((Object)dns.getObjectAt(start + j));
            if (subtreeRdn.size() == dnsRdn.size()) {
                if (!subtreeRdn.getFirst().getType().equals((ASN1Primitive)dnsRdn.getFirst().getType())) {
                    return false;
                }
                if (!(subtreeRdn.size() == 1 && subtreeRdn.getFirst().getType().equals((ASN1Primitive)RFC4519Style.serialNumber) ? !dnsRdn.getFirst().getValue().toString().startsWith(subtreeRdn.getFirst().getValue().toString()) : !IETFUtils.rDNAreEqual((RDN)subtreeRdn, (RDN)dnsRdn))) continue;
                return false;
            }
            return false;
        }
    ...
    private void checkPermittedDN(Set permitted, ASN1Sequence dns) throws NameConstraintValidatorException {
        ...
        for (ASN1Sequence subtree : permitted) {
            if (!ASN1PKIXNameConstraintValidator.withinDNSubtree(dns, subtree)) continue;
            return;
        }
    ...
    private Set intersectDN(Set permitted, Set dns) {
        ...
        while (it.hasNext()) {
            ASN1Sequence dn = ASN1Sequence.getInstance((Object)((GeneralSubtree)it.next()).getBase().getName().toASN1Primitive());
            if (permitted == null) {
                if (dn == null) continue;
                intersect.add(dn);
                continue;
            ...
            for (ASN1Sequence subtree : permitted) {
                if (ASN1PKIXNameConstraintValidator.withinDNSubtree(dn, subtree)) {
                    intersect.add(dn);
                    continue;
                }
                if (!ASN1PKIXNameConstraintValidator.withinDNSubtree(subtree, dn)) continue;
    ...
    private Set unionDN(Set excluded, ASN1Sequence dn) {
        ...
        for (ASN1Sequence subtree : excluded) {
            ...
            if (ASN1PKIXNameConstraintValidator.withinDNSubtree(subtree, dn)) {
                intersect.add(dn);
                continue;
            }
            intersect.add(subtree);
    ...
    private Set intersectOtherName(Set permitted, Set otherNames) {
        HashSet intersect = new HashSet(permitted);
        intersect.retainAll(otherNames);
        return intersect;
    }

    private Set unionOtherName(Set permitted, OtherName otherName) {
        HashSet<OtherName> union = new HashSet<OtherName>(permitted);
        union.add(otherName);
        return union;
    }
    ...
    private Set intersectIP(Set permitted, Set ips) {
        ...
        while (it.hasNext()) {
            byte[] ip = ASN1OctetString.getInstance((Object)((GeneralSubtree)it.next()).getBase().getName()).getOctets();
            if (permitted == null) {
                if (ip == null) continue;
                intersect.add(ip);
                continue;
    ...
    private Set unionIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2) {
        ...
        if (Arrays.areEqual((byte[])ipWithSubmask1, (byte[])ipWithSubmask2)) {
    ...
    private Set intersectIPRange(byte[] ipWithSubmask1, byte[] ipWithSubmask2) {
        ...
        byte[] max = ASN1PKIXNameConstraintValidator.min(minMax[1], minMax[3]);
        byte[] min = ASN1PKIXNameConstraintValidator.max(minMax[0], minMax[2]);
        if (ASN1PKIXNameConstraintValidator.compareTo(min, max) == 1) {
            return Collections.EMPTY_SET;
        }
        byte[] ip = ASN1PKIXNameConstraintValidator.or(minMax[0], minMax[2]);
        byte[] subnetmask = ASN1PKIXNameConstraintValidator.or(subnetmask1, subnetmask2);
    ...
    private boolean isIPConstrained(byte[] ip, byte[] constraint) {
        ...
        return Arrays.areEqual((byte[])permittedSubnetAddress, (byte[])ipSubnetAddress);
    }

    private boolean otherNameIsConstrained(OtherName name, OtherName constraint) {
        return constraint.equals((Object)name);
    }
    ...
    private boolean withinDomain(String testDomain, String domain) {
        ...
        String[] domainParts = Strings.split((String)tempDomain, (char)'.');
        String[] testDomainParts = Strings.split((String)testDomain, (char)'.');
    ...
    private boolean isUriConstrained(String uri, String constraint) {
        String host = ASN1PKIXNameConstraintValidator.extractHostFromURL(uri);
    ...
    private String extractNameAsString(GeneralName name) {
        return ASN1IA5String.getInstance((Object)name.getName()).getString();
    }
    ...
    private static int compareTo(byte[] ip1, byte[] ip2) {
        if (Arrays.areEqual((byte[])ip1, (byte[])ip2)) {
            return 0;
        }
        if (Arrays.areEqual((byte[])ASN1PKIXNameConstraintValidator.max(ip1, ip2), (byte[])ip1)) {
            return 1;
        }
        return -1;
    }
    ...
    private String stringifyOtherNameCollection(Set otherNames) {
        ...
        while (it.hasNext()) {
            ...
            temp.append(":");
            try {
                temp.append(Hex.toHexString((byte[])name.getValue().toASN1Primitive().getEncoded()));
            }
            catch (IOException e) {
