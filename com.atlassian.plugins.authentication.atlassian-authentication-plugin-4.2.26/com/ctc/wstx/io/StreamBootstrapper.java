...
extends InputBootstrapper {
    ...
    public Reader bootstrapInput(ReaderConfig cfg, boolean mainDoc, int xmlVersion) throws IOException, XMLStreamException {
        ...
        if (normEnc == "UTF-8") {
            ...
            if (this.mInputPtr < this.mInputEnd) {
                in = in == null ? new ByteArrayInputStream(this.mByteBuffer, this.mInputPtr, this.mInputEnd - this.mInputPtr) : new MergedStream(cfg, in, this.mByteBuffer, this.mInputPtr, this.mInputEnd);
            } else if (in == null) {
                in = new ByteArrayInputStream(this.mByteBuffer, 0, 0);
            }
            if (normEnc == "UTF-16") {
    ...
    @Override
    protected int readQuotedValue(char[] outputBuffer, int quoteChar) throws IOException, WstxException {
        boolean mb;
        int outputPtr = 0;
    ...
    protected Location getLocation() {
        ...
        return new WstxInputLocation(null, this.mPublicId, this.mSystemId, (long)(total - 1), this.mInputRow, col);
    }
