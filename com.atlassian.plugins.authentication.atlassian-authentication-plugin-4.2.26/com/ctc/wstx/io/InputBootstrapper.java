...
public abstract class InputBootstrapper {
    ...
    private final int readXmlVersion() throws IOException, WstxException {
        ...
        if ((len = this.readQuotedValue(this.mKeywordBuffer, c = this.handleEq("version"))) == 3 && this.mKeywordBuffer[0] == '1' && this.mKeywordBuffer[1] == '.') {
            c = this.mKeywordBuffer[2];
            if (c == 48) {
                return 256;
            }
            if (c == 49) {
        ...
        String got = len < 0 ? "'" + new String(this.mKeywordBuffer) + "[..]'" : (len == 0 ? "<empty>" : "'" + new String(this.mKeywordBuffer, 0, len) + "'");
    ...
    private final String readXmlEncoding() throws IOException, WstxException {
        ...
        if ((len = this.readQuotedValue(this.mKeywordBuffer, c = this.handleEq("encoding"))) == 0) {
            this.reportPseudoAttrProblem("encoding", null, null, null);
        }
        if (len < 0) {
            return new String(this.mKeywordBuffer);
        }
        return new String(this.mKeywordBuffer, 0, len);
    }
    ...
    private final String readXmlStandalone() throws IOException, WstxException {
        ...
        if ((len = this.readQuotedValue(this.mKeywordBuffer, c = this.handleEq("standalone"))) == 2) {
            if (this.mKeywordBuffer[0] == 'n' && this.mKeywordBuffer[1] == 'o') {
                return "no";
            }
        } else if (len == 3 && this.mKeywordBuffer[0] == 'y' && this.mKeywordBuffer[1] == 'e' && this.mKeywordBuffer[2] == 's') {
            return "yes";
        }
        String got = len < 0 ? "'" + new String(this.mKeywordBuffer) + "[..]'" : (len == 0 ? "<empty>" : "'" + new String(this.mKeywordBuffer, 0, len) + "'");
