...
public final class EntityStatement {
    ...
    public Base64URL verifySignature(JWKSet jwkSet) throws BadJOSEException, JOSEException {
        List<JWK> jwkMatches = new JWKSelector(JWKMatcher.forJWSHeader(this.statementJWT.getHeader())).select(jwkSet);
        ...
        for (JWK candidateJWK : jwkMatches) {
            ...
            JWSVerifier jwsVerifier = verifierFactory.createJWSVerifier(this.statementJWT.getHeader(), publicKey);
        ...
        try {
            new EntityStatementClaimsVerifier(null).verify(this.statementJWT.getJWTClaimsSet());
        }
    ...
    public static EntityStatement parse(String signedStmtString) throws ParseException {
        try {
            return EntityStatement.parse(SignedJWT.parse(signedStmtString));
        }
        catch (java.text.ParseException e) {
