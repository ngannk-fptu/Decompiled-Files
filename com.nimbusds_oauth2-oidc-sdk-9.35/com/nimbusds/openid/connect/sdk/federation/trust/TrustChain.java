...
public final class TrustChain {
    ...
    public void verifySignatures(JWKSet trustAnchorJWKSet) throws BadJOSEException, JOSEException {
        ...
        try {
            ...
            throw new BadJOSEException("Invalid leaf statement: " + e.getMessage(), (Throwable)e);
        }
        for (int i = 0; i < this.superiors.size(); ++i) {
            ...
            try {
                ...
                throw new BadJOSEException("Invalid statement from " + stmt.getClaimsSet().getIssuer() + ": " + e.getMessage(), (Throwable)e);
            }
        }
    }
    ...
    private static boolean hasJWKWithThumbprint(JWKSet jwkSet, Base64URL thumbprint) {
        ...
        for (JWK jwk : jwkSet.getKeys()) {
            try {
                if (!thumbprint.equals((Object)jwk.computeThumbprint())) continue;
                return true;
            }
