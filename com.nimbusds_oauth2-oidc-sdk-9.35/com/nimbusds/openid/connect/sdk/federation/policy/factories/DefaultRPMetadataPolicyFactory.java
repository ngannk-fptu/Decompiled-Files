...
implements RPMetadataPolicyFactory {
    ...
    public MetadataPolicy create(OIDCClientMetadata initialMetadata, OIDCClientInformation target) throws PolicyFormulationException {
        ...
        for (Map.Entry en : target.toJSONObject().entrySet()) {
            if (en.equals(new AbstractMap.SimpleImmutableEntry(en.getKey(), initialJSONObject.get(en.getKey())))) continue;
            MetadataPolicyEntry policyEntry = new MetadataPolicyEntry((String)en.getKey(), Collections.singletonList(DefaultRPMetadataPolicyFactory.createValueOperation(en)));
