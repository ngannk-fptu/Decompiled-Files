...
implements JWSKeySelector<DPoPProofContext> {
    ...
    public List<Key> selectJWSKeys(JWSHeader header, DPoPProofContext context) throws KeySourceException {
        ...
        if (JWSAlgorithm.Family.RSA.contains((Object)alg) && jwk instanceof RSAKey) {
            try {
                candidates.add(((RSAKey)jwk).toRSAPublicKey());
            }
            catch (JOSEException e) {
                throw new KeySourceException("Invalid RSA JWK: " + e.getMessage(), (Throwable)e);
            }
        } else if (JWSAlgorithm.Family.EC.contains((Object)alg) && jwk instanceof ECKey) {
            try {
                candidates.add(((ECKey)jwk).toECPublicKey());
            }
            catch (JOSEException e) {
                throw new KeySourceException("Invalid EC JWK: " + e.getMessage(), (Throwable)e);
            }
        } else {
