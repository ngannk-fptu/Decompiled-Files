...
implements HttpMessageWriter<MultiValueMap<String, ?>> {
    ...
    private Mono<Void> writeMultipart(MultiValueMap<String, ?> map, ReactiveHttpOutputMessage outputMessage, @Nullable MediaType mediaType, Map<String, Object> hints) {
        ...
        LogFormatUtils.traceDebug((Log)this.logger, traceOn -> Hints.getLogPrefix((Map)hints) + "Encoding " + (this.isEnableLoggingRequestDetails() ? LogFormatUtils.formatValue((Object)map, (traceOn == false ? 1 : 0) != 0) : "parts " + map.keySet() + " (content masked)"));
        DataBufferFactory bufferFactory = outputMessage.bufferFactory();
        Flux body = Flux.fromIterable((Iterable)map.entrySet()).concatMap(entry -> this.encodePartValues(boundary, (String)entry.getKey(), (List)entry.getValue(), bufferFactory)).concatWith(this.generateLastLine(boundary, bufferFactory)).doOnDiscard(PooledDataBuffer.class, DataBufferUtils::release);
    ...
    private <T> Flux<DataBuffer> encodePart(byte[] boundary, String name, T value, DataBufferFactory factory) {
        ...
        if (value instanceof HttpEntity) {
            ...
            headers.putAll((Map<? extends String, ? extends List<String>>)((Object)httpEntity.getHeaders()));
        ...
        if (!headers.containsKey("Content-Disposition")) {
            if (body instanceof Resource) {
                ...
                body = Mono.from((Publisher)((Publisher)body)).doOnNext(o -> headers.setContentDispositionFormData(name, ((Resource)o).getFilename()));
            } else {
