...
extends AbstractServerHttpRequest {
    ...
    private static MultiValueMap<String, String> createDefaultHttpHeaders(HttpServletRequest request) {
        MultiValueMap headers = CollectionUtils.toMultiValueMap((Map)new LinkedCaseInsensitiveMap(8, Locale.ENGLISH));
        Enumeration names = request.getHeaderNames();
        while (names.hasMoreElements()) {
            String name = (String)names.nextElement();
            Enumeration values = request.getHeaders(name);
            while (values.hasMoreElements()) {
                headers.add((Object)name, (Object)((String)values.nextElement()));
            }
        }
        return headers;
    }
    ...
    private static MultiValueMap<String, String> initHeaders(MultiValueMap<String, String> headerValues, HttpServletRequest request) {
        ...
        if (!StringUtils.hasLength((String)((String)headerValues.getFirst("Content-Type"))) && StringUtils.hasLength((String)(requestContentType = request.getContentType()))) {
        ...
        if (contentType != null && contentType.getCharset() == null && StringUtils.hasLength((String)(encoding = request.getCharacterEncoding()))) {
            ...
            headers.setContentType(new MediaType(contentType, (Map<String, String>)params));
        }
        if (headerValues.getFirst("Content-Type") == null && (contentLength = request.getContentLength()) != -1) {
