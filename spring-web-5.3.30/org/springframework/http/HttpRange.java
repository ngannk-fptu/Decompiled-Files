...
public abstract class HttpRange {
    ...
    public ResourceRegion toResourceRegion(Resource resource) {
        Assert.isTrue((resource.getClass() != InputStreamResource.class ? 1 : 0) != 0, (String)"Cannot convert an InputStreamResource to a ResourceRegion");
        ...
        Assert.isTrue((start < contentLength ? 1 : 0) != 0, () -> "'position' exceeds the resource length " + contentLength);
    ...
    public static List<HttpRange> parseRanges(@Nullable String ranges) {
        if (!StringUtils.hasLength((String)ranges)) {
            return Collections.emptyList();
        }
        if (!ranges.startsWith(BYTE_RANGE_PREFIX)) {
            throw new IllegalArgumentException("Range '" + ranges + "' does not start with 'bytes='");
        }
        String[] tokens = StringUtils.tokenizeToStringArray((String)(ranges = ranges.substring(BYTE_RANGE_PREFIX.length())), (String)",");
    ...
    private static HttpRange parseRange(String range) {
        Assert.hasLength((String)range, (String)"Range String must not be empty");
    ...
    private static long getLengthFor(Resource resource) {
        try {
            ...
            Assert.isTrue((contentLength > 0L ? 1 : 0) != 0, (String)"Resource content length should be > 0");
    ...
    public static String toString(Collection<HttpRange> ranges) {
        Assert.notEmpty(ranges, (String)"Ranges Collection must not be empty");
    ...
    extends HttpRange {
        ...
        public boolean equals(@Nullable Object other) {
            ...
            return this.firstPos == otherRange.firstPos && ObjectUtils.nullSafeEquals((Object)this.lastPos, (Object)otherRange.lastPos);
        }

        public int hashCode() {
            return ObjectUtils.nullSafeHashCode((Object)this.firstPos) * 31 + ObjectUtils.nullSafeHashCode((Object)this.lastPos);
        }
