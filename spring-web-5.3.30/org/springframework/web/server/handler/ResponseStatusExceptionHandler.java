...
implements WebExceptionHandler {
    ...
    private String formatError(Throwable ex, ServerHttpRequest request) {
        ...
        String message = LogFormatUtils.formatValue((Object)ex.getMessage(), (int)-1, (boolean)true);
        ...
        if (code != -1) {
            if (response.setRawStatusCode(code)) {
                if (ex instanceof ResponseStatusException) {
                    ((ResponseStatusException)((Object)ex)).getResponseHeaders().forEach((name, values) -> values.forEach(value -> response.getHeaders().add((String)name, (String)value)));
                }
                result = true;
            }
        } else {
    ...
    protected int determineRawStatusCode(Throwable ex) {
        if (ex instanceof ResponseStatusException) {
            return ((ResponseStatusException)((Object)ex)).getRawStatusCode();
        }
        return -1;
    }
}
