    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (multipartResolver.isMultipart(processedRequest = request)) {
            if (this.logger.isTraceEnabled()) {
                this.logger.trace((Object)"Resolving multipart request");
            }
            processedRequest = multipartResolver.resolveMultipart(processedRequest);
        } else if (this.logger.isTraceEnabled()) {
            this.logger.trace((Object)"Not a multipart request");
        }
        try {
    ...
    protected MultipartResolver lookupMultipartResolver() {
        ...
        if (wac != null && wac.containsBean(beanName)) {
            if (this.logger.isDebugEnabled()) {
                this.logger.debug((Object)("Using MultipartResolver '" + beanName + "' for MultipartFilter"));
            }
