...
public class ContextLoader {
    ...
    public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info((Object)"Root WebApplicationContext: initialization started");
        }
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                ...
                logger.info((Object)("Root WebApplicationContext initialized in " + elapsedTime + " ms"));
            }
            return this.context;
        }
    ...
    protected Class<?> determineContextClass(ServletContext servletContext) {
        ...
        try {
            return ClassUtils.forName((String)contextClassName, (ClassLoader)ContextLoader.class.getClassLoader());
        }
        catch (ClassNotFoundException ex) {
    ...
    protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
        ...
        for (Class<ApplicationContextInitializer<ConfigurableApplicationContext>> clazz : initializerClasses) {
            ...
            this.contextInitializers.add((ApplicationContextInitializer<ConfigurableApplicationContext>)BeanUtils.instantiateClass(clazz));
        }
        AnnotationAwareOrderComparator.sort(this.contextInitializers);
        for (ApplicationContextInitializer applicationContextInitializer : this.contextInitializers) {
            applicationContextInitializer.initialize((ConfigurableApplicationContext)wac);
        }
    }
    ...
    protected List<Class<ApplicationContextInitializer<ConfigurableApplicationContext>>> determineContextInitializerClasses(ServletContext servletContext) {
        ...
        if (globalClassNames != null) {
            for (String className : StringUtils.tokenizeToStringArray((String)globalClassNames, (String)INIT_PARAM_DELIMITERS)) {
        ...
        if ((localClassNames = servletContext.getInitParameter(CONTEXT_INITIALIZER_CLASSES_PARAM)) != null) {
            for (String className : StringUtils.tokenizeToStringArray((String)localClassNames, (String)INIT_PARAM_DELIMITERS)) {
                classes.add(this.loadInitializerClass(className));
            }
        }
        return classes;
