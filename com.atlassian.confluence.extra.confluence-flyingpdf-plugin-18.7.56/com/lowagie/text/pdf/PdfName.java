...
implements Comparable {
    ...
    public int compareTo(Object object) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (myBytes[i] >= objBytes[i]) continue;
            return -1;
        }
    ...
    static {
        ...
        try {
            for (Field curFld : fields) {
                if ((curFld.getModifiers() & 0x19) != 25 || !curFld.getType().equals(PdfName.class)) continue;
                PdfName name = (PdfName)curFld.get(null);
                staticNames.put(PdfName.decodeName(name.toString()), name);
            }
        }
        catch (Exception e) {
            e.printStackTrace();
