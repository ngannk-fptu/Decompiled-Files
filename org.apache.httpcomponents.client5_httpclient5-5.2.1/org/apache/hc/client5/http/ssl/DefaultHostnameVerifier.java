...
implements HttpClientHostnameVerifier {
    ...
    public void verify(String host, X509Certificate cert) throws SSLException {
        ...
        switch (hostType) {
            case IPv4: {
                DefaultHostnameVerifier.matchIPAddress(host, DefaultHostnameVerifier.getSubjectAltNames(cert, 7));
                break;
            }
            case IPv6: {
                DefaultHostnameVerifier.matchIPv6Address(host, DefaultHostnameVerifier.getSubjectAltNames(cert, 7));
                break;
            }
            default: {
                List<SubjectName> subjectAlts = DefaultHostnameVerifier.getSubjectAltNames(cert, 2);
                if (subjectAlts.isEmpty()) {
                    DefaultHostnameVerifier.matchCN(host, cert, this.publicSuffixMatcher);
                    break;
                }
    ...
    static String extractCN(String subjectPrincipal) throws SSLException {
        ...
        for (NameValuePair attribute : attributes) {
            if (TextUtils.isBlank((CharSequence)attribute.getName()) || attribute.getValue() == null) {
    ...
    static HostNameType determineHostFormat(String host) {
        ...
        if (InetAddressUtils.isIPv6Address((String)s)) {
    ...
    static List<SubjectName> getSubjectAltNames(X509Certificate cert) {
        return DefaultHostnameVerifier.getSubjectAltNames(cert, -1);
    }
    ...
    static List<SubjectName> getSubjectAltNames(X509Certificate cert, int subjectName) {
        try {
            ...
            for (List<?> entry : entries) {
                ...
                if (type == null || type != subjectName && -1 != subjectName) continue;
