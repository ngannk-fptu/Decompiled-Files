...
final class DistinguishedNameParser {
    ...
    private static final BitSet EQUAL_OR_COMMA_OR_PLUS = Tokenizer.INIT_BITSET((int[])new int[]{61, 44, 43});
    private static final BitSet COMMA_OR_PLUS = Tokenizer.INIT_BITSET((int[])new int[]{44, 43});
    ...
    List<NameValuePair> parse(CharArrayBuffer buf, Tokenizer.Cursor cursor) {
        ...
        this.tokenParser.skipWhiteSpace((CharSequence)buf, cursor);
        while (!cursor.atEnd()) {
        ...
        public void copyUnquotedContent(CharSequence buf, Tokenizer.Cursor cursor, BitSet delimiters, StringBuilder dst) {
            ...
            while (i < indexTo) {
                ...
                if (escaped) {
                    ...
                    if (delimiters != null && delimiters.get(current) || Tokenizer.isWhitespace((char)current) || current == '\"') break;
                    if (current == '\\') {
                        escaped = true;
                    } else {
                        dst.append(current);
