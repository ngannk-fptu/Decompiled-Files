...
final class MultihomeIOSessionRequester {
    ...
    public Future<IOSession> connect(final ConnectionInitiator connectionInitiator, final NamedEndpoint remoteEndpoint, final SocketAddress remoteAddress, final SocketAddress localAddress, final Timeout connectTimeout, final Object attachment, FutureCallback<IOSession> callback) {
        ...
        if (remoteAddress != null) {
            ...
            Future sessionFuture = connectionInitiator.connect(remoteEndpoint, remoteAddress, localAddress, connectTimeout, attachment, (FutureCallback)new FutureCallback<IOSession>(){
                ...
                public void failed(Exception cause) {
                    ...
                    if (cause instanceof IOException) {
                        ...
                        future.failed((Exception)ConnectExceptionSupport.enhance(iOException, remoteEndpoint, inetAddressArray));
                    } else {
                        future.failed(cause);
                    }
                }
                ...
                Future sessionFuture = connectionInitiator.connect(remoteEndpoint, (SocketAddress)remoteAddress, localAddress, connectTimeout, attachment, (FutureCallback)new FutureCallback<IOSession>(){
                    ...
                    public void failed(Exception cause) {
                        if (attempt.get() >= remoteAddresses.length) {
                            ...
                            if (cause instanceof IOException) {
                                future.failed((Exception)ConnectExceptionSupport.enhance((IOException)cause, remoteEndpoint, remoteAddresses));
                            } else {
                                future.failed(cause);
                            }
                        } else {
