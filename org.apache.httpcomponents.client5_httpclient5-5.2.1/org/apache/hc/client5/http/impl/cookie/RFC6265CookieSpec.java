...
implements CookieSpec {
    ...
    private static final BitSet TOKEN_DELIMS = Tokenizer.INIT_BITSET((int[])new int[]{61, 59});
    private static final BitSet VALUE_DELIMS = Tokenizer.INIT_BITSET((int[])new int[]{59});
    private static final BitSet SPECIAL_CHARS = Tokenizer.INIT_BITSET((int[])new int[]{32, 34, 44, 59, 92});
    ...
    public final List<Cookie> parse(Header header, CookieOrigin origin) throws MalformedCookieException {
        ...
        String value = this.tokenParser.parseValue((CharSequence)buffer, cursor, VALUE_DELIMS);
        if (!cursor.atEnd()) {
            cursor.updatePos(cursor.getPos() + 1);
        }
        ...
        while (!cursor.atEnd()) {
            String paramName = this.tokenParser.parseToken((CharSequence)buffer, cursor, TOKEN_DELIMS).toLowerCase(Locale.ROOT);
            String paramValue = null;
            if (!cursor.atEnd()) {
                char paramDelim = buffer.charAt(cursor.getPos());
                cursor.updatePos(cursor.getPos() + 1);
                if (paramDelim == '=') {
                    paramValue = this.tokenParser.parseToken((CharSequence)buffer, cursor, VALUE_DELIMS);
                    if (!cursor.atEnd()) {
                        cursor.updatePos(cursor.getPos() + 1);
                    }
                }
