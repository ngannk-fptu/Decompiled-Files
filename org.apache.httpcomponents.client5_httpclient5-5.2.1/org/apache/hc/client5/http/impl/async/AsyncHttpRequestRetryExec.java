...
implements AsyncExecChainHandler {
    ...
    private void internalExecute(final State state, final HttpRequest request, final AsyncEntityProducer entityProducer, final AsyncExecChain.Scope scope, AsyncExecChain chain, final AsyncExecCallback asyncExecCallback) throws HttpException, IOException {
        ...
        chain.proceed((HttpRequest)BasicRequestBuilder.copy((HttpRequest)request).build(), entityProducer, scope, new AsyncExecCallback(){
            ...
            public AsyncDataConsumer handleResponse(HttpResponse response, EntityDetails entityDetails) throws HttpException, IOException {
                ...
                state.retrying = AsyncHttpRequestRetryExec.this.retryStrategy.retryRequest(response, scope.execCount.get(), (HttpContext)clientContext);
                if (state.retrying) {
                    state.delay = AsyncHttpRequestRetryExec.this.retryStrategy.getRetryInterval(response, scope.execCount.get(), (HttpContext)clientContext);
                    if (LOG.isDebugEnabled()) {
            ...
            public void failed(Exception cause) {
                if (cause instanceof IOException) {
                    ...
                    if (entityProducer != null && !entityProducer.isRepeatable()) {
                    ...
                    } else if (AsyncHttpRequestRetryExec.this.retryStrategy.retryRequest(request, (IOException)cause, scope.execCount.get(), (HttpContext)clientContext)) {
                        if (LOG.isDebugEnabled()) {
                        ...
                        state.delay = AsyncHttpRequestRetryExec.this.retryStrategy.getRetryInterval(request, (IOException)cause, execCount - 1, (HttpContext)clientContext);
                        scope.scheduler.scheduleExecution(request, entityProducer, scope, asyncExecCallback, state.delay);
                        return;
                    }
                }
