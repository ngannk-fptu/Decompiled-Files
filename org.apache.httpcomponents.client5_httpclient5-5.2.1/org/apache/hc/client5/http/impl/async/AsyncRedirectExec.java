        ...
        chain.proceed(request, entityProducer, scope, new AsyncExecCallback(){
            ...
            public AsyncDataConsumer handleResponse(HttpResponse response, EntityDetails entityDetails) throws HttpException, IOException {
                ...
                if (config.isRedirectsEnabled() && AsyncRedirectExec.this.redirectStrategy.isRedirected(request, response, (HttpContext)clientContext)) {
                    HttpRoute newRoute;
                    BasicRequestBuilder redirectBuilder;
                    ...
                    switch (statusCode) {
                        ...
                        case 302: {
                            ...
                            redirectBuilder = BasicRequestBuilder.copy((HttpRequest)scope.originalRequest);
                            break;
                        }
                        case 303: {
                    ...
                    if (!Objects.equals(currentRoute.getTargetHost(), newTarget) && !Objects.equals(currentRoute, newRoute = AsyncRedirectExec.this.routePlanner.determineRoute(newTarget, (HttpContext)clientContext))) {
                        ...
                        state.currentScope = new AsyncExecChain.Scope(scope.exchangeId, newRoute, scope.originalRequest, scope.cancellableDependency, scope.clientContext, scope.execRuntime, scope.scheduler, scope.execCount);
                    }
                }
                if (state.redirectURI != null) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("{} redirecting to '{}' via {}", new Object[]{exchangeId, state.redirectURI, currentRoute});
                    }
                    return null;
                }
