...
implements AsyncExecChainHandler {
                    ...
                    this.createTunnel(state, proxy, target, scope, chain, new AsyncExecCallback(){
                        ...
                        public void completed() {
                            ...
                            if (state.challenged) {
                                ...
                                asyncExecCallback.failed((Exception)((Object)new TunnelRefusedException("Tunnel refused", null)));
                            } else {
                                if (LOG.isDebugEnabled()) {
    ...
    private void createTunnel(final State state, final HttpHost proxy, HttpHost nextHop, AsyncExecChain.Scope scope, AsyncExecChain chain, final AsyncExecCallback asyncExecCallback) throws HttpException, IOException {
        ...
        this.proxyHttpProcessor.process((HttpRequest)connect, null, (HttpContext)clientContext);
        this.authenticator.addAuthResponse(proxy, ChallengeType.PROXY, (HttpRequest)connect, proxyAuthExchange, (HttpContext)clientContext);
        chain.proceed((HttpRequest)connect, null, scope, new AsyncExecCallback(){
