...
extends AbstractMinimalHttpAsyncClientBase {
    ...
    MinimalH2AsyncClient(IOEventHandlerFactory eventHandlerFactory, AsyncPushConsumerRegistry pushConsumerRegistry, IOReactorConfig reactorConfig, ThreadFactory threadFactory, ThreadFactory workerThreadFactory, DnsResolver dnsResolver, TlsStrategy tlsStrategy) {
        ...
        this.connPool = new InternalH2ConnPool(this.connectionInitiator, (Resolver<HttpHost, InetSocketAddress>)((Resolver)object -> null), tlsStrategy);
    }
                ...
                Future<IOSession> sessionFuture = this.connPool.getSession(target, connectTimeout, new FutureCallback<IOSession>(){
                    ...
                    public void completed(IOSession session) {
                        ...
                        if (LOG.isDebugEnabled()) {
                            ...
                            session.enqueue((Command)new RequestExecutionCommand((AsyncClientExchangeHandler)new LoggingAsyncClientExchangeHandler(LOG, exchangeId, internalExchangeHandler), pushHandlerFactory, (CancellableDependency)cancellable, (HttpContext)clientContext), Command.Priority.NORMAL);
                        } else {
                            session.enqueue((Command)new RequestExecutionCommand(internalExchangeHandler, pushHandlerFactory, (CancellableDependency)cancellable, (HttpContext)clientContext), Command.Priority.NORMAL);
                        }
                    }
