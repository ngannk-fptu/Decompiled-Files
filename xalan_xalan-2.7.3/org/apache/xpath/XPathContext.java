...
extends DTMManager {
    ...
    public XPathContext(boolean recursiveVarContext) {
        ...
        this.m_variableStacks = recursiveVarContext ? new VariableStack() : new VariableStack(1);
    }
    ...
    private void assertion(boolean b, String msg) throws TransformerException {
        ...
        if (!b && (errorHandler = this.getErrorListener()) != null) {
    ...
    public void popRTFContext() {
        ...
        if (this.m_which_rtfdtm == previous) {
            if (previous >= 0) {
                boolean bl = ((SAX2RTFDTM)this.m_rtfdtm_stack.elementAt(previous)).popRewindMark();
            }
        } else {
            while (this.m_which_rtfdtm != previous) {
                boolean isEmpty = ((SAX2RTFDTM)this.m_rtfdtm_stack.elementAt(this.m_which_rtfdtm)).popRewindMark();
                --this.m_which_rtfdtm;
            }
        }
    }
