...
public abstract class NodeCounter {
    ...
    protected String formatNumbers(int[] values) {
        ...
        for (int i = 0; i < nValues; ++i) {
            if (values[i] == Integer.MIN_VALUE) continue;
            isEmpty = false;
        }
        if (isEmpty) {
            return "";
    ...
    private void formatValue(int value, String format, StringBuffer buffer) {
        int c = format.charAt(0);
        if (Character.isDigit((char)c)) {
            int i;
            char zero = (char)(c - Character.getNumericValue((char)c));
            ...
            for (i = 0; i < format.length() - s.length(); ++i) {
                temp.append(zero);
            }
            temp.append(s);
            if (this._groupSize > 0) {
        ...
        } else if (c == 105 && !this._letterValue.equals("alphabetic")) {
            buffer.append(this.romanValue(value));
        } else if (c == 73 && !this._letterValue.equals("alphabetic")) {
