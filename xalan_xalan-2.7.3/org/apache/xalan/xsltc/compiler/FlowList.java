...
public final class FlowList {
    ...
    public FlowList copyAndRedirect(InstructionList oldList, InstructionList newList) {
        ...
        int n = this._elements.size();
        Iterator<InstructionHandle> oldIter = oldList.iterator();
        Iterator<InstructionHandle> newIter = newList.iterator();
        while (oldIter.hasNext()) {
            InstructionHandle oldIh = oldIter.next();
            InstructionHandle newIh = newIter.next();
            for (int i = 0; i < n; ++i) {
                if (this._elements.elementAt(i) != oldIh) continue;
                result.add(newIh);
            }
        }
        return result;
