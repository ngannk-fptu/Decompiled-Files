...
Comparable<Calendar> {
    ...
    public final int getRelatedYear() {
        ...
        for (CalType testType : CalType.values()) {
            if (!typeString.equals(testType.getId())) continue;
            type = testType;
            break;
        }
        switch (type) {
    ...
    private static PatternData getPatternData(ULocale locale, String calType) {
        ...
        if (baseLocID.length() > 0 && !baseLocID.equals("und")) {
            ...
            if (validLoc != baseLoc && ((baseReg = baseLoc.getCountry()).length() > 0 && !baseReg.equals(validLoc.getCountry()) || !baseLoc.getLanguage().equals(validLoc.getLanguage()))) {
