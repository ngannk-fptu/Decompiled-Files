...
implements MultiplierProducer {
    ...
    public String getPattern(int magnitude, PluralRules rules, DecimalQuantity dq) {
        ...
        if (dq.isHasIntegerValue()) {
            ...
            if (i == 0L) {
                patternString = this.patterns[CompactData.getIndex(magnitude, StandardPlural.EQ_0)];
            } else if (i == 1L) {
                patternString = this.patterns[CompactData.getIndex(magnitude, StandardPlural.EQ_1)];
            }
            if (patternString != null) {
                return patternString;
            }
        }
        if ((patternString = this.patterns[CompactData.getIndex(magnitude, plural = dq.getStandardPlural(rules))]) == null && plural != StandardPlural.OTHER) {
