...
Serializable {
    ...
    public AbstractAspectJAdvice(Method aspectJAdviceMethod, AspectJExpressionPointcut pointcut, AspectInstanceFactory aspectInstanceFactory) {
        Assert.notNull((Object)aspectJAdviceMethod, (String)"Advice method must not be null");
    ...
    public void setArgumentNames(String argNames) {
        String[] tokens = StringUtils.commaDelimitedListToStringArray((String)argNames);
    ...
    public void setArgumentNamesFromStringArray(String ... args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            this.argumentNames[i] = StringUtils.trimWhitespace((String)args[i]);
            if (AbstractAspectJAdvice.isVariableName(this.argumentNames[i])) continue;
    ...
    protected void setReturningNameNoCheck(String name) {
        if (AbstractAspectJAdvice.isVariableName(name)) {
            ...
            try {
                this.discoveredReturningType = ClassUtils.forName((String)name, (ClassLoader)this.getAspectClassLoader());
            }
            catch (Throwable ex) {
    ...
    protected void setThrowingNameNoCheck(String name) {
        if (AbstractAspectJAdvice.isVariableName(name)) {
            ...
            try {
                this.discoveredThrowingType = ClassUtils.forName((String)name, (ClassLoader)this.getAspectClassLoader());
            }
            catch (Throwable ex) {
    ...
    protected ParameterNameDiscoverer createParameterNameDiscoverer() {
        ...
        discoverer.addDiscoverer((ParameterNameDiscoverer)adviceParameterNameDiscoverer);
        return discoverer;
    }

    private void bindExplicitArguments(int numArgumentsLeftToBind) {
        int argumentIndexOffset;
        Assert.state((this.argumentNames != null ? 1 : 0) != 0, (String)"No argument names available");
        ...
        for (int i = argumentIndexOffset = this.parameterTypes.length - numArgumentsLeftToBind; i < this.argumentNames.length; ++i) {
    ...
    private void configurePointcutParameters(String[] argumentNames, int argumentIndexOffset) {
        ...
        for (int i = 0; i < argumentNames.length; ++i) {
            if (i < argumentIndexOffset || argumentNames[i].equals(this.returningName) || argumentNames[i].equals(this.throwingName)) continue;
    ...
    protected Object invokeAdviceMethodWithGivenArgs(Object[] args) throws Throwable {
        ...
        try {
            ReflectionUtils.makeAccessible((Method)this.aspectJAdviceMethod);
