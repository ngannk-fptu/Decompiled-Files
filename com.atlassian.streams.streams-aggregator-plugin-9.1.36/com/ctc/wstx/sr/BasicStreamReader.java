...
LocationInfo {
    ...
    protected BasicStreamReader(InputBootstrapper bs, BranchingReaderSource input, ReaderCreator owner, ReaderConfig cfg, InputElementStack elemStack, boolean forER) throws XMLStreamException {
        ...
        if (this.mCfgCoalesceText) {
            ...
            this.mShortestTextSegment = forER ? Integer.MAX_VALUE : cfg.getShortestReportedTextSegment();
        }
        ...
        input.initInputLocation(this, this.mCurrDepth);
    ...
    public String getElementText() throws XMLStreamException {
        ...
        while ((type = this.next()) != 2) {
            if ((1 << type & 0x1250) != 0) {
                ...
                if ((long)sb.length() > this.mConfig.getMaxTextLength()) {
                    this.mPendingException = this.throwWfcException("Text size limit (" + this.mConfig.getMaxTextLength() + ") exceeded", false);
                }
    ...
    public char[] getTextCharacters() {
        if ((1 << this.mCurrToken & 0x1070) == 0) {
            this.throwNotTextXxx(this.mCurrToken);
        }
        if (this.mTokenState < this.mStTextThreshold) {
            this.safeFinishToken();
        }
    ...
    public final int next() throws XMLStreamException {
        ...
        if (this.mParseState == 1) {
            ...
            if (type == 12) {
                ...
                if ((long)this.mCurrTextLength > this.mConfig.getMaxTextLength()) {
                    this.mPendingException = this.throwWfcException("Text size limit (" + this.mConfig.getMaxTextLength() + ") exceeded", false);
                }
            } else if (type == 4) {
                if (this.mValidateText) {
    ...
    private final void parseAttrValue(char openingQuote, TextBuilder tb) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (outPtr >= outLen) {
                if (tb.getCharSize() >= this.mConfig.getMaxAttributeSize()) {
                    this.throwParseError("Maximum attribute size (" + this.mConfig.getMaxAttributeSize() + ") exceeded");
                }
                outBuf = tb.bufferFull(1);
                outLen = outBuf.length;
            }
    ...
    private void skipCommentOrCData(String errorMsg, char endChar, boolean preventDoubles) throws XMLStreamException {
        ...
        while (true) {
            char c;
            char c2 = c = this.mInputPtr < this.mInputEnd ? this.mInputBuffer[this.mInputPtr++] : this.getNextCharFromCurrent(errorMsg);
            if (c < ' ') {
                if (c == '\n' || c == '\r') {
                    this.skipCRLF(c);
                } else if (c != '\t') {
            ...
            if ((long)(++count) > this.mConfig.getMaxTextLength()) {
                this.mPendingException = this.throwWfcException("Text size limit (" + this.mConfig.getMaxTextLength() + ") exceeded", false);
            }
        ...
        switch (this.mCurrToken) {
            case 12: {
                if (this.mCfgCoalesceText) {
                    ...
                    this.mTokenState = this.readCDataSecondary(this.mShortestTextSegment) ? 3 : 2;
                }
                return;
            }
            case 4: {
    ...
    private void readComment2(TextBuffer tb) throws XMLStreamException {
        ...
        while (true) {
            ...
            if (outPtr >= outLen) {
                ...
                outLen = outBuf.length;
                outPtr = 0;
                if ((long)this.mTextBuffer.size() > this.mConfig.getMaxTextLength()) {
                    this.mPendingException = this.throwWfcException("Text size limit (" + this.mConfig.getMaxTextLength() + ") exceeded", false);
                }
            }
            outBuf[outPtr++] = c;
    ...
    private final int readPIPrimary() throws XMLStreamException {
        ...
        if (target.equalsIgnoreCase("xml")) {
            ...
            if (!BasicStreamReader.isSpaceChar(c2 = this.getNextCharFromCurrent(" in xml declaration"))) {
                this.throwUnexpectedChar(c2, "excepted a space in xml declaration after 'xml'");
            }
    ...
    protected final boolean readTextSecondary(int shortestSegment, boolean deferErrors) throws XMLStreamException {
        ...
        while (true) {
            ...
            if ((long)this.mTextBuffer.size() > this.mConfig.getMaxTextLength()) {
                this.mPendingException = this.throwWfcException("Text size limit (" + this.mConfig.getMaxTextLength() + ") exceeded", deferErrors);
                return false;
            }
            outPtr = 0;
        }
    ...
    private int readAndWriteText(Writer w) throws IOException, XMLStreamException {
        ...
        while (true) {
            ...
            if (c == '>') {
                if (this.mInputPtr < 2 || this.mInputBuffer[this.mInputPtr - 2] != ']' || this.mInputBuffer[this.mInputPtr - 1] != ']') continue;
                len = this.mInputPtr - start;
                if (len > 0) {
                    w.write(this.mInputBuffer, start, len);
                }
    ...
    protected EntityDecl findEntity(String id, Object arg) throws XMLStreamException {
        ...
        if (ed == null && this.mGeneralEntities != null) {
            ed = (EntityDecl)this.mGeneralEntities.get(id);
        }
    ...
    private void throwNotTextXxx(int type) {
        throw new IllegalStateException("getTextXxx() methods can not be called on " + this.tokenTypeDesc(this.mCurrToken));
    }
