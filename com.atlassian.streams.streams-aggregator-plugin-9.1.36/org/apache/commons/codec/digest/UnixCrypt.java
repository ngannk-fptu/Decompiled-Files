...
public class UnixCrypt {
    ...
    public static String crypt(byte[] original, String salt) {
        ...
        for (i = 0; i < key.length && i < original.length; ++i) {
            byte iChar = original[i];
            key[i] = (byte)(iChar << 1);
        }
        int[] schedule = UnixCrypt.desSetKey(key);
        int[] out = UnixCrypt.body(schedule, eSwap0, eSwap1);
        byte[] b = new byte[9];
        UnixCrypt.intToFourBytes(out[0], b, 0);
        UnixCrypt.intToFourBytes(out[1], b, 4);
        b[8] = 0;
        int y = 0;
        int u = 128;
        for (int i2 = 2; i2 < 13; ++i2) {
            ...
            for (int j = 0; j < 6; ++j) {
                ...
                buffer.setCharAt(i2, (char)COV2CHAR[c]);
            }
        }
    ...
    public static String crypt(String original) {
        return UnixCrypt.crypt(original.getBytes(StandardCharsets.UTF_8));
    }

    public static String crypt(String original, String salt) {
        return UnixCrypt.crypt(original.getBytes(StandardCharsets.UTF_8), salt);
    }
    ...
    private static int[] body(int[] schedule, int eSwap0, int eSwap1) {
        ...
        for (int j = 0; j < 25; ++j) {
            for (int i = 0; i < 32; i += 4) {
                left = UnixCrypt.dEncrypt(left, right, i, eSwap0, eSwap1, schedule);
                right = UnixCrypt.dEncrypt(right, left, i + 2, eSwap0, eSwap1, schedule);
            }
            t = left;
            left = right;
            right = t;
    ...
    private static int dEncrypt(int el, int r, int s, int e0, int e1, int[] sArr) {
        ...
        int u = v & e0;
        u = u ^ u << 16 ^ r ^ sArr[s];
        int t = (v &= e1) ^ v << 16 ^ r ^ sArr[s + 1];
        t = t >>> 4 | t << 28;
        return el ^= SPTRANS[1][t & 0x3F] | SPTRANS[3][t >>> 8 & 0x3F] | SPTRANS[5][t >>> 16 & 0x3F] | SPTRANS[7][t >>> 24 & 0x3F] | SPTRANS[0][u & 0x3F] | SPTRANS[2][u >>> 8 & 0x3F] | SPTRANS[4][u >>> 16 & 0x3F] | SPTRANS[6][u >>> 24 & 0x3F];
    }

    private static int[] desSetKey(byte[] key) {
        int[] schedule = new int[32];
        int c = UnixCrypt.fourBytesToInt(key, 0);
        int d = UnixCrypt.fourBytesToInt(key, 4);
        int[] results = new int[2];
        UnixCrypt.permOp(d, c, 4, 0xF0F0F0F, results);
        d = results[0];
        c = results[1];
        c = UnixCrypt.hPermOp(c, -2, -859045888);
        d = UnixCrypt.hPermOp(d, -2, -859045888);
        UnixCrypt.permOp(d, c, 1, 0x55555555, results);
        d = results[0];
        c = results[1];
        UnixCrypt.permOp(c, d, 8, 0xFF00FF, results);
        c = results[0];
        d = results[1];
        UnixCrypt.permOp(d, c, 1, 0x55555555, results);
        d = results[0];
    ...
    private static int fourBytesToInt(byte[] b, int offset) {
        int value = UnixCrypt.byteToUnsigned(b[offset++]);
        value |= UnixCrypt.byteToUnsigned(b[offset++]) << 8;
        value |= UnixCrypt.byteToUnsigned(b[offset++]) << 16;
        return value |= UnixCrypt.byteToUnsigned(b[offset++]) << 24;
    }
