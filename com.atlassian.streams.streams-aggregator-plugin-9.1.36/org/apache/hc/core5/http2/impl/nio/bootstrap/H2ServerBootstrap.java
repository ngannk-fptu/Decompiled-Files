...
public class H2ServerBootstrap {
    ...
    public final <T> H2ServerBootstrap register(String uriPattern, final AsyncServerRequestHandler<T> requestHandler) {
    ...
    public final <T> H2ServerBootstrap registerVirtual(String hostname, String uriPattern, final AsyncServerRequestHandler<T> requestHandler) {
    ...
    public HttpAsyncServer create() {
        ...
        RequestHandlerRegistry<Supplier<AsyncServerExchangeHandler>> registry = new RequestHandlerRegistry<Supplier<AsyncServerExchangeHandler>>(actualCanonicalHostName, new Supplier<LookupRegistry<Supplier<AsyncServerExchangeHandler>>>(){

            @Override
            public LookupRegistry<Supplier<AsyncServerExchangeHandler>> get() {
                return H2ServerBootstrap.this.lookupRegistry != null ? H2ServerBootstrap.this.lookupRegistry : UriPatternType.newMatcher(UriPatternType.URI_PATTERN);
            }
        });
        ...
        if (!this.filters.isEmpty()) {
            ...
            handlerFactory = new DefaultAsyncResponseExchangeHandlerFactory(registry, new Decorator<AsyncServerExchangeHandler>(){
                ...
                public AsyncServerExchangeHandler decorate(AsyncServerExchangeHandler handler) {
                    return new BasicAsyncServerExpectationDecorator(handler, H2ServerBootstrap.this.exceptionCallback);
                }
            });
        }
        ...
        HttpVersionPolicy actualVersionProtocol = this.versionPolicy != null ? this.versionPolicy : HttpVersionPolicy.NEGOTIATE;
        ...
        ServerHttpProtocolNegotiatorFactory ioEventHandlerFactory = new ServerHttpProtocolNegotiatorFactory(http1StreamHandlerFactory, http2StreamHandlerFactory, actualVersionProtocol, actualTlsStrategy, this.handshakeTimeout);
