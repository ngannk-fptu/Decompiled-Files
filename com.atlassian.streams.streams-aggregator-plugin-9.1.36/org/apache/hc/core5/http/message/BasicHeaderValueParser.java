...
implements HeaderValueParser {
    ...
    private static final BitSet TOKEN_DELIMS = Tokenizer.INIT_BITSET(61, 59, 44);
    private static final BitSet VALUE_DELIMS = Tokenizer.INIT_BITSET(59, 44);
    ...
    public NameValuePair parseNameValuePair(CharSequence buffer, ParserCursor cursor) {
        Args.notNull(buffer, "Char sequence");
        Args.notNull(cursor, "Parser cursor");
        String name = this.tokenizer.parseToken(buffer, cursor, TOKEN_DELIMS);
        if (cursor.atEnd()) {
        ...
        String value = this.tokenizer.parseValue(buffer, cursor, VALUE_DELIMS);
        if (!cursor.atEnd()) {
            cursor.updatePos(cursor.getPos() + 1);
        }
