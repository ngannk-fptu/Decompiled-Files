...
extends AbstractMap<Object, V> {
    ...
    public V put(CharSequence text, V value) {
        return this.put(((Object)text).toString(), value);
    }
    ...
    public V put(char[] text, V value) {
        int slot;
        if (this.ignoreCase) {
            for (int i = 0; i < text.length; i += Character.toChars(Character.toLowerCase(this.charUtils.codePointAt(text, i)), text, i)) {
            }
        }
    ...
    private boolean equals(char[] text1, int off, int len, char[] text2) {
        ...
        if (this.ignoreCase) {
            ...
            for (int i = 0; i < len; i += Character.charCount(codePointAt)) {
                ...
                if (Character.toLowerCase(codePointAt) == this.charUtils.codePointAt(text2, i)) continue;
                return false;
            }
        } else {
            for (int i = 0; i < len; ++i) {
