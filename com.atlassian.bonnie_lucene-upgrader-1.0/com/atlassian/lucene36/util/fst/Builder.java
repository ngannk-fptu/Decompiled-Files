...
import com.atlassian.lucene36.util.fst.NodeHash;
...
public class Builder<T> {
    ...
    public Builder(FST.INPUT_TYPE inputType, int minSuffixCount1, int minSuffixCount2, boolean doShareSuffix, boolean doShareNonSingletonNodes, int shareMaxTailLength, Outputs<T> outputs, FreezeTail<T> freezeTail, boolean willPackFST) {
        this.minSuffixCount1 = minSuffixCount1;
        this.minSuffixCount2 = minSuffixCount2;
        this.freezeTail = freezeTail;
        this.doShareNonSingletonNodes = doShareNonSingletonNodes;
        this.shareMaxTailLength = shareMaxTailLength;
        this.fst = new FST<T>(inputType, outputs, willPackFST);
        this.dedupHash = doShareSuffix ? new NodeHash<T>(this.fst) : null;
    ...
    public void setAllowArrayArcs(boolean b) {
        this.fst.setAllowArrayArcs(b);
    }

    private CompiledNode compileNode(UnCompiledNode<T> nodeIn, int tailLength) throws IOException {
        int node = this.dedupHash != null && (this.doShareNonSingletonNodes || nodeIn.numArcs <= 1) && tailLength <= this.shareMaxTailLength ? (nodeIn.numArcs == 0 ? this.fst.addNode(nodeIn) : this.dedupHash.add(nodeIn)) : this.fst.addNode(nodeIn);
        assert (node != -2);
        nodeIn.clear();
