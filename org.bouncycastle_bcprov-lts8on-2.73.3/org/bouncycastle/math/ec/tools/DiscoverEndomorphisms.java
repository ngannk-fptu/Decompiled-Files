...
public class DiscoverEndomorphisms {
    ...
    public static void main(String[] args) {
        if (args.length > 0) {
            for (int i = 0; i < args.length; ++i) {
                DiscoverEndomorphisms.discoverEndomorphisms(args[i]);
            }
        } else {
            TreeSet curveNames = new TreeSet(DiscoverEndomorphisms.enumToList(ECNamedCurveTable.getNames()));
            curveNames.addAll(DiscoverEndomorphisms.enumToList(CustomNamedCurves.getNames()));
            Iterator it = curveNames.iterator();
            while (it.hasNext()) {
                DiscoverEndomorphisms.discoverEndomorphisms((String)it.next());
            }
        }
    }
    ...
    public static void discoverEndomorphisms(X9ECParameters x9) {
        ...
        DiscoverEndomorphisms.discoverEndomorphisms(x9, "<UNKNOWN>");
    }
    ...
    private static void discoverEndomorphisms(String curveName) {
        ...
        DiscoverEndomorphisms.discoverEndomorphisms(x9, curveName);
    }
    ...
    private static void discoverEndomorphisms(X9ECParameters x9, String displayName) {
        ...
        if (ECAlgorithms.isFpCurve(c)) {
            ...
            if (c.getB().isZero() && characteristic.mod(ECConstants.FOUR).equals(ECConstants.ONE)) {
                ...
                DiscoverEndomorphisms.printGLVTypeAParameters(x9);
            }
            if (c.getA().isZero() && characteristic.mod(ECConstants.THREE).equals(ECConstants.ONE)) {
                ...
                DiscoverEndomorphisms.printGLVTypeBParameters(x9);
            }
        }
    }

    private static void printGLVTypeAParameters(X9ECParameters x9) {
        BigInteger[] lambdas = DiscoverEndomorphisms.solveQuadraticEquation(x9.getN(), ECConstants.ONE, ECConstants.ZERO, ECConstants.ONE);
        ECFieldElement[] iValues = DiscoverEndomorphisms.findNonTrivialOrder4FieldElements(x9.getCurve());
        DiscoverEndomorphisms.printGLVTypeAParameters(x9, lambdas[0], iValues);
        System.out.println("OR");
        DiscoverEndomorphisms.printGLVTypeAParameters(x9, lambdas[1], iValues);
    }
    ...
    private static void printGLVTypeAParameters(X9ECParameters x9, BigInteger lambda, ECFieldElement[] iValues) {
        ...
        DiscoverEndomorphisms.printProperty("i", i.toBigInteger().toString(16));
        DiscoverEndomorphisms.printProperty("lambda", lambda.toString(16));
        DiscoverEndomorphisms.printScalarDecompositionParameters(x9.getN(), lambda);
    }

    private static void printGLVTypeBParameters(X9ECParameters x9) {
        BigInteger[] lambdas = DiscoverEndomorphisms.solveQuadraticEquation(x9.getN(), ECConstants.ONE, ECConstants.ONE, ECConstants.ONE);
        ECFieldElement[] betaValues = DiscoverEndomorphisms.findNonTrivialOrder3FieldElements(x9.getCurve());
        DiscoverEndomorphisms.printGLVTypeBParameters(x9, lambdas[0], betaValues);
        System.out.println("OR");
        DiscoverEndomorphisms.printGLVTypeBParameters(x9, lambdas[1], betaValues);
    }
    ...
    private static void printGLVTypeBParameters(X9ECParameters x9, BigInteger lambda, ECFieldElement[] betaValues) {
        ...
        DiscoverEndomorphisms.printProperty("beta", beta.toBigInteger().toString(16));
        DiscoverEndomorphisms.printProperty("lambda", lambda.toString(16));
        DiscoverEndomorphisms.printScalarDecompositionParameters(x9.getN(), lambda);
    }
    ...
    private static void printScalarDecompositionParameters(BigInteger n, BigInteger lambda) {
        ...
        BigInteger[] v1 = null;
        BigInteger[] v2 = null;
        BigInteger[] rt = DiscoverEndomorphisms.extEuclidGLV(n, lambda);
        v1 = new BigInteger[]{rt[2], rt[3].negate()};
        v2 = DiscoverEndomorphisms.chooseShortest(new BigInteger[]{rt[0], rt[1].negate()}, new BigInteger[]{rt[4], rt[5].negate()});
        if (!DiscoverEndomorphisms.isVectorBoundedBySqrt(v2, n) && DiscoverEndomorphisms.areRelativelyPrime(v1[0], v1[1]) && (vw = DiscoverEndomorphisms.extEuclidBezout(new BigInteger[]{(s = (r = v1[0]).add((t = v1[1]).multiply(lambda)).divide(n)).abs(), t.abs()})) != null) {
            BigInteger[] I2;
            BigInteger check;
            BigInteger v = vw[0];
            BigInteger w = vw[1];
            ...
            if (!(check = s.multiply(v).subtract(t.multiply(w))).equals(ECConstants.ONE)) {
                throw new IllegalStateException();
            }
            BigInteger x = w.multiply(n).subtract(v.multiply(lambda));
            BigInteger base1 = v.negate();
            BigInteger base2 = x.negate();
            BigInteger sqrtN = DiscoverEndomorphisms.isqrt(n.subtract(ECConstants.ONE)).add(ECConstants.ONE);
            BigInteger[] I1 = DiscoverEndomorphisms.calculateRange(base1, sqrtN, t);
            BigInteger[] range = DiscoverEndomorphisms.intersect(I1, I2 = DiscoverEndomorphisms.calculateRange(base2, sqrtN, r));
            if (range != null) {
                BigInteger alpha = range[0];
                while (alpha.compareTo(range[1]) <= 0) {
                    BigInteger[] candidate = new BigInteger[]{x.add(alpha.multiply(r)), v.add(alpha.multiply(t))};
                    if (DiscoverEndomorphisms.isShorter(candidate, v2)) {
                        v2 = candidate;
                    }
                    alpha = alpha.add(ECConstants.ONE);
                }
            }
        }
        BigInteger d = v1[0].multiply(v2[1]).subtract(v1[1].multiply(v2[0]));
        int bits = n.bitLength() + 16 - (n.bitLength() & 7);
        BigInteger g1 = DiscoverEndomorphisms.roundQuotient(v2[1].shiftLeft(bits), d);
        BigInteger g2 = DiscoverEndomorphisms.roundQuotient(v1[1].shiftLeft(bits), d).negate();
        DiscoverEndomorphisms.printProperty("v1", "{ " + v1[0].toString(16) + ", " + v1[1].toString(16) + " }");
        DiscoverEndomorphisms.printProperty("v2", "{ " + v2[0].toString(16) + ", " + v2[1].toString(16) + " }");
        DiscoverEndomorphisms.printProperty("d", d.toString(16));
        DiscoverEndomorphisms.printProperty("(OPT) g1", g1.toString(16));
        DiscoverEndomorphisms.printProperty("(OPT) g2", g2.toString(16));
        DiscoverEndomorphisms.printProperty("(OPT) bits", Integer.toString(bits));
    }
    ...
    private static BigInteger[] calculateRange(BigInteger mid, BigInteger off, BigInteger div) {
        ...
        return DiscoverEndomorphisms.order(i1, i2);
    }
    ...
    private static BigInteger[] extEuclidBezout(BigInteger[] ab) {
        boolean swap;
        boolean bl = swap = ab[0].compareTo(ab[1]) < 0;
        if (swap) {
            DiscoverEndomorphisms.swap(ab);
        }
        BigInteger r0 = ab[0];
        BigInteger r1 = ab[1];
    ...
    private static BigInteger[] extEuclidGLV(BigInteger n, BigInteger lambda) {
        ...
        while (true) {
            ...
            BigInteger r2 = qr[1];
            BigInteger t2 = t0.subtract(q.multiply(t1));
            if (DiscoverEndomorphisms.isLessThanSqrt(r1, n)) {
    ...
    private static boolean isLessThanSqrt(BigInteger a, BigInteger b) {
        a = a.abs();
        b = b.abs();
        int target = b.bitLength();
        int maxBits = a.bitLength() * 2;
    ...
    private static BigInteger[] solveQuadraticEquation(BigInteger n, BigInteger a, BigInteger b, BigInteger c) {
        ...
        BigInteger root = DiscoverEndomorphisms.modSqrt(det, n);
        if (root == null) {
    ...
    private static ECFieldElement[] findNonTrivialOrder3FieldElements(ECCurve c) {
        ...
        while ((b = (r = BigIntegers.createRandomInRange(ECConstants.TWO, q.subtract(ECConstants.TWO), random)).modPow(e, q)).equals(ECConstants.ONE)) {
        }
    ...
    private static BigInteger isqrt(BigInteger x) {
        ...
        BigInteger g1;
        while (!(g1 = g0.add(x.divide(g0)).shiftRight(1)).equals(g0)) {
            g0 = g1;
        }
        return g1;
    }
    ...
    private static BigInteger modSqrt(BigInteger x, BigInteger p) {
        ...
        while (!q.testBit(0)) {
            if (x.modPow(q = q.shiftRight(1), p).equals(ECConstants.ONE)) continue;
            return DiscoverEndomorphisms.modSqrtComplex(x, q, p, pSub1Halved);
        }
    ...
    private static BigInteger modSqrtComplex(BigInteger x, BigInteger q, BigInteger p, BigInteger pSub1Halved) {
        BigInteger t;
        BigInteger a = DiscoverEndomorphisms.firstNonResidue(p, pSub1Halved);
        BigInteger negPow = t = pSub1Halved;
        while (!q.testBit(0)) {
            q = q.shiftRight(1);
            t = t.shiftRight(1);
            if (x.modPow(q, p).equals(a.modPow(t, p))) continue;
            t = t.add(negPow);
        }
    ...
    private static BigInteger firstNonResidue(BigInteger p, BigInteger pSub1Halved) {
        for (int a = 2; a < 1000; ++a) {
            ...
            if (A.modPow(pSub1Halved, p).equals(ECConstants.ONE)) continue;
            return A;
        }
        throw new IllegalStateException();
    }
