...
extends ECPoint.AbstractF2m {
    SecT131R2Point(ECCurve curve, ECFieldElement x, ECFieldElement y) {
        super(curve, x, y);
    }

    SecT131R2Point(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs) {
        super(curve, x, y, zs);
    }
    ...
    public ECPoint twicePlus(ECPoint b) {
        ...
        if (A.isZero()) {
            return new SecT131R2Point(curve, A, curve.getB().sqrt());
        }
    ...
    public ECPoint negate() {
        ...
        ECFieldElement L = this.y;
        ECFieldElement Z = this.zs[0];
        return new SecT131R2Point(this.curve, X, L.add(Z), new ECFieldElement[]{Z});
    }
}
