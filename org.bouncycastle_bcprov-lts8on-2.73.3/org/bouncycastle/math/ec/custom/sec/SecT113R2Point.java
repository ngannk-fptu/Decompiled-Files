...
extends ECPoint.AbstractF2m {
    SecT113R2Point(ECCurve curve, ECFieldElement x, ECFieldElement y) {
        super(curve, x, y);
    }

    SecT113R2Point(ECCurve curve, ECFieldElement x, ECFieldElement y, ECFieldElement[] zs) {
        super(curve, x, y, zs);
    }
    ...
    public ECPoint add(ECPoint b) {
        ...
        if (X2.isZero()) {
            ...
            if (X3.isZero()) {
                return new SecT113R2Point(curve, X3, curve.getB().sqrt());
            }
    ...
    public ECPoint twicePlus(ECPoint b) {
        ...
        if (A.isZero()) {
            return new SecT113R2Point(curve, A, curve.getB().sqrt());
        }
    ...
    public ECPoint negate() {
        ...
        ECFieldElement L = this.y;
        ECFieldElement Z = this.zs[0];
        return new SecT113R2Point(this.curve, X, L.add(Z), new ECFieldElement[]{Z});
    }
}
