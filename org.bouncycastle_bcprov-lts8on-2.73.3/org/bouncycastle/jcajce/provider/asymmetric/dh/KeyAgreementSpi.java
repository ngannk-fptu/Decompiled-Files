...
extends BaseAgreementSpi {
    ...
    protected Key engineDoPhase(Key key, boolean lastPhase) throws InvalidKeyException, IllegalStateException {
        ...
        if (this.unifiedAgreement != null) {
            ...
            DHPublicKeyParameters staticKey = this.generatePublicKeyParameter((PublicKey)key);
            DHPublicKeyParameters ephemKey = this.generatePublicKeyParameter(this.dheParameters.getOtherPartyEphemeralKey());
        ...
        if (this.mqvAgreement != null) {
            ...
            DHPublicKeyParameters staticKey = this.generatePublicKeyParameter((PublicKey)key);
            DHPublicKeyParameters ephemKey = this.generatePublicKeyParameter(this.mqvParameters.getOtherPartyEphemeralKey());
            DHMQVPublicParameters pKey = new DHMQVPublicParameters(staticKey, ephemKey);
            this.result = this.bigIntToBytes(this.mqvAgreement.calculateAgreement(pKey));
            return null;
        }
        ...
        this.result = this.bigIntToBytes(res);
        if (lastPhase) {
            return null;
        }
        ...
        if (params != null) {
            if (params instanceof DHParameterSpec) {
                ...
                if (this.dheParameters.getEphemeralPublicKey() != null) {
                    this.unifiedAgreement.init(new DHUPrivateParameters(this.generatePrivateKeyParameter(privKey), this.generatePrivateKeyParameter(this.dheParameters.getEphemeralPrivateKey()), this.generatePublicKeyParameter(this.dheParameters.getEphemeralPublicKey())));
                } else {
                    this.unifiedAgreement.init(new DHUPrivateParameters(this.generatePrivateKeyParameter(privKey), this.generatePrivateKeyParameter(this.dheParameters.getEphemeralPrivateKey())));
                }
                ...
                if (this.mqvParameters.getEphemeralPublicKey() != null) {
                    this.mqvAgreement.init(new DHMQVPrivateParameters(this.generatePrivateKeyParameter(privKey), this.generatePrivateKeyParameter(this.mqvParameters.getEphemeralPrivateKey()), this.generatePublicKeyParameter(this.mqvParameters.getEphemeralPublicKey())));
                } else {
                    this.mqvAgreement.init(new DHMQVPrivateParameters(this.generatePrivateKeyParameter(privKey), this.generatePrivateKeyParameter(this.mqvParameters.getEphemeralPrivateKey())));
                }
            } else {
                if (!(params instanceof UserKeyingMaterialSpec)) throw new InvalidAlgorithmParameterException("DHKeyAgreement only accepts DHParameterSpec");
                if (this.kdf == null) {
