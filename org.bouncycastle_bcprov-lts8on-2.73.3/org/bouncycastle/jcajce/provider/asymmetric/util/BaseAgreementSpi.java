...
extends KeyAgreementSpi {
    ...
    protected void engineInit(Key key, SecureRandom random) throws InvalidKeyException {
        try {
            this.doInitFromKey(key, null, random);
        }
    ...
    protected void engineInit(Key key, AlgorithmParameterSpec params, SecureRandom random) throws InvalidKeyException, InvalidAlgorithmParameterException {
        if (params instanceof HybridValueParameterSpec) {
            ...
            this.doInitFromKey(key, this.hybridSpec.getBaseParameterSpec(), random);
        } else {
            this.hybridSpec = null;
            this.doInitFromKey(key, params, random);
        }
    }
    ...
    protected byte[] engineGenerateSecret() throws IllegalStateException {
        if (this.kdf != null) {
            ...
            try {
                return this.getSharedSecretBytes(secret, null, secret.length * 8);
            }
    ...
    protected SecretKey engineGenerateSecret(String algorithm) throws NoSuchAlgorithmException {
        ...
        int keySize = BaseAgreementSpi.getKeySize(oidAlgorithm);
        byte[] secret = this.getSharedSecretBytes(this.calcSecret(), oidAlgorithm, keySize);
        String algName = BaseAgreementSpi.getAlgorithm(algorithm);
    ...
    private byte[] getSharedSecretBytes(byte[] secret, String oidAlgorithm, int keySize) throws NoSuchAlgorithmException {
        if (this.kdf != null) {
            ...
            if (this.kdf instanceof DHKEKGenerator) {
                ...
                DHKDFParameters params = new DHKDFParameters(oid, keySize, secret, this.ukmParameters);
    ...
    private byte[] calcSecret() {
        if (this.hybridSpec != null) {
            byte[] s = this.doCalcSecret();
            byte[] sec = Arrays.concatenate(s, this.hybridSpec.getT());
            Arrays.clear(s);
            return sec;
        }
