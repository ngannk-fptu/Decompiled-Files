...
implements BlockCipher {
    private static final byte[][] C = new byte[][]{Hex.decodeStrict("517cc1b727220a94fe13abe8fa9a6ee0"), Hex.decodeStrict("6db14acc9e21c820ff28b1d5ef5de2b0"), Hex.decodeStrict("db92371d2126e9700324977504e8c90e")};
    ...
    public void init(boolean forEncryption, CipherParameters params) throws IllegalArgumentException {
        ...
        this.roundKeys = ARIAEngine.keySchedule(forEncryption, ((KeyParameter)params).getKey());
    ...
    public int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws DataLengthException, IllegalStateException {
        ...
        int i = 0;
        int rounds = this.roundKeys.length - 3;
        while (i < rounds) {
            ARIAEngine.FO(z, this.roundKeys[i++]);
            ARIAEngine.FE(z, this.roundKeys[i++]);
        }
        ARIAEngine.FO(z, this.roundKeys[i++]);
        ARIAEngine.xor(z, this.roundKeys[i++]);
        ARIAEngine.SL2(z);
        ARIAEngine.xor(z, this.roundKeys[i]);
    ...
    protected static byte[][] keySchedule(boolean forEncryption, byte[] K) {
        ...
        ARIAEngine.FO(W1, CK1);
        ARIAEngine.xor(W1, KR);
        System.arraycopy(W1, 0, W2, 0, 16);
        ARIAEngine.FE(W2, CK2);
        ARIAEngine.xor(W2, W0);
        System.arraycopy(W2, 0, W3, 0, 16);
        ARIAEngine.FO(W3, CK3);
        ARIAEngine.xor(W3, W1);
        int numRounds = 12 + keyLenIdx * 2;
        byte[][] rks = new byte[numRounds + 1][16];
        ARIAEngine.keyScheduleRound(rks[0], W0, W1, 19);
        ARIAEngine.keyScheduleRound(rks[1], W1, W2, 19);
        ARIAEngine.keyScheduleRound(rks[2], W2, W3, 19);
        ARIAEngine.keyScheduleRound(rks[3], W3, W0, 19);
        ARIAEngine.keyScheduleRound(rks[4], W0, W1, 31);
        ARIAEngine.keyScheduleRound(rks[5], W1, W2, 31);
        ARIAEngine.keyScheduleRound(rks[6], W2, W3, 31);
        ARIAEngine.keyScheduleRound(rks[7], W3, W0, 31);
        ARIAEngine.keyScheduleRound(rks[8], W0, W1, 67);
        ARIAEngine.keyScheduleRound(rks[9], W1, W2, 67);
        ARIAEngine.keyScheduleRound(rks[10], W2, W3, 67);
        ARIAEngine.keyScheduleRound(rks[11], W3, W0, 67);
        ARIAEngine.keyScheduleRound(rks[12], W0, W1, 97);
        if (numRounds > 12) {
            ARIAEngine.keyScheduleRound(rks[13], W1, W2, 97);
            ARIAEngine.keyScheduleRound(rks[14], W2, W3, 97);
            if (numRounds > 14) {
                ARIAEngine.keyScheduleRound(rks[15], W3, W0, 97);
                ARIAEngine.keyScheduleRound(rks[16], W0, W1, 109);
            }
        }
        if (!forEncryption) {
            ARIAEngine.reverseKeys(rks);
