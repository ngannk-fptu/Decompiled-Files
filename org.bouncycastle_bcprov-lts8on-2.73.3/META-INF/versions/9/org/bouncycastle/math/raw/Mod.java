...
public abstract class Mod {
    ...
    public static int modOddInverse(int[] m, int[] x, int[] z) {
        ...
        for (int divSteps = 0; divSteps < maxDivsteps; divSteps += 30) {
            delta = Mod.divsteps30(delta, F[0], G[0], t);
            Mod.updateDE30(len30, D, E, t, m0Inv32, M);
            Mod.updateFG30(len30, F, G, t);
        }
        int signF = F[len30 - 1] >> 31;
        Mod.cnegate30(len30, signF, F);
    ...
    public static boolean modOddInverseVar(int[] m, int[] x, int[] z) {
        ...
        while (!Nat.isZero(lenFG, G)) {
            ...
            divsteps += 30;
            eta = Mod.divsteps30Var(eta, F[0], G[0], t);
            Mod.updateDE30(lenDE, D, E, t, m0Inv32, M);
            Mod.updateFG30(lenFG, F, G, t);
            int fn = F[lenFG - 1];
            int gn = G[lenFG - 1];
            int cond = lenFG - 2 >> 31;
            cond |= fn ^ fn >> 31;
    ...
    private static void updateFG30(int len30, int[] F, int[] G, int[] t) {
        ...
        for (int i = 1; i < len30; ++i) {
            ...
            gi = G[i];
            F[i - 1] = (int)(cf += (long)u * (long)fi + (long)v * (long)gi) & 0x3FFFFFFF;
            cf >>= 30;
