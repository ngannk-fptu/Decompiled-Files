...
public abstract class KeyFactory {
    ...
    public static KeyFactory create(Class keyInterface, Customizer customizer2) {
        return KeyFactory.create(keyInterface.getClassLoader(), keyInterface, customizer2);
    }
    ...
    public static KeyFactory create(ClassLoader loader, Class keyInterface, Customizer customizer2) {
        return KeyFactory.create(loader, keyInterface, customizer2, Collections.emptyList());
    }

    public static KeyFactory create(ClassLoader loader, Class keyInterface, KeyFactoryCustomizer customizer2, List<KeyFactoryCustomizer> next) {
        Generator gen = new Generator();
        gen.setInterface(keyInterface);
        gen.setContextClass(keyInterface);
        if (customizer2 != null) {
            gen.addCustomizer(customizer2);
        }
    ...
    extends AbstractClassGenerator {
        ...
        @Deprecated
        public void setCustomizer(Customizer customizer2) {
            this.customizers = CustomizerRegistry.singleton(customizer2);
        }

        public void addCustomizer(KeyFactoryCustomizer customizer2) {
        ...
        public void generateClass(ClassVisitor v) {
            ...
            for (int i3 = 0; i3 < parameterTypes.length; ++i3) {
                Type parameterType;
                Type fieldType = parameterType = parameterTypes[i3];
                for (FieldTypeCustomizer customizer2 : fieldTypeCustomizers) {
                    fieldType = customizer2.getOutType(i3, fieldType);
                }
                seed += fieldType.hashCode();
                ce.declare_field(18, this.getFieldName(i3), fieldType, null);
                e.dup();
                e.load_arg(i3);
                for (FieldTypeCustomizer customizer2 : fieldTypeCustomizers) {
                    customizer2.customize(e, i3, parameterType);
                }
                e.putfield(this.getFieldName(i3));
            }
            e.return_value();
            e.end_method();
            ...
            for (int i4 = 0; i4 < parameterTypes.length; ++i4) {
                e.load_this();
                e.getfield(this.getFieldName(i4));
                EmitUtils.hash_code(e, parameterTypes[i4], hm, this.customizers);
            }
            e.return_value();
            e.end_method();
            ...
            for (i2 = 0; i2 < parameterTypes.length; ++i2) {
                e.load_this();
                e.getfield(this.getFieldName(i2));
                e.load_arg(0);
                e.checkcast_this();
                e.getfield(this.getFieldName(i2));
                EmitUtils.not_equals(e, parameterTypes[i2], fail, this.customizers);
            }
            e.push(1);
            e.return_value();
            e.mark(fail);
            ...
            for (i2 = 0; i2 < parameterTypes.length; ++i2) {
                ...
                e.load_this();
                e.getfield(this.getFieldName(i2));
                EmitUtils.append_string(e, parameterTypes[i2], EmitUtils.DEFAULT_DELIMITERS, this.customizers);
            }
