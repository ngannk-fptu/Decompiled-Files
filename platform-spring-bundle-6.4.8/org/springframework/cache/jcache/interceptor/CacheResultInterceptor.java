...
extends AbstractKeyCacheInterceptor<CacheResultOperation, CacheResult> {
    ...
    protected void cacheException(@Nullable Cache exceptionCache, ExceptionTypeFilter filter2, Object cacheKey, Throwable ex) {
        if (exceptionCache == null) {
            return;
        }
        if (filter2.match(ex.getClass())) {
    ...
    private Cache resolveExceptionCache(CacheOperationInvocationContext<CacheResultOperation> context) {
        ...
        if (exceptionCacheResolver != null) {
            return CacheResultInterceptor.extractFrom(context.getOperation().getExceptionCacheResolver().resolveCaches(context));
        }
        return null;
    }
    ...
    private static <T extends Throwable> T cloneException(T exception) {
        try {
            return (T)((Throwable)SerializationUtils.deserialize(SerializationUtils.serialize(exception)));
        }
        catch (Exception ex) {
            return null;
        }
    ...
    private static int findCommonAncestorIndex(StackTraceElement[] callStack, String className, String methodName) {
        for (int i2 = 0; i2 < callStack.length; ++i2) {
            StackTraceElement element = callStack[i2];
