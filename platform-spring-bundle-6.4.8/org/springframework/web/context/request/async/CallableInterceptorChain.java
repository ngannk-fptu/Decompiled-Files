...
class CallableInterceptorChain {
    ...
    public Object applyPostProcess(NativeWebRequest request, Callable<?> task, Object concurrentResult) {
        ...
        for (int i2 = this.preProcessIndex; i2 >= 0; --i2) {
            try {
                this.interceptors.get(i2).postProcess(request, task, concurrentResult);
                continue;
            }
            catch (Throwable ex) {
    ...
    public void triggerAfterCompletion(NativeWebRequest request, Callable<?> task) {
        for (int i2 = this.interceptors.size() - 1; i2 >= 0; --i2) {
            try {
                this.interceptors.get(i2).afterCompletion(request, task);
                continue;
            }
            catch (Throwable ex) {
                if (!logger.isTraceEnabled()) continue;
