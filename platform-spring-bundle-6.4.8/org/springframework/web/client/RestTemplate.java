    ...
    implements ResponseExtractor<ResponseEntity<T>> {
        ...
        public ResponseEntity<T> extractData(ClientHttpResponse response) throws IOException {
            if (this.delegate != null) {
                T body2 = this.delegate.extractData(response);
                return ((ResponseEntity.BodyBuilder)ResponseEntity.status(response.getRawStatusCode()).headers(response.getHeaders())).body(body2);
            }
    ...
    extends AcceptHeaderRequestCallback {
        ...
        private void logBody(Object body2, @Nullable MediaType mediaType, HttpMessageConverter<?> converter) {
            if (RestTemplate.this.logger.isDebugEnabled()) {
                if (mediaType != null) {
                    RestTemplate.this.logger.debug((Object)("Writing [" + body2 + "] as \"" + mediaType + "\""));
                } else {
                    RestTemplate.this.logger.debug((Object)("Writing [" + body2 + "] with " + converter.getClass().getName()));
                }
            }
        }
    }
