...
implements ServerResponse.BodyBuilder {
    ...
    public ServerResponse body(Object body2) {
        return DefaultEntityResponseBuilder.fromObject(body2).status(this.statusCode).headers((HttpHeaders headers) -> headers.putAll(this.headers)).cookies((MultiValueMap<String, Cookie> cookies) -> cookies.addAll(this.cookies)).build();
    }

    @Override
    public <T> ServerResponse body(T body2, ParameterizedTypeReference<T> bodyType) {
        return DefaultEntityResponseBuilder.fromObject(body2, bodyType).status(this.statusCode).headers((HttpHeaders headers) -> headers.putAll(this.headers)).cookies((MultiValueMap<String, Cookie> cookies) -> cookies.addAll(this.cookies)).build();
    }

    @Override
    public ServerResponse render(String name, Object ... modelAttributes) {
        return new DefaultRenderingResponseBuilder(name).status(this.statusCode).headers((HttpHeaders headers) -> headers.putAll(this.headers)).cookies((MultiValueMap<String, Cookie> cookies) -> cookies.addAll(this.cookies)).modelAttributes(modelAttributes).build();
    }

    @Override
    public ServerResponse render(String name, Map<String, ?> model) {
        return new DefaultRenderingResponseBuilder(name).status(this.statusCode).headers((HttpHeaders headers) -> headers.putAll(this.headers)).cookies((MultiValueMap<String, Cookie> cookies) -> cookies.addAll(this.cookies)).modelAttributes(model).build();
    }
