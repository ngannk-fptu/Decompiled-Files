...
DisposableBean {
    ...
    static Request buildRequest(HttpHeaders headers, byte[] content, URI uri, HttpMethod method) throws MalformedURLException {
        ...
        RequestBody body2 = content.length > 0 || okhttp3.internal.http.HttpMethod.requiresRequestBody((String)method.name()) ? RequestBody.create((MediaType)contentType, (byte[])content) : null;
        Request.Builder builder = new Request.Builder().url(uri.toURL()).method(method.name(), body2);
