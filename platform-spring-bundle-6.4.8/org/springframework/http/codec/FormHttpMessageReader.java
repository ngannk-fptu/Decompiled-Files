...
implements HttpMessageReader<MultiValueMap<String, String>> {
    ...
    public Mono<MultiValueMap<String, String>> readMono(ResolvableType elementType, ReactiveHttpInputMessage message, Map<String, Object> hints) {
        ...
        return DataBufferUtils.join(message.getBody(), this.maxInMemorySize).map(buffer -> {
            ...
            String body2 = charBuffer.toString();
            DataBufferUtils.release(buffer);
            MultiValueMap<String, String> formData = this.parseFormData(charset, body2);
    ...
    private void logFormData(MultiValueMap<String, String> formData, Map<String, Object> hints) {
        LogFormatUtils.traceDebug(this.logger, traceOn -> Hints.getLogPrefix(hints) + "Read " + (this.isEnableLoggingRequestDetails() ? LogFormatUtils.formatValue(formData, traceOn == false) : "form fields " + formData.keySet() + " (content masked)"));
    }
    ...
    private MultiValueMap<String, String> parseFormData(Charset charset, String body2) {
