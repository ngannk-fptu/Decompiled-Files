...
class TypeConverterDelegate {
    ...
    public <T> T convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue, @Nullable Object newValue, @Nullable Class<T> requiredType, @Nullable TypeDescriptor typeDescriptor) throws IllegalArgumentException {
        ...
        if (editor == null && conversionService != null && newValue != null && typeDescriptor != null && conversionService.canConvert(sourceTypeDesc = TypeDescriptor.forObject(newValue), typeDescriptor)) {
            try {
    ...
    private Collection<?> convertToTypedCollection(Collection<?> original, @Nullable String propertyName, Class<?> requiredType, @Nullable TypeDescriptor typeDescriptor) {
        ...
        try {
            convertedCopy = approximable ? CollectionFactory.createApproximateCollection(original, original.size()) : (Collection)ReflectionUtils.accessibleConstructor(requiredType, new Class[0]).newInstance(new Object[0]);
        }
        catch (Throwable ex) {
            if (logger.isDebugEnabled()) {
    ...
    private Map<?, ?> convertToTypedMap(Map<?, ?> original, @Nullable String propertyName, Class<?> requiredType, @Nullable TypeDescriptor typeDescriptor) {
        ...
        try {
            convertedCopy = approximable ? CollectionFactory.createApproximateMap(original, original.size()) : (Map)ReflectionUtils.accessibleConstructor(requiredType, new Class[0]).newInstance(new Object[0]);
        }
        catch (Throwable ex) {
            if (logger.isDebugEnabled()) {
