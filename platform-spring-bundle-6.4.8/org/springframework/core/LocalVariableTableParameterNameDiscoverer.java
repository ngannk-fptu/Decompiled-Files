    ...
    private Map<Executable, String[]> inspectClass(Class<?> clazz) {
        ...
        if (is == null) {
            if (logger.isDebugEnabled()) {
                logger.debug((Object)("Cannot find '.class' file for class [" + clazz + "] - unable to determine constructor/method parameter names"));
            }
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug((Object)("Exception thrown while reading '.class' file for class [" + clazz + "] - unable to determine constructor/method parameter names"), (Throwable)ex);
            }
        }
        catch (IllegalArgumentException ex) {
            if (logger.isDebugEnabled()) {
                logger.debug((Object)("ASM ClassReader failed to parse class file [" + clazz + "], probably due to a new Java class file version that isn't supported yet - unable to determine constructor/method parameter names"), (Throwable)ex);
            }
        }
        finally {
            try {
    ...
    extends MethodVisitor {
        ...
        public void visitLocalVariable(String name, String description, String signature, Label start, Label end, int index) {
            ...
            for (int i2 = 0; i2 < this.lvtSlotIndex.length; ++i2) {
                if (this.lvtSlotIndex[i2] != index) continue;
        ...
        private Executable resolveExecutable() {
            ...
            for (int i2 = 0; i2 < this.args.length; ++i2) {
                argTypes[i2] = ClassUtils.resolveClassName(this.args[i2].getClassName(), loader);
            }
            try {
        ...
        private static int[] computeLvtSlotIndices(boolean isStatic, Type[] paramTypes) {
            ...
            for (int i2 = 0; i2 < paramTypes.length; ++i2) {
                lvtIndex[i2] = nextIndex++;
                if (!LocalVariableTableVisitor.isWideType(paramTypes[i2])) continue;
                nextIndex += 2;
            }
            return lvtIndex;
        }
