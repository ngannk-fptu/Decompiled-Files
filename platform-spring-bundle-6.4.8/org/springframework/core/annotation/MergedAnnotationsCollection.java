...
implements MergedAnnotations {
    ...
    private MergedAnnotationsCollection(Collection<MergedAnnotation<?>> annotations) {
        ...
        for (int i2 = 0; i2 < this.annotations.length; ++i2) {
            MergedAnnotation<?> annotation = this.annotations[i2];
            ...
            this.mappings[i2] = AnnotationTypeMappings.forAnnotationType(annotation.getType());
        }
    }
    ...
    private boolean isPresent(Object requiredType, boolean directOnly) {
        ...
        if (!directOnly) {
            for (AnnotationTypeMappings mappings : this.mappings) {
                for (int i2 = 1; i2 < mappings.size(); ++i2) {
                    AnnotationTypeMapping mapping = mappings.get(i2);
    ...
    private <A extends Annotation> MergedAnnotation<A> find(Object requiredType, @Nullable Predicate<? super MergedAnnotation<A>> predicate, @Nullable MergedAnnotationSelector<A> selector) {
        ...
        for (int i2 = 0; i2 < this.annotations.length; ++i2) {
            MergedAnnotation<?> root = this.annotations[i2];
            AnnotationTypeMappings mappings = this.mappings[i2];
    ...
    implements Spliterator<MergedAnnotation<A>> {
        ...
        public long estimateSize() {
            ...
            for (int i2 = 0; i2 < MergedAnnotationsCollection.this.annotations.length; ++i2) {
                AnnotationTypeMappings mappings = MergedAnnotationsCollection.this.mappings[i2];
                int numberOfMappings = mappings.size();
                size += (numberOfMappings -= Math.min(this.mappingCursors[i2], mappings.size()));
            }
            return size;
        }
