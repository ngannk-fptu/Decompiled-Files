...
abstract class AnnotationsScanner {
        ...
        try {
            ...
            while (source != null && source != Object.class && remaining > 0 && !AnnotationsScanner.hasPlainJavaAnnotationsOnly(source)) {
                ...
                for (int i2 = 0; i2 < declaredAnnotations.length; ++i2) {
                    if (declaredAnnotations[i2] == null) continue;
                    boolean isRelevant = false;
                    for (int relevantIndex = 0; relevantIndex < relevant.length; ++relevantIndex) {
                        if (relevant[relevantIndex] == null || declaredAnnotations[i2].annotationType() != relevant[relevantIndex].annotationType()) continue;
                        isRelevant = true;
    ...
    private static <C> Method[] getBaseTypeMethods(C context, Class<?> baseType) {
        ...
        if (methods == null) {
            ...
            for (int i2 = 0; i2 < methods.length; ++i2) {
                if ((isInterface || !Modifier.isPrivate(methods[i2].getModifiers())) && !AnnotationsScanner.hasPlainJavaAnnotationsOnly(methods[i2]) && AnnotationsScanner.getDeclaredAnnotations(methods[i2], false).length != 0) continue;
                methods[i2] = null;
                ++cleared;
            }
    ...
    private static boolean hasSameGenericTypeParameters(Method rootMethod, Method candidateMethod, Class<?>[] rootParameterTypes) {
        ...
        for (int i2 = 0; i2 < rootParameterTypes.length; ++i2) {
            Class<?> resolvedParameterType = ResolvableType.forMethodParameter(candidateMethod, i2, sourceDeclaringClass).resolve();
