...
ResourceLoaderAware {
    ...
    private boolean indexSupportsIncludeFilter(TypeFilter filter2) {
        if (filter2 instanceof AnnotationTypeFilter) {
            Class<? extends Annotation> annotation = ((AnnotationTypeFilter)filter2).getAnnotationType();
            return AnnotationUtils.isAnnotationDeclaredLocally(Indexed.class, annotation) || annotation.getName().startsWith("javax.");
        }
        if (filter2 instanceof AssignableTypeFilter) {
            Class<?> target = ((AssignableTypeFilter)filter2).getTargetType();
    ...
    @Nullable
    private String extractStereotype(TypeFilter filter2) {
        if (filter2 instanceof AnnotationTypeFilter) {
            return ((AnnotationTypeFilter)filter2).getAnnotationType().getName();
        }
        if (filter2 instanceof AssignableTypeFilter) {
            return ((AssignableTypeFilter)filter2).getTargetType().getName();
        }
        return null;
    }
    ...
    private Set<BeanDefinition> addCandidateComponentsFromIndex(CandidateComponentsIndex index, String basePackage) {
        ...
        try {
            ...
            for (TypeFilter filter2 : this.includeFilters) {
                String stereotype = this.extractStereotype(filter2);
                if (stereotype == null) {
                    throw new IllegalArgumentException("Failed to extract stereotype from " + filter2);
                }
    ...
    private Set<BeanDefinition> scanCandidateComponents(String basePackage) {
        ...
        try {
            ...
            Resource[] resources2 = this.getResourcePatternResolver().getResources(packageSearchPath);
            boolean traceEnabled = this.logger.isTraceEnabled();
            boolean debugEnabled = this.logger.isDebugEnabled();
