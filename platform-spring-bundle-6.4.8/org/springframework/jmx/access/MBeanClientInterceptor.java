...
DisposableBean {
    ...
    private Object convertDataArrayToTargetArray(Object[] array, Class<?> targetClass) throws NoSuchMethodException {
        ...
        for (int i2 = 0; i2 < array.length; ++i2) {
            Array.set(resultArray, i2, ReflectionUtils.invokeMethod(fromMethod, null, array[i2]));
        }
        return resultArray;
    }
    ...
    private Collection<?> convertDataArrayToTargetCollection(Object[] array, Class<?> collectionType, Class<?> elementType) throws NoSuchMethodException {
        ...
        for (int i2 = 0; i2 < array.length; ++i2) {
            resultColl.add(ReflectionUtils.invokeMethod(fromMethod, null, array[i2]));
        }
        return resultColl;
    }
    ...
    implements Comparable<MethodCacheKey> {
        ...
        public int compareTo(MethodCacheKey other) {
            ...
            for (int i2 = 0; i2 < this.parameterTypes.length; ++i2) {
                result = this.parameterTypes[i2].getName().compareTo(other.parameterTypes[i2].getName());
