 ...
 *  com.atlassian.confluence.search.v2.searchfilter.BooleanSearchFilter
 *  com.atlassian.confluence.search.v2.searchfilter.TermSearchFilter
...
@Deprecated
public class SearchFilterBasedEntityPrefetchProvider
implements EntityPrefetchProvider {
    private final SearchManager searchManager;
    private final TransactionTemplate transactionTemplate;

    public SearchFilterBasedEntityPrefetchProvider(@Nonnull SearchManager searchManager, @Nonnull TransactionTemplate transactionTemplate) {
    ...
    public Stream<LogRecord> prefetchAndConvert(int maxPageSize, @Nonnull SearchQuery searchQuery, @Nonnull Function<Searchable, LogRecord> logConverterFn) {
        ...
        Iterable entityIterable = () -> new EntityPageIterator(pageSize, (offset, limit) -> this.fetchPages(idsHolder, (int)offset, (int)limit, logConverterFn));
    ...
    public DocIdsHolder prefetchDocIds(@Nonnull SearchQuery searchQuery) {
        ...
        try {
            this.searchManager.scan(EnumSet.of(SearchIndex.CONTENT), searchQuery, (Set)Sets.newHashSet((Iterable)ImmutableList.of((Object)"handle")), resultDocId -> docIdsHolder.addDocId(this.getDocIdFromScanResult((Map<String, String[]>)resultDocId)));
        }
    ...
    public ContentSearch contentSearch(@Nonnull SearchQuery searchQuery, @Nullable SearchSort searchSort, int offset, int limit) {
        return new ContentSearch(searchQuery, searchSort, null, offset, limit);
    }

    @Nonnull
    @VisibleForTesting
    List<LogRecord> fetchPages(DocIdsHolder docIdsHolder, int offset, int limit, Function<Searchable, LogRecord> logConverterFn) {
        if (offset > docIdsHolder.size()) {
            return Collections.emptyList();
        }
        return (List)this.transactionTemplate.execute(() -> {
            BooleanSearchFilter filter = new BooleanSearchFilter(Collections.emptyList(), this.constructIdFilters(docIdsHolder, offset, limit), Collections.emptyList());
            try {
                List searchableList = this.searchManager.searchEntities((ISearch)new ContentSearch((SearchQuery)AllQuery.getInstance(), null, (SearchFilter)filter, 0, limit), SearchManager.EntityVersionPolicy.INDEXED_VERSION);
    ...
    @Nonnull
    private List<TermSearchFilter> constructIdFilters(@Nonnull DocIdsHolder docIdsHolder, int offset, int limit) {
        return docIdsHolder.getIds(offset, limit).stream().map(id -> new TermSearchFilter("handle", id)).collect(Collectors.toList());
    }
