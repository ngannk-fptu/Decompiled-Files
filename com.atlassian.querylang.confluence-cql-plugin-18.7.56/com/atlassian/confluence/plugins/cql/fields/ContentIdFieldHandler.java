...
public class ContentIdFieldHandler
extends BaseFieldHandler
implements EqualityFieldHandler<String, V2SearchQueryWrapper> {
    private static final String FIELD_NAME = "id";

    public ContentIdFieldHandler() {
        this(FIELD_NAME);
    }

    public ContentIdFieldHandler(String fieldName) {
        super(fieldName, true);
    }

    public FieldOrder buildOrder(OrderDirection direction) {
        return new V2SearchSortWrapper((SearchSort)new ContentIdSort(direction == OrderDirection.ASC ? SearchSort.Order.ASCENDING : SearchSort.Order.DESCENDING));
    }
    ...
    public V2SearchQueryWrapper build(SetExpressionData expressionData, Iterable<String> values) {
        ...
        ContentIdQuery query = new ContentIdQuery(Collections.unmodifiableList(StreamSupport.stream(values.spliterator(), false).map(V2FieldHandlerHelper::stringToContentId).collect(Collectors.toList())));
    ...
    public V2SearchQueryWrapper build(EqualityExpressionData expressionData, String value) {
        ...
        return V2FieldHandlerHelper.wrapV2Search((SearchQuery)new ContentIdQuery(V2FieldHandlerHelper.stringToContentId((String)value)), (ExpressionData)expressionData);
    }
}
