...
implements EqualityFieldHandler<String, V2SearchQueryWrapper> {
    ...
    public V2SearchQueryWrapper build(SetExpressionData expressionData, Iterable<String> values) {
        ...
        AncestorIdQuery query = new AncestorIdQuery(Collections.unmodifiableList(StreamSupport.stream(values.spliterator(), false).map(V2FieldHandlerHelper::stringToContentId).collect(Collectors.toList())));
    ...
    public V2SearchQueryWrapper build(EqualityExpressionData expressionData, String value) {
        ...
        return V2FieldHandlerHelper.wrapV2Search((SearchQuery)new AncestorIdQuery(V2FieldHandlerHelper.stringToContentId((String)value)), (ExpressionData)expressionData);
    }
}
