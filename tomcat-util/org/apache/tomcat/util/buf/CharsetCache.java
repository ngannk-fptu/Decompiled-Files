...
public class CharsetCache {
    ...
    public CharsetCache() {
        for (String charsetName : INITIAL_CHARSETS) {
            ...
            this.addToCache(charsetName, charset);
        }
        for (String charsetName : LAZY_CHARSETS) {
            this.addToCache(charsetName, DUMMY_CHARSET);
        }
    }

    private void addToCache(String name, Charset charset) {
        this.cache.put(name, charset);
        for (String alias : charset.aliases()) {
            this.cache.put(alias.toLowerCase(Locale.ENGLISH), charset);
        }
    }

    public Charset getCharset(String charsetName) {
        String lcCharsetName = charsetName.toLowerCase(Locale.ENGLISH);
        Charset result = (Charset)this.cache.get(lcCharsetName);
        if (result == DUMMY_CHARSET) {
            Charset charset = Charset.forName(lcCharsetName);
            if (charset == null) {
                this.cache.remove(lcCharsetName);
                result = null;
            } else {
                this.addToCache(lcCharsetName, charset);
                result = charset;
            }
        }
        return result;
    ...
    private static class DummyCharset
    extends Charset {
