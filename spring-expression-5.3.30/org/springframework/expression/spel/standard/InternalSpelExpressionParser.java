...
extends TemplateAwareExpressionParser {
    ...
    protected SpelExpression doParseExpression(String expressionString, @Nullable ParserContext context) throws ParseException {
        ...
        try {
            ...
            if (t != null) {
                throw new SpelParseException(this.expressionString, t.startPos, SpelMessage.MORE_INPUT, this.toString(this.nextToken()));
            }
    ...
    private SpelNodeImpl eatProductExpression() {
        ...
        while (this.peekToken(TokenKind.STAR, TokenKind.DIV, TokenKind.MOD)) {
            ...
            if (t.kind != TokenKind.MOD) continue;
    ...
    private SpelNodeImpl eatUnaryExpression() {
        SpelNodeImpl expr;
        Token t;
        if (this.peekToken(TokenKind.NOT, TokenKind.PLUS, TokenKind.MINUS)) {
            t = this.takeToken();
    ...
    private void eatConstructorArgs(List<SpelNodeImpl> accumulatedArguments) {
        if (!this.peekToken(TokenKind.LPAREN)) {
            throw this.internalException(this.positionOf(this.peekToken()), SpelMessage.MISSING_CONSTRUCTOR_ARGS, new Object[0]);
        }
    ...
    private boolean maybeEatProjection(boolean nullSafeNavigation) {
        Token t = this.peekToken();
        if (t == null || !this.peekToken(TokenKind.PROJECT, true)) {
            return false;
        }
    ...
    private boolean maybeEatInlineListOrMap() {
        Token t = this.peekToken();
        if (t == null || !this.peekToken(TokenKind.LCURLY, true)) {
            return false;
        }
        SpelNodeImpl expr = null;
        Token closingCurly = this.peekToken();
        if (closingCurly != null && this.peekToken(TokenKind.RCURLY, true)) {
    ...
    private boolean maybeEatIndexer() {
        Token t = this.peekToken();
        if (t == null || !this.peekToken(TokenKind.LSQUARE, true)) {
            return false;
        }
        SpelNodeImpl expr = this.eatExpression();
        if (expr == null) {
    ...
    private boolean maybeEatSelection(boolean nullSafeNavigation) {
        Token t = this.peekToken();
        if (t == null || !this.peekSelectToken()) {
            return false;
        }
        this.nextToken();
        SpelNodeImpl expr = this.eatExpression();
    ...
    private boolean isValidQualifiedId(@Nullable Token node) {
        ...
        return StringUtils.hasLength((String)value) && VALID_QUALIFIED_ID_PATTERN.matcher(value).matches();
    }
