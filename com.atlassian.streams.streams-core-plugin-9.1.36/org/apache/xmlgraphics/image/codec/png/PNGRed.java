...
extends AbstractRed {
    ...
    public PNGRed(InputStream stream, PNGDecodeParam decodeParam) throws IOException {
        ...
        if ((magic = distream.readLong()) != -8552249625308161526L) {
        ...
        while (true) {
            PNGChunk chunk;
            String chunkType;
            if ((chunkType = PNGRed.getChunkType(distream)).equals("IHDR")) {
                chunk = PNGRed.readChunk(distream);
    ...
    private static String getChunkType(DataInputStream distream) {
        try {
            ...
            distream.reset();
            String typeString = "" + (char)(type >> 24 & 0xFF) + (char)(type >> 16 & 0xFF) + (char)(type >> 8 & 0xFF) + (char)(type & 0xFF);
            return typeString;
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    private void parse_IEND_chunk(PNGChunk chunk) throws Exception {
        ...
        ((InputStream)infStream).close();
        ((InputStream)seqStream).close();
        this.streamVec = null;
        SampleModel sm = this.theTile.getSampleModel();
        if (this.colorType == 3 && !this.expandPalette) {
            cm = this.outputHasAlphaPalette ? new IndexColorModel(this.bitDepth, this.paletteEntries, this.redPalette, this.greenPalette, this.bluePalette, this.alphaPalette) : new IndexColorModel(this.bitDepth, this.paletteEntries, this.redPalette, this.greenPalette, this.bluePalette);
            ...
            cm = PNGRed.createComponentColorModel(sm);
        }
        this.init((CachableRed)null, this.bounds, cm, sm, 0, 0, this.properties);
    }
    ...
    private void parse_pHYs_chunk(PNGChunk chunk) {
        ...
        if (this.emitProperties) {
            ...
            this.properties.put("pixel_aspect_ratio", Float.valueOf((float)xPixelsPerUnit / (float)yPixelsPerUnit));
            if (unitSpecifier == 1) {
    ...
    private static void decodePaethFilter(byte[] curr, byte[] prev, int count, int bpp) {
        ...
        for (i = bpp; i < count; ++i) {
            ...
            curr[i] = (byte)(raw + PNGRed.paethPredictor(priorPixel, priorRow, priorRowPixel));
        }
    }
    ...
    private void decodePass(WritableRaster imRas, int xOffset, int yOffset, int xStep, int yStep, int passWidth, int passHeight) {
        ...
        while (srcY < passHeight) {
            ...
            switch (filter) {
                ...
                case 1: {
                    PNGRed.decodeSubFilter(curr, bytesPerRow, this.bytesPerPixel);
                    break;
                }
                case 2: {
                    PNGRed.decodeUpFilter(curr, prior, bytesPerRow);
                    break;
                }
                case 3: {
                    PNGRed.decodeAverageFilter(curr, prior, bytesPerRow, this.bytesPerPixel);
                    break;
                }
                case 4: {
                    PNGRed.decodePaethFilter(curr, prior, bytesPerRow, this.bytesPerPixel);
                    break;
                }
                default: {
