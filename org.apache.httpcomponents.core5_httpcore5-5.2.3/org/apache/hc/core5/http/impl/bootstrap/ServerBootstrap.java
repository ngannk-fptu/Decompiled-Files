...
public class ServerBootstrap {
    ...
    public HttpServer create() {
        ...
        RequestHandlerRegistry<HttpRequestHandler> handlerRegistry = new RequestHandlerRegistry<HttpRequestHandler>(this.canonicalHostName != null ? this.canonicalHostName : InetAddressUtils.getCanonicalLocalHostName(), () -> this.lookupRegistry != null ? this.lookupRegistry : UriPatternType.newMatcher(UriPatternType.URI_PATTERN));
        ...
        if (!this.filters.isEmpty()) {
            ...
            for (FilterEntry<HttpFilterHandler> entry : this.filters) {
                switch (entry.position) {
                    ...
                    case LAST: {
                        filterChainDefinition.addBefore(StandardFilter.MAIN_HANDLER.name(), (HttpFilterHandler)entry.filterHandler, entry.name);
                    }
                }
            }
