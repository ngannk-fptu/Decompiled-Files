...
class SQLDiagnostic {
    ...
    void addWarning(SQLWarning w) {
        if (this.warnings == null) {
            ...
            this.lastWarning.setNextWarning(w);
        }
        this.lastWarning = w;
    }
    ...
    void addDiagnostic(int number, int state, int serverity, String message, String server, String procName, int line) {
        if (serverity > 10) {
            SQLException e = new SQLException(message, SQLDiagnostic.getStateCode(number, this.serverType, "S1000"), number);
            if (this.serverType == 1 && (number == 8152 || number == 8115 || number == 220) || this.serverType == 2 && (number == 247 || number == 9502)) {
                SQLException tmp = e;
            ...
            SQLWarning w = new SQLWarning(message, SQLDiagnostic.getStateCode(number, this.serverType, "01000"), number);
            this.addWarning(w);
        }
    }
    ...
    private static String getStateCode(int number, int serverType, String defState) {
        HashMap stateTable = serverType == 2 ? sybStates : mssqlStates;
        String state = (String)stateTable.get(new Integer(number));
        if (state != null) {
            return state;
        }
        return defState;
    ...
    static {
        ...
        mssqlStates.put(new Integer(257), "22005");
        ...
        mssqlStates.put(new Integer(1010), "22019");
