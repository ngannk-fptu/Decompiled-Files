...
final class ConstructorInjectorStore {
    ...
    private <T> ConstructorInjector<T> createConstructor(InjectionPoint injectionPoint, Errors errors) throws ErrorsException {
        ...
        ImmutableList injectorAspects = this.injector.state.getMethodAspects();
        ImmutableList methodAspects = membersInjector.getAddedAspects().isEmpty() ? injectorAspects : ImmutableList.copyOf((Iterable)Iterables.concat(injectorAspects, membersInjector.getAddedAspects()));
        ProxyFactory factory = new ProxyFactory(injectionPoint, (Iterable<MethodAspect>)methodAspects);
