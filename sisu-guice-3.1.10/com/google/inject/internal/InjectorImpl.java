...
Lookups {
    ...
    InjectorImpl(InjectorImpl parent, State state, InjectorOptions injectorOptions) {
        ...
        this.localContext = parent != null ? parent.localContext : new ThreadLocal();
    }
    ...
    public Injector createChildInjector(Module ... modules) {
        return this.createChildInjector((Iterable<? extends Module>)ImmutableList.copyOf((Object[])modules));
    }
    ...
    <T> void initializeBinding(BindingImpl<T> binding, Errors errors) throws ErrorsException {
        if (binding instanceof DelayedInitialize) {
            ((DelayedInitialize)((Object)binding)).initialize(this, errors);
        }
    }
    ...
    <T> void initializeJitBinding(BindingImpl<T> binding, Errors errors) throws ErrorsException {
        if (binding instanceof DelayedInitialize) {
            ...
            DelayedInitialize delayed = (DelayedInitialize)((Object)binding);
            try {
                ...
                if (!successful) {
                    this.removeFailedJitBinding(binding, null);
    ...
    <T> BindingImpl<T> createProvidedByBinding(Key<T> key, Scoping scoping, ProvidedBy providedBy, Errors errors) throws ErrorsException {
        ...
        ProvidedByInternalFactory<T> internalFactory = new ProvidedByInternalFactory<T>(rawType, providerType, providerKey, !this.options.disableCircularProxies);
        Class<T> source = rawType;
        LinkedProviderBindingImpl<T> binding = LinkedProviderBindingImpl.createWithInitializer(this, key, source, Scoping.scope(key, this, internalFactory, source, scoping), scoping, providerKey, internalFactory);
    ...
    private <T> BindingImpl<T> createImplementedByBinding(Key<T> key, Scoping scoping, ImplementedBy implementedBy, Errors errors) throws ErrorsException {
        ...
        InternalFactory internalFactory = new InternalFactory<T>(){
            ...
            public T get(Errors errors, InternalContext context, Dependency<?> dependency, boolean linked) throws ErrorsException {
                ...
                try {
                    Object t = targetBinding.getInternalFactory().get(errors.withSource(targetKey), context, dependency, true);
                    return t;
                }
                finally {
                    context.popState();
    ...
    <T> SingleParameterInjector<T> createParameterInjector(Dependency<T> dependency, Errors errors) throws ErrorsException {
        BindingImpl<T> binding = this.getBindingOrThrow(dependency.getKey(), errors, JitLimitation.NO_JIT);
    ...
    <T> Provider<T> getProviderOrThrow(Key<T> key, Errors errors) throws ErrorsException {
        final BindingImpl<T> binding = this.getBindingOrThrow(key, errors, JitLimitation.NO_JIT);
                ...
                try {
                    Object t = InjectorImpl.this.callInContext(new ContextualCallable<T>(){
                        ...
                        public T call(InternalContext context) throws ErrorsException {
                            ...
                            try {
                                Object t = binding.getInternalFactory().get(errors, context, dependency, false);
                                return t;
                            }
                            finally {
    ...
    private static class BindingsMultimap {
        ...
        <T> void put(TypeLiteral<T> type, Binding<T> binding) {
            ArrayList bindingsForType = this.multimap.get(type);
    ...
    static class InjectorOptions {
        ...
        public String toString() {
            return Objects.toStringHelper(this.getClass()).add("stage", (Object)this.stage).add("jitDisabled", this.jitDisabled).add("disableCircularProxies", this.disableCircularProxies).add("atInjectRequired", this.atInjectRequired).add("exactBindingAnnotationsRequired", this.exactBindingAnnotationsRequired).toString();
        }
    }
}
