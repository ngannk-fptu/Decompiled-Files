...
public final class BytecodeGen {
    ...
    private static ClassLoader getClassLoader(Class<?> type, ClassLoader delegate) {
        ...
        if (Visibility.forType(type) == Visibility.PUBLIC) {
            if (delegate != SystemBridgeHolder.SYSTEM_BRIDGE.getParent()) {
                return (ClassLoader)CLASS_LOADER_CACHE.getUnchecked((Object)delegate);
            }
    ...
    static {
        ...
        GUICE_CLASS_LOADER = BytecodeGen.canonicalize(BytecodeGen.class.getClassLoader());
        ...
        CacheBuilder builder = CacheBuilder.newBuilder().weakKeys().weakValues();
        if (!CUSTOM_LOADER_ENABLED) {
            builder.maximumSize(0L);
        }
        CLASS_LOADER_CACHE = builder.build((CacheLoader)new CacheLoader<ClassLoader, ClassLoader>(){
