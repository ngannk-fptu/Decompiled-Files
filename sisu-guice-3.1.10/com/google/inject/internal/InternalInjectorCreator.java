...
public final class InternalInjectorCreator {
    ...
    void loadEagerSingletons(InjectorImpl injector, Stage stage, final Errors errors) {
        ImmutableList candidateBindings = ImmutableList.copyOf((Iterable)Iterables.concat(injector.state.getExplicitBindingsThisLevel().values(), injector.jitBindings.values()));
        for (final BindingImpl binding : candidateBindings) {
            if (!this.isEagerSingleton(injector, binding, stage)) continue;
            try {
