...
public class Key<T> {
    ...
    static AnnotationStrategy strategyFor(Class<? extends Annotation> annotationType) {
        if (Annotations.isAllDefaultMethods(annotationType = Annotations.canonicalizeIfNamed(annotationType))) {
            return Key.strategyFor(Annotations.generateAnnotation(annotationType));
        }
    ...
    private static void ensureRetainedAtRuntime(Class<? extends Annotation> annotationType) {
        Preconditions.checkArgument((boolean)Annotations.isRetainedAtRuntime(annotationType), (String)"%s is not retained at runtime. Please annotate it with @Retention(RUNTIME).", (Object[])new Object[]{annotationType.getName()});
    }

    private static void ensureIsBindingAnnotation(Class<? extends Annotation> annotationType) {
        Preconditions.checkArgument((boolean)Annotations.isBindingAnnotation(annotationType), (String)"%s is not a binding annotation. Please annotate it with @BindingAnnotation.", (Object[])new Object[]{annotationType.getName()});
    }
