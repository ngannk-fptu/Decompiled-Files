...
extends AbstractBeanDefinitionReader {
    ...
    public int loadBeanDefinitions(EncodedResource encodedResource, @Nullable String prefix) throws BeanDefinitionStoreException {
        if (this.logger.isTraceEnabled()) {
            this.logger.trace((Object)("Loading properties bean definitions from " + encodedResource));
        }
    ...
    public int registerBeanDefinitions(Map<?, ?> map, @Nullable String prefix, String resourceDescription) throws BeansException {
        ...
        for (Object key : map.keySet()) {
            ...
            if (sepIdx != -1) {
                ...
                if (this.logger.isTraceEnabled()) {
                    this.logger.trace((Object)("Found bean name '" + beanName + "'"));
                }
    ...
    protected void registerBeanDefinition(String beanName, Map<?, ?> map, String prefix, String resourceDescription) throws BeansException {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            String key = StringUtils.trimWhitespace((String)((String)entry.getKey()));
            if (!key.startsWith(prefixWithSep)) continue;
            String property = key.substring(beginIndex);
            if (CLASS_KEY.equals(property)) {
                className = StringUtils.trimWhitespace((String)((String)entry.getValue()));
                continue;
            }
            if (PARENT_KEY.equals(property)) {
                parent = StringUtils.trimWhitespace((String)((String)entry.getValue()));
                continue;
            }
            if (ABSTRACT_KEY.equals(property)) {
                String val = StringUtils.trimWhitespace((String)((String)entry.getValue()));
                isAbstract = TRUE_VALUE.equals(val);
                continue;
            }
            if (SCOPE_KEY.equals(property)) {
                scope = StringUtils.trimWhitespace((String)((String)entry.getValue()));
                continue;
            }
            if (SINGLETON_KEY.equals(property)) {
                String val = StringUtils.trimWhitespace((String)((String)entry.getValue()));
                scope = !StringUtils.hasLength((String)val) || TRUE_VALUE.equals(val) ? "singleton" : "prototype";
                continue;
            }
            ...
            if (property.endsWith(REF_SUFFIX)) {
                ...
                String ref = StringUtils.trimWhitespace((String)((String)entry.getValue()));
        ...
        if (this.logger.isTraceEnabled()) {
            this.logger.trace((Object)("Registering bean definition for bean name '" + beanName + "' with " + pvs));
        }
