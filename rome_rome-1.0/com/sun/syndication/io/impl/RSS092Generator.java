...
extends RSS091UserlandGenerator {
    ...
    protected void populateItem(Item item, Element eItem, int index) {
        ...
        for (int i = 0; i < this.getNumberOfEnclosures(enclosures); ++i) {
            eItem.addContent((Content)this.generateEnclosure((Enclosure)enclosures.get(i)));
        }
        List categories = item.getCategories();
        for (int i = 0; i < categories.size(); ++i) {
            eItem.addContent((Content)this.generateCategoryElement((Category)categories.get(i)));
        }
    }
