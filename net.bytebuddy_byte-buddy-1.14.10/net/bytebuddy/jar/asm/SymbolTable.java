...
final class SymbolTable {
    ...
    int addForwardUninitializedType(String value, Label label) {
        int labelIndex = this.getOrAddLabelEntry((Label)label).index;
        int hashCode = SymbolTable.hash(130, value, labelIndex);
        Entry entry = this.get(hashCode);
        while (entry != null) {
            if (entry.tag == 130 && entry.hashCode == hashCode && entry.data == (long)labelIndex && entry.value.equals(value)) {
                return entry.index;
            }
            entry = entry.next;
        }
        return this.addTypeInternal(new Entry(this.typeCount, 130, value, labelIndex, hashCode));
    }
    ...
    int addMergedType(int typeTableIndex1, int typeTableIndex2) {
        ...
        int hashCode = SymbolTable.hash(131, typeTableIndex1 + typeTableIndex2);
        Entry entry = this.get(hashCode);
        while (entry != null) {
            if (entry.tag == 131 && entry.hashCode == hashCode && entry.data == data) {
                return entry.info;
            }
            entry = entry.next;
        }
        ...
        this.put((Entry)new Entry((int)this.typeCount, (int)131, (long)data, (int)hashCode)).info = commonSuperTypeIndex = this.addType(this.classWriter.getCommonSuperClass(type1, type2));
    ...
    private LabelEntry getOrAddLabelEntry(Label label) {
        ...
        LabelEntry labelEntry = this.labelEntries[hashCode % this.labelEntries.length];
        ...
        if (this.labelCount > this.labelEntries.length * 3 / 4) {
            ...
            for (int i = currentCapacity - 1; i >= 0; --i) {
                LabelEntry currentEntry = this.labelEntries[i];
                while (currentEntry != null) {
        ...
        labelEntry.next = this.labelEntries[index];
        this.labelEntries[index] = labelEntry;
