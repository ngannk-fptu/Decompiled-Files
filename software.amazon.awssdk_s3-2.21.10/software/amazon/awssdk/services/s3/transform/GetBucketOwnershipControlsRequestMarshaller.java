...
implements Marshaller<GetBucketOwnershipControlsRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("?ownershipControls").httpMethod(SdkHttpMethod.GET).hasExplicitPayloadMember(false).hasPayloadMembers(false).putAdditionalMetadata(AwsXmlProtocolFactory.ROOT_MARSHALL_LOCATION_ATTRIBUTE, null).putAdditionalMetadata(AwsXmlProtocolFactory.XML_NAMESPACE_ATTRIBUTE, null).build();
    ...
    public SdkHttpFullRequest marshall(GetBucketOwnershipControlsRequest getBucketOwnershipControlsRequest) {
        Validate.paramNotNull((Object)((Object)getBucketOwnershipControlsRequest), (String)"getBucketOwnershipControlsRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)getBucketOwnershipControlsRequest);
        }
        catch (Exception e) {
