...
@SdkInternalApi
public class PutBucketOwnershipControlsRequestMarshaller
implements Marshaller<PutBucketOwnershipControlsRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("?ownershipControls").httpMethod(SdkHttpMethod.PUT).hasExplicitPayloadMember(true).hasPayloadMembers(true).putAdditionalMetadata(AwsXmlProtocolFactory.ROOT_MARSHALL_LOCATION_ATTRIBUTE, null).putAdditionalMetadata(AwsXmlProtocolFactory.XML_NAMESPACE_ATTRIBUTE, (Object)"http://s3.amazonaws.com/doc/2006-03-01/").build();
    private final AwsXmlProtocolFactory protocolFactory;

    public PutBucketOwnershipControlsRequestMarshaller(AwsXmlProtocolFactory protocolFactory) {
        this.protocolFactory = protocolFactory;
    }

    public SdkHttpFullRequest marshall(PutBucketOwnershipControlsRequest putBucketOwnershipControlsRequest) {
        Validate.paramNotNull((Object)((Object)putBucketOwnershipControlsRequest), (String)"putBucketOwnershipControlsRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)putBucketOwnershipControlsRequest);
        }
        catch (Exception e) {
