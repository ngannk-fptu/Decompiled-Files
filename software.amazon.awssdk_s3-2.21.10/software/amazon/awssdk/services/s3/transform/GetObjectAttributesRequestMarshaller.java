...
implements Marshaller<GetObjectAttributesRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/{Key+}?attributes").httpMethod(SdkHttpMethod.GET).hasExplicitPayloadMember(false).hasPayloadMembers(false).putAdditionalMetadata(AwsXmlProtocolFactory.ROOT_MARSHALL_LOCATION_ATTRIBUTE, null).putAdditionalMetadata(AwsXmlProtocolFactory.XML_NAMESPACE_ATTRIBUTE, null).build();
    ...
    public SdkHttpFullRequest marshall(GetObjectAttributesRequest getObjectAttributesRequest) {
        ...
        try {
            ...
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)getObjectAttributesRequest);
        }
        catch (Exception e) {
