...
implements Marshaller<HeadObjectRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/{Key+}").httpMethod(SdkHttpMethod.HEAD).hasExplicitPayloadMember(false).hasPayloadMembers(false).putAdditionalMetadata(AwsXmlProtocolFactory.ROOT_MARSHALL_LOCATION_ATTRIBUTE, null).putAdditionalMetadata(AwsXmlProtocolFactory.XML_NAMESPACE_ATTRIBUTE, null).build();
    ...
    public SdkHttpFullRequest marshall(HeadObjectRequest headObjectRequest) {
        Validate.paramNotNull((Object)((Object)headObjectRequest), (String)"headObjectRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)headObjectRequest);
        }
        catch (Exception e) {
