...
implements Marshaller<SelectObjectContentRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/{Key+}?select&select-type=2").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasPayloadMembers(true).putAdditionalMetadata(AwsXmlProtocolFactory.ROOT_MARSHALL_LOCATION_ATTRIBUTE, (Object)"SelectObjectContentRequest").putAdditionalMetadata(AwsXmlProtocolFactory.XML_NAMESPACE_ATTRIBUTE, (Object)"http://s3.amazonaws.com/doc/2006-03-01/").build();
    ...
    public SdkHttpFullRequest marshall(SelectObjectContentRequest selectObjectContentRequest) {
        Validate.paramNotNull((Object)((Object)selectObjectContentRequest), (String)"selectObjectContentRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)selectObjectContentRequest);
        }
        catch (Exception e) {
