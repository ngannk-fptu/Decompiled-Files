    ...
    private static void validateValue(BigInteger v, SchemaType sType, ValidationContext context) {
        ...
        if (td != null) {
            ...
            if (len > td.getBigIntegerValue().intValue()) {
                context.invalid("cvc-totalDigits-valid", new Object[]{len, temp, td.getBigIntegerValue().intValue(), QNameHelper.readable(sType)});
                return;
            }
        }
        ...
        if (vals != null) {
            for (XmlAnySimpleType val : vals) {
                if (!v.equals(JavaIntegerHolderEx.getBigIntegerValue(val))) continue;
                return;
            }
