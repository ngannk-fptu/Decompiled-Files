...
extends AbstractMojo {
    ...
    private Document build() throws MojoExecutionException {
        ...
        try (InputStream is = ((Object)((Object)this)).getClass().getResourceAsStream(PLUGIN_HELP_PATH);){
            Document document = DocumentHelper.readDocument(new XmlOptions(), is);
            return document;
        }
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (MavenPluginHelp.isNotEmpty(name) && !name.contains(id)) {
            this.append(sb, name + " " + version, 0);
        } else if (MavenPluginHelp.isNotEmpty(name)) {
    ...
    private void writeGoal(StringBuilder sb, String goalPrefix, Element mojo) throws MojoExecutionException {
        ...
        if (this.goal == null || this.goal.length() <= 0 || mojoGoal.equals(this.goal)) {
            ...
            if (deprecated != null && MavenPluginHelp.isNotEmpty(deprecated.getTextContent())) {
    ...
    private void writeParameter(StringBuilder sb, Node parameter, Node configurationElement) throws MojoExecutionException {
        ...
        if (fieldConfigurationElement != null && MavenPluginHelp.isNotEmpty(fieldConfigurationElement.getTextContent())) {
    ...
    private void append(StringBuilder sb, String description, int indent) {
        for (String line : MavenPluginHelp.toLines(description, indent, this.indentSize, this.lineLength)) {
    ...
    private static List<String> toLines(String text, int indent, int indentSize, int lineLength) {
        ...
        String ind = MavenPluginHelp.repeat("\t", indent);
        for (String plainLine : plainLines = text.split("(\r\n)|(\r)|(\n)")) {
            MavenPluginHelp.toLines(lines, ind + plainLine, indentSize, lineLength);
        }
        return lines;
    }
    ...
    private static void toLines(List<String> lines, String line, int indentSize, int lineLength) {
        String[] tokens;
        int lineIndent = MavenPluginHelp.getIndentLevel(line);
        ...
        for (String token : tokens = line.split(" +")) {
            if (buf.length() > 0) {
                if (buf.length() + token.length() >= lineLength) {
                    ...
                    buf.setLength(0);
                    buf.append(MavenPluginHelp.repeat(" ", lineIndent * indentSize));
                } else {
                    buf.append(' ');
                }
            }
            for (int j = 0; j < token.length(); ++j) {
                ...
                if (c == '\t') {
                    buf.append(MavenPluginHelp.repeat(" ", indentSize - buf.length() % indentSize));
                    continue;
                }
                if (c == '\u00a0') {
                    buf.append(' ');
