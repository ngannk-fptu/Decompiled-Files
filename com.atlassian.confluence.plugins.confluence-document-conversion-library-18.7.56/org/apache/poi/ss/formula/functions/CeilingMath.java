...
public final class CeilingMath
implements FreeRefFunction {
    ...
    public ValueEval evaluate(ValueEval[] args, OperationEvaluationContext ec) {
        ...
        try {
            RoundingMode mode;
            Double xval = CeilingMath.evaluateValue(args[0], ec.getRowIndex(), ec.getColumnIndex());
            if (xval == null) {
                return ErrorEval.VALUE_INVALID;
            }
            double multiplier = 1.0;
            if (args.length > 1) {
                Double arg1Val = CeilingMath.evaluateValue(args[1], ec.getRowIndex(), ec.getColumnIndex());
                multiplier = arg1Val != null ? arg1Val : 1.0;
            }
            boolean roundNegativeNumsDown = false;
            if (args.length > 2) {
                Double arg2Val = CeilingMath.evaluateValue(args[2], ec.getRowIndex(), ec.getColumnIndex());
