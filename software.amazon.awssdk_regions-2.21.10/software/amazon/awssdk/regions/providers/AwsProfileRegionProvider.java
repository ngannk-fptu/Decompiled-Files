...
implements AwsRegionProvider {
    ...
    public Region getRegion() {
        return this.profileFile.get().profile(this.profileName).map(p -> (String)p.properties().get("region")).map(Region::of).orElseThrow(() -> SdkClientException.builder().message("No region provided in profile: " + this.profileName).build());
    }
}
