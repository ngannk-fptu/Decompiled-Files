...
implements Marshaller<TagResourceRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasImplicitPayloadMembers(true).hasPayloadMembers(true).operationIdentifier("secretsmanager.TagResource").build();
    ...
    public SdkHttpFullRequest marshall(TagResourceRequest tagResourceRequest) {
        Validate.paramNotNull((Object)((Object)tagResourceRequest), (String)"tagResourceRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)tagResourceRequest);
        }
        catch (Exception e) {
