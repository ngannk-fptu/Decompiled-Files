...
implements Marshaller<GetResourcePolicyRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasImplicitPayloadMembers(true).hasPayloadMembers(true).operationIdentifier("secretsmanager.GetResourcePolicy").build();
    private final BaseAwsJsonProtocolFactory protocolFactory;

    public GetResourcePolicyRequestMarshaller(BaseAwsJsonProtocolFactory protocolFactory) {
        this.protocolFactory = protocolFactory;
    }

    public SdkHttpFullRequest marshall(GetResourcePolicyRequest getResourcePolicyRequest) {
        Validate.paramNotNull((Object)((Object)getResourcePolicyRequest), (String)"getResourcePolicyRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)getResourcePolicyRequest);
        }
        catch (Exception e) {
