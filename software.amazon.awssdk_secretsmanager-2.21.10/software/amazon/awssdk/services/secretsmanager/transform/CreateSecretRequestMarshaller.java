...
implements Marshaller<CreateSecretRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasImplicitPayloadMembers(true).hasPayloadMembers(true).operationIdentifier("secretsmanager.CreateSecret").build();
    ...
    public SdkHttpFullRequest marshall(CreateSecretRequest createSecretRequest) {
        Validate.paramNotNull((Object)((Object)createSecretRequest), (String)"createSecretRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)createSecretRequest);
        }
        catch (Exception e) {
