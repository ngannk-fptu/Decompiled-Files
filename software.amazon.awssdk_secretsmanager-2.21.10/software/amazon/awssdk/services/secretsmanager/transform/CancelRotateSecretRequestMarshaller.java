...
implements Marshaller<CancelRotateSecretRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasImplicitPayloadMembers(true).hasPayloadMembers(true).operationIdentifier("secretsmanager.CancelRotateSecret").build();
    private final BaseAwsJsonProtocolFactory protocolFactory;

    public CancelRotateSecretRequestMarshaller(BaseAwsJsonProtocolFactory protocolFactory) {
        this.protocolFactory = protocolFactory;
    }

    public SdkHttpFullRequest marshall(CancelRotateSecretRequest cancelRotateSecretRequest) {
        Validate.paramNotNull((Object)((Object)cancelRotateSecretRequest), (String)"cancelRotateSecretRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)cancelRotateSecretRequest);
        }
        catch (Exception e) {
