...
implements Marshaller<DescribeSecretRequest> {
    private static final OperationInfo SDK_OPERATION_BINDING = OperationInfo.builder().requestUri("/").httpMethod(SdkHttpMethod.POST).hasExplicitPayloadMember(false).hasImplicitPayloadMembers(true).hasPayloadMembers(true).operationIdentifier("secretsmanager.DescribeSecret").build();
    ...
    public SdkHttpFullRequest marshall(DescribeSecretRequest describeSecretRequest) {
        Validate.paramNotNull((Object)((Object)describeSecretRequest), (String)"describeSecretRequest");
        try {
            ProtocolMarshaller protocolMarshaller = this.protocolFactory.createProtocolMarshaller(SDK_OPERATION_BINDING);
            return (SdkHttpFullRequest)protocolMarshaller.marshall((SdkPojo)describeSecretRequest);
        }
        catch (Exception e) {
