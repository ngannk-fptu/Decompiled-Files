...
implements BeanDefinitionParser {
    ...
    protected ClassPathBeanDefinitionScanner configureScanner(ParserContext parserContext, Element element) {
        ...
        try {
            ...
            parserContext.getReaderContext().error(ex.getMessage(), parserContext.extractSource((Object)element), ex.getCause());
        }
        try {
    ...
    protected void parseTypeFilters(Element element, ClassPathBeanDefinitionScanner scanner, ParserContext parserContext) {
        ...
        for (int i = 0; i < nodeList.getLength(); ++i) {
            ...
            try {
                ...
                parserContext.getReaderContext().warning("Ignoring non-present type filter class: " + ex, parserContext.extractSource((Object)element));
                continue;
            }
            catch (Exception ex) {
                parserContext.getReaderContext().error(ex.getMessage(), parserContext.extractSource((Object)element), ex.getCause());
    ...
    protected TypeFilter createTypeFilter(Element element, @Nullable ClassLoader classLoader, ParserContext parserContext) throws ClassNotFoundException {
        ...
        if ("annotation".equals(filterType)) {
            return new AnnotationTypeFilter(ClassUtils.forName((String)expression, (ClassLoader)classLoader));
        }
    ...
    private Object instantiateUserDefinedStrategy(String className, Class<?> strategyType, @Nullable ClassLoader classLoader) {
        Object result;
        try {
            result = ReflectionUtils.accessibleConstructor((Class)ClassUtils.forName((String)className, (ClassLoader)classLoader), (Class[])new Class[0]).newInstance(new Object[0]);
        }
        catch (ClassNotFoundException ex) {
