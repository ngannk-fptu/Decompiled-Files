...
implements HttpRequestInterceptor {
    ...
    public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
        ...
        CookieOrigin cookieOrigin = new CookieOrigin(hostName, port >= 0 ? port : 0, !TextUtils.isEmpty((CharSequence)path) ? path : "/", route.isSecure());
        CookieSpecProvider provider = (CookieSpecProvider)registry.lookup(policy);
        if (provider == null) {
            if (this.log.isDebugEnabled()) {
                this.log.debug((Object)("Unsupported cookie policy: " + policy));
            }
            return;
        }
        ...
        context.setAttribute("http.cookie-spec", (Object)cookieSpec);
        context.setAttribute("http.cookie-origin", (Object)cookieOrigin);
    }
}
