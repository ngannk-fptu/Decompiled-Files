...
extends AbstractSegment {
    ...
    private void getMinMax(double p1, double p2, double p3, double p4, double[] minMax) {
        ...
        for (int r = 0; r < roots; ++r) {
            double tv = eqn[r];
            if (tv <= 0.0 || tv >= 1.0) continue;
            if ((tv = (1.0 - tv) * (1.0 - tv) * (1.0 - tv) * p1 + 3.0 * tv * (1.0 - tv) * (1.0 - tv) * p2 + 3.0 * tv * tv * (1.0 - tv) * p3 + tv * tv * tv * p4) < minMax[0]) {
                minMax[0] = tv;
                continue;
            }
            if (!(tv > minMax[1])) continue;
            minMax[1] = tv;
        }
    }
    ...
    protected double subLength(double leftLegLen, double rightLegLen, double maxErr) {
        ...
        c.p4.x = this.p4.x;
        c.p4.y = this.p4.y;
        return len += c.subLength(midLen, rightLegLen / 2.0, maxErr / 2.0);
    }
