...
implements AwsChunkSigner {
    ...
    public String signChunk(byte[] chunkData, String previousSignature) {
        ...
        try {
            ...
            return BinaryUtils.toHex((byte[])bytes);
        }
        catch (Exception e) {
    ...
    public String signChecksumChunk(byte[] calculatedChecksum, String previousSignature, String checksumHeaderForTrailer) {
        Map<String, List<String>> canonicalizeSigningHeaders = HeaderTransformsHelper.canonicalizeSigningHeaders(Collections.singletonMap(checksumHeaderForTrailer, Arrays.asList(BinaryUtils.toBase64((byte[])calculatedChecksum))));
        String canonicalizedHeaderString = HeaderTransformsHelper.getCanonicalizedHeaderString(canonicalizeSigningHeaders);
        String chunkStringToSign = "AWS4-HMAC-SHA256-TRAILER\n" + this.dateTime + "\n" + this.keyPath + "\n" + previousSignature + "\n" + BinaryUtils.toHex((byte[])this.sha256ForTrailer.digest(canonicalizedHeaderString.getBytes(StandardCharsets.UTF_8)));
        return BinaryUtils.toHex((byte[])this.trailerHmacSha256.doFinal(chunkStringToSign.getBytes(StandardCharsets.UTF_8)));
    }
