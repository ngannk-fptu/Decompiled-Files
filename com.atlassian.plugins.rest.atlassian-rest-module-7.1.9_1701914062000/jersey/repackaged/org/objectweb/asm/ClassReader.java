    ...
    private void readCode(MethodVisitor mv, Context context, int u) {
            ...
            if ("LocalVariableTable".equals(attrName)) {
                ...
                if ((context.flags & 2) == 0) {
                    ...
                    for (j = this.readUnsignedShort(u + 8); j > 0; --j) {
                        ...
                        labels[label].line = this.readUnsignedShort(v + 12);
                        v += 4;
                    }
                }
            } else if ("RuntimeVisibleTypeAnnotations".equals(attrName)) {
        ...
        if (stackMap != 0) {
            ...
            for (i = stackMap; i < stackMap + stackMapSize - 2; ++i) {
                int v2;
                if (b[i] != 8 || (v2 = this.readUnsignedShort(i + 1)) < 0 || v2 >= codeLength || (b[codeStart + v2] & 0xFF) != 187) continue;
        ...
        if (tanns != null) {
            for (int i9 = 0; i9 < tanns.length; ++i9) {
                if (this.readByte((int)tanns[i9]) >> 1 != 32) continue;
                int v3 = this.readAnnotationTarget(context, tanns[i9]);
                v3 = this.readAnnotationValues(v3 + 2, c, true, mv.visitLocalVariableAnnotation(context.typeRef, context.typePath, context.start, context.end, context.index, this.readUTF8(v3, c), true));
            }
        }
        if (itanns != null) {
            for (int i10 = 0; i10 < itanns.length; ++i10) {
                if (this.readByte((int)itanns[i10]) >> 1 != 32) continue;
                int v4 = this.readAnnotationTarget(context, itanns[i10]);
