...
implements Digest {
    ...
    static byte[] aesEnc(byte[] s, byte[] rk) {
        ...
        HarakaBase.xorWith(rk, s);
        return s;
    }

    static byte mulX(byte p) {
        return (byte)((p & 0x7F) << 1 ^ ((p & 0x80) >> 7) * 27);
    }
    ...
    private static byte[] mixColumns(byte[] s) {
        ...
        int j = 0;
        for (int i = 0; i < 4; ++i) {
            out[j++] = (byte)(HarakaBase.mulX(s[4 * i]) ^ HarakaBase.mulX(s[4 * i + 1]) ^ s[4 * i + 1] ^ s[4 * i + 2] ^ s[4 * i + 3]);
            out[j++] = (byte)(s[4 * i] ^ HarakaBase.mulX(s[4 * i + 1]) ^ HarakaBase.mulX(s[4 * i + 2]) ^ s[4 * i + 2] ^ s[4 * i + 3]);
            out[j++] = (byte)(s[4 * i] ^ s[4 * i + 1] ^ HarakaBase.mulX(s[4 * i + 2]) ^ HarakaBase.mulX(s[4 * i + 3]) ^ s[4 * i + 3]);
            out[j++] = (byte)(HarakaBase.mulX(s[4 * i]) ^ s[4 * i] ^ s[4 * i + 1] ^ s[4 * i + 2] ^ HarakaBase.mulX(s[4 * i + 3]));
        }
        return out;
    }
