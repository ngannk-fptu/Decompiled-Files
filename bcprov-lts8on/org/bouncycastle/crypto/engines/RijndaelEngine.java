...
implements BlockCipher {
    ...
    private long applyS(long r, byte[] box) {
        ...
        for (int j = 0; j < this.BC; j += 8) {
            res |= (long)(box[(int)(r >> j & 0xFFL)] & 0xFF) << j;
        }
        return res;
    }
    ...
    private void MixColumn() {
        ...
        for (int j = 0; j < this.BC; j += 8) {
            ...
            r0 |= (long)((this.mul0x2(a0) ^ this.mul0x3(a1) ^ a2 ^ a3) & 0xFF) << j;
            r1 |= (long)((this.mul0x2(a1) ^ this.mul0x3(a2) ^ a3 ^ a0) & 0xFF) << j;
            r2 |= (long)((this.mul0x2(a2) ^ this.mul0x3(a3) ^ a0 ^ a1) & 0xFF) << j;
            r3 |= (long)((this.mul0x2(a3) ^ this.mul0x3(a0) ^ a1 ^ a2) & 0xFF) << j;
        }
        this.A0 = r0;
        this.A1 = r1;
        this.A2 = r2;
    ...
    private void InvMixColumn() {
        ...
        for (int j = 0; j < this.BC; j += 8) {
            ...
            r0 |= (long)((this.mul0xe(a0) ^ this.mul0xb(a1) ^ this.mul0xd(a2) ^ this.mul0x9(a3)) & 0xFF) << j;
            r1 |= (long)((this.mul0xe(a1) ^ this.mul0xb(a2) ^ this.mul0xd(a3) ^ this.mul0x9(a0)) & 0xFF) << j;
            r2 |= (long)((this.mul0xe(a2) ^ this.mul0xb(a3) ^ this.mul0xd(a0) ^ this.mul0x9(a1)) & 0xFF) << j;
            r3 |= (long)((this.mul0xe(a3) ^ this.mul0xb(a0) ^ this.mul0xd(a1) ^ this.mul0x9(a2)) & 0xFF) << j;
        }
        this.A0 = r0;
        this.A1 = r1;
        this.A2 = r2;
    ...
    private long[][] generateWorkingKey(byte[] key) {
        ...
        this.ROUNDS = keyBits >= this.blockBits ? KC + 6 : this.BC / 8 + 6;
        int index = 0;
        for (i2 = 0; i2 < key.length; ++i2) {
            tk[i2 % 4][i2 / 4] = key[index++];
        }
        int t = 0;
        for (j = 0; j < KC && t < (this.ROUNDS + 1) * (this.BC / 8); ++j, ++t) {
            for (i = 0; i < 4; ++i) {
                long[] lArray = W[t / (this.BC / 8)];
                int n = i;
                lArray[n] = lArray[n] | (long)(tk[i][j] & 0xFF) << t * 8 % this.BC;
            }
        }
        while (t < (this.ROUNDS + 1) * (this.BC / 8)) {
            for (i2 = 0; i2 < 4; ++i2) {
                byte[] byArray = tk[i2];
                byArray[0] = (byte)(byArray[0] ^ S[tk[(i2 + 1) % 4][KC - 1] & 0xFF]);
            }
            byte[] byArray = tk[0];
            byArray[0] = (byte)(byArray[0] ^ rcon[rconpointer++]);
            if (KC <= 6) {
                for (j = 1; j < KC; ++j) {
                    for (i = 0; i < 4; ++i) {
                        byte[] byArray2 = tk[i];
                        int n = j;
                        byArray2[n] = (byte)(byArray2[n] ^ tk[i][j - 1]);
                    }
                }
            } else {
                for (j = 1; j < 4; ++j) {
                    for (i = 0; i < 4; ++i) {
                        byte[] byArray3 = tk[i];
                        int n = j;
                        byArray3[n] = (byte)(byArray3[n] ^ tk[i][j - 1]);
                    }
                }
                for (i2 = 0; i2 < 4; ++i2) {
                    byte[] byArray4 = tk[i2];
                    byArray4[4] = (byte)(byArray4[4] ^ S[tk[i2][3] & 0xFF]);
                }
                for (j = 5; j < KC; ++j) {
                    for (i = 0; i < 4; ++i) {
                        byte[] byArray5 = tk[i];
                        int n = j;
                        byArray5[n] = (byte)(byArray5[n] ^ tk[i][j - 1]);
                    }
                }
            }
            for (j = 0; j < KC && t < (this.ROUNDS + 1) * (this.BC / 8); ++j, ++t) {
    ...
    private void unpackBlock(byte[] bytes, int off) {
        ...
        for (int j = 8; j != this.BC; j += 8) {
            this.A0 |= (long)(bytes[index++] & 0xFF) << j;
            this.A1 |= (long)(bytes[index++] & 0xFF) << j;
            this.A2 |= (long)(bytes[index++] & 0xFF) << j;
            this.A3 |= (long)(bytes[index++] & 0xFF) << j;
        }
    }
    ...
    private void packBlock(byte[] bytes, int off) {
        int index = off;
        for (int j = 0; j != this.BC; j += 8) {
            bytes[index++] = (byte)(this.A0 >> j);
            bytes[index++] = (byte)(this.A1 >> j);
            bytes[index++] = (byte)(this.A2 >> j);
            bytes[index++] = (byte)(this.A3 >> j);
        }
    }
