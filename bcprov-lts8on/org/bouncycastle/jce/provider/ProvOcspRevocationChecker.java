...
implements PKIXCertRevocationChecker {
    ...
    public void check(java.security.cert.Certificate certificate) throws CertPathValidatorException {
        ...
        if (ocspUri == null) {
            if (this.ocspURL != null) {
                try {
                    ...
                    throw new CertPathValidatorException("configuration error: " + e.getMessage(), (Throwable)e, this.parameters.getCertPath(), this.parameters.getIndex());
                }
            } else {
                ocspUri = ProvOcspRevocationChecker.getOcspResponderURI(cert);
            }
        }
        byte[] nonce = null;
        ...
        if (ocspResponses.get(cert) == null && ocspUri != null) {
            ...
            Certificate issuer = this.extractCert();
            ...
            try {
                ...
                throw new CertPathValidatorException("unable to encode OCSP response", (Throwable)e, this.parameters.getCertPath(), this.parameters.getIndex());
            }
        } else {
            List<Extension> exts = this.parent.getOcspExtensions();
            for (int i = 0; i != exts.size(); ++i) {
                Extension ext = exts.get(i);
                byte[] value = ext.getValue();
                if (!OCSPObjectIdentifiers.id_pkix_ocsp_nonce.getId().equals(ext.getId())) continue;
                nonce = value;
            }
        }
        if (ocspResponses.isEmpty()) throw new RecoverableCertPathValidatorException("no OCSP response found for any certificate", null, this.parameters.getCertPath(), this.parameters.getIndex());
        OCSPResponse ocspResponse = OCSPResponse.getInstance(ocspResponses.get(cert));
        ASN1Integer serialNumber = new ASN1Integer(cert.getSerialNumber());
        if (ocspResponse == null) throw new RecoverableCertPathValidatorException("no OCSP response found for certificate", null, this.parameters.getCertPath(), this.parameters.getIndex());
        if (0 != ocspResponse.getResponseStatus().getIntValue()) throw new CertPathValidatorException("OCSP response failed: " + ocspResponse.getResponseStatus().getValue(), null, this.parameters.getCertPath(), this.parameters.getIndex());
        ResponseBytes respBytes = ResponseBytes.getInstance(ocspResponse.getResponseBytes());
        if (!respBytes.getResponseType().equals(OCSPObjectIdentifiers.id_pkix_ocsp_basic)) return;
        try {
            BasicOCSPResponse basicResp = BasicOCSPResponse.getInstance(respBytes.getResponse().getOctets());
            if (!preValidated && !ProvOcspRevocationChecker.validatedOcspResponse(basicResp, this.parameters, nonce, this.parent.getOcspResponderCert(), this.helper)) return;
            ...
            for (int i = 0; i != s.size(); ++i) {
                ...
                if (!serialNumber.equals(resp.getCertID().getSerialNumber())) continue;
                ...
                if (resp.getCertStatus().getTagNo() != 1) throw new CertPathValidatorException("certificate revoked, details unknown", null, this.parameters.getCertPath(), this.parameters.getIndex());
                RevokedInfo info = RevokedInfo.getInstance(resp.getCertStatus().getStatus());
                CRLReason reason = info.getRevocationReason();
                throw new CertPathValidatorException("certificate revoked, reason=(" + reason + "), date=" + info.getRevocationTime().getDate(), null, this.parameters.getCertPath(), this.parameters.getIndex());
            }
            return;
        }
            ...
            throw new CertPathValidatorException("unable to process OCSP response", (Throwable)e, this.parameters.getCertPath(), this.parameters.getIndex());
        }
    }
    ...
    static URI getOcspResponderURI(X509Certificate cert) {
        ...
        for (int i = 0; i != descriptions.length; ++i) {
            ...
            if (!AccessDescription.id_ad_ocsp.equals(aDesc.getAccessMethod()) || (name = aDesc.getAccessLocation()).getTagNo() != 6) continue;
            try {
                return new URI(((ASN1String)((Object)name.getName())).getString());
            }
    ...
    static boolean validatedOcspResponse(BasicOCSPResponse basicResp, PKIXCertRevocationCheckerParameters parameters, byte[] nonce, X509Certificate responderCert, JcaJceHelper helper) throws CertPathValidatorException {
        try {
            ...
            Signature sig = helper.createSignature(ProvOcspRevocationChecker.getSignatureName(basicResp.getSignatureAlgorithm()));
            X509Certificate sigCert = ProvOcspRevocationChecker.getSignerCert(basicResp, parameters.getSigningCert(), responderCert, helper);
            ...
            if (sigCert != null) {
                ...
                if (!ProvOcspRevocationChecker.responderMatches(basicResp.getTbsResponseData().getResponderID(), ocspCert, helper)) {
                ...
                List<String> extendedKeyUsage = ocspCert.getExtendedKeyUsage();
            ...
            if (sig.verify(basicResp.getSignature().getBytes())) {
                ...
                if (nonce != null && !Arrays.areEqual(nonce, (ext = (exts = basicResp.getTbsResponseData().getResponseExtensions()).getExtension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce)).getExtnValue().getOctets())) {
            ...
            throw new CertPathValidatorException("OCSP response failure: " + e.getMessage(), (Throwable)e, parameters.getCertPath(), parameters.getIndex());
        }
        catch (IOException e) {
            throw new CertPathValidatorException("OCSP response failure: " + e.getMessage(), (Throwable)e, parameters.getCertPath(), parameters.getIndex());
        }
    ...
    private static X509Certificate getSignerCert(BasicOCSPResponse basicResp, X509Certificate signingCert, X509Certificate responderCert, JcaJceHelper helper) throws NoSuchProviderException, NoSuchAlgorithmException {
        ...
        if (keyHash != null) {
            ...
            if (sigCert != null && Arrays.areEqual(keyHash, ProvOcspRevocationChecker.calcKeyHash(digest, sigCert.getPublicKey()))) {
                return sigCert;
            }
            sigCert = signingCert;
            if (sigCert != null && Arrays.areEqual(keyHash, ProvOcspRevocationChecker.calcKeyHash(digest, sigCert.getPublicKey()))) {
            ...
            X509Certificate sigCert = responderCert;
            if (sigCert != null && name.equals(X500Name.getInstance(BCStrictStyle.INSTANCE, sigCert.getSubjectX500Principal().getEncoded()))) {
                return sigCert;
            }
            sigCert = signingCert;
            if (sigCert != null && name.equals(X500Name.getInstance(BCStrictStyle.INSTANCE, sigCert.getSubjectX500Principal().getEncoded()))) {
    ...
    private static boolean responderMatches(ResponderID responderID, X509Certificate certificate, JcaJceHelper helper) throws NoSuchProviderException, NoSuchAlgorithmException {
        ...
        if (keyHash != null) {
            MessageDigest digest = helper.createMessageDigest("SHA1");
            return Arrays.areEqual(keyHash, ProvOcspRevocationChecker.calcKeyHash(digest, certificate.getPublicKey()));
        }
        X500Name name = X500Name.getInstance(BCStrictStyle.INSTANCE, responderID.getName());
    ...
    private Certificate extractCert() throws CertPathValidatorException {
        try {
            ...
            throw new CertPathValidatorException("cannot process signing cert: " + e.getMessage(), (Throwable)e, this.parameters.getCertPath(), this.parameters.getIndex());
        }
    }

    private CertID createCertID(CertID base, Certificate issuer, ASN1Integer serialNumber) throws CertPathValidatorException {
        return this.createCertID(base.getHashAlgorithm(), issuer, serialNumber);
    }

    private CertID createCertID(AlgorithmIdentifier digestAlg, Certificate issuer, ASN1Integer serialNumber) throws CertPathValidatorException {
        try {
            MessageDigest digest = this.helper.createMessageDigest(MessageDigestUtils.getDigestName(digestAlg.getAlgorithm()));
            DEROctetString issuerNameHash = new DEROctetString(digest.digest(issuer.getSubject().getEncoded("DER")));
            DEROctetString issuerKeyHash = new DEROctetString(digest.digest(issuer.getSubjectPublicKeyInfo().getPublicKeyData().getBytes()));
    ...
    private static String getSignatureName(AlgorithmIdentifier sigAlgId) {
        ...
        if (params != null && !DERNull.INSTANCE.equals(params) && sigAlgId.getAlgorithm().equals(PKCSObjectIdentifiers.id_RSASSA_PSS)) {
            ...
            return ProvOcspRevocationChecker.getDigestName(rsaParams.getHashAlgorithm().getAlgorithm()) + "WITHRSAANDMGF1";
        }
