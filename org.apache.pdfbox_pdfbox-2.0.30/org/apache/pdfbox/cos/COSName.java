...
implements Comparable<COSName> {
    private static final Map<String, COSName> nameMap = new ConcurrentHashMap<String, COSName>(8192);
    ...
    public static final COSName PART = new COSName("Part");
    ...
    public void writePDF(OutputStream output) throws IOException {
        byte[] bytes;
        output.write(47);
        for (byte b : bytes = this.getName().getBytes(Charsets.UTF_8)) {
            int current = b & 0xFF;
            if (current >= 65 && current <= 90 || current >= 97 && current <= 122 || current >= 48 && current <= 57 || current == 43 || current == 45 || current == 95 || current == 64 || current == 42 || current == 36 || current == 59 || current == 46) {
