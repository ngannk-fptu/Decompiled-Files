        ...
        try {
            ...
            for (int i = 0; i < pathsArray.length / 8; ++i) {
                ...
                if (Float.compare(len0, 0.0f) != 0) {
                    x0 += (pathsArray[i * 8] - pathsArray[i * 8 + 4]) / len0 * len0 / 7.0f;
                    y0 += (pathsArray[i * 8 + 1] - pathsArray[i * 8 + 5]) / len0 * (len0 / 7.0f);
                }
                ...
                if (Float.compare(len1, 0.0f) != 0) {
                    x1 += (pathsArray[i * 8 + 2] - pathsArray[i * 8 + 6]) / len1 * len1 / 7.0f;
                    y1 += (pathsArray[i * 8 + 3] - pathsArray[i * 8 + 7]) / len1 * len1 / 7.0f;
                }
                cs.moveTo(x0, y0);
                cs.lineTo(x1, y1);
            }
