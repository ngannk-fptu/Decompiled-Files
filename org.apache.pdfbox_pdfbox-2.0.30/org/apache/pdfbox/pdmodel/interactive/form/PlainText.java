...
class PlainText {
    ...
    PlainText(String textValue) {
        if (textValue.isEmpty()) {
            this.paragraphs = new ArrayList<Paragraph>(1);
        ...
        List<Line> getLines(PDFont font, float fontSize, float width) throws IOException {
            ...
            while (end != -1) {
                ...
                if (wordWidth > width && textLine.getWords().isEmpty()) {
                    ...
                    wordNeedsSplit = true;
                    while (!((substringWidth = font.getStringWidth(substring = word.substring(0, --splitOffset)) * scale) < width)) {
                    }
                    word = substring;
                    lineWidth = wordWidth = font.getStringWidth(word) * scale;
                }
