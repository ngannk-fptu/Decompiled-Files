...
public class OutputManager {
    ...
    protected boolean isWhiteSpace(char[] text) {
        for (char aText : text) {
            if (XMLUtilities.isXMLSpace((char)aText)) continue;
            return false;
        }
        return true;
    }
    ...
    protected boolean formatText(char[] text, String margin, boolean preceedingSpace) throws IOException {
        ...
        block0: while (i < text.length) {
            while (i < text.length) {
                if (XMLUtilities.isXMLSpace((char)text[i])) {
                    startsWithSpace = true;
                    ++i;
                    continue;
                }
                StringBuffer sb = new StringBuffer();
                while (i < text.length && !XMLUtilities.isXMLSpace((char)text[i])) {
                    sb.append(text[i++]);
                }
                if (sb.length() == 0) {
        ...
        public boolean isAttribute(String s) {
            if (this.name.length == s.length()) {
                for (int i = 0; i < this.name.length; ++i) {
                    if (this.name[i] == s.charAt(i)) continue;
                    return false;
                }
                return true;
            }
