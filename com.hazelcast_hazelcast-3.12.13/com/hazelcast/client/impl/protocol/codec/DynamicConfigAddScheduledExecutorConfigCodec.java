...
public final class DynamicConfigAddScheduledExecutorConfigCodec {
    public static final DynamicConfigMessageType REQUEST_TYPE = DynamicConfigMessageType.DYNAMICCONFIG_ADDSCHEDULEDEXECUTORCONFIG;
    ...
    public static ClientMessage encodeRequest(String name, int poolSize, int durability, int capacity, String quorumName, String mergePolicy, int mergeBatchSize) {
        int requiredDataSize = RequestParameters.calculateDataSize(name, poolSize, durability, capacity, quorumName, mergePolicy, mergeBatchSize);
        ClientMessage clientMessage = ClientMessage.createForEncode(requiredDataSize);
        clientMessage.setMessageType(REQUEST_TYPE.id());
        clientMessage.setRetryable(false);
        clientMessage.setAcquiresResource(false);
    ...
    public static class RequestParameters {
        ...
        public static int calculateDataSize(String name, int poolSize, int durability, int capacity) {
            int dataSize = ClientMessage.HEADER_SIZE;
        ...
        public static int calculateDataSize(String name, int poolSize, int durability, int capacity, String quorumName, String mergePolicy, int mergeBatchSize) {
            int dataSize = ClientMessage.HEADER_SIZE;
            dataSize += ParameterUtil.calculateDataSize(name);
            dataSize += 4;
            dataSize += 4;
