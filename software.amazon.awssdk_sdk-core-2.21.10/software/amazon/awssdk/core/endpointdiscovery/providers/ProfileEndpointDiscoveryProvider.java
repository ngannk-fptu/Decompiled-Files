...
implements EndpointDiscoveryProvider {
    ...
    public boolean resolveEndpointDiscovery() {
        return this.profileFile.get().profile(this.profileName).map(p -> (String)p.properties().get("aws_endpoint_discovery_enabled")).map(Boolean::parseBoolean).orElseThrow(() -> SdkClientException.builder().message("No endpoint discovery setting provided in profile: " + this.profileName).build());
    }
