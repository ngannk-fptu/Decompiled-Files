...
public final class CrtBasedChecksumProvider {
    ...
    private static final Lazy<Optional<Class<?>>> CRT_CRC32_CLASS_LOADER = new Lazy(() -> CrtBasedChecksumProvider.initializeCrtChecksumClass(CRT_CLASSPATH_FOR_CRC32));
    private static final Lazy<Optional<Class<?>>> CRT_CRC32_C_CLASS_LOADER = new Lazy(() -> CrtBasedChecksumProvider.initializeCrtChecksumClass(CRT_CLASSPATH_FOR_CRC32C));
    ...
    private static Checksum createCrtBasedChecksum(Lazy<Optional<Class<?>>> lazyClassLoader) {
        return ((Optional)lazyClassLoader.getValue()).map(checksumClass -> {
            try {
    ...
    private static Optional<Class<?>> initializeCrtChecksumClass(String classPath) {
        try {
            ...
            LOG.debug(() -> "Cannot find the " + classPath + " class. To invoke a request that requires a CRT based checksums.", (Throwable)e);
