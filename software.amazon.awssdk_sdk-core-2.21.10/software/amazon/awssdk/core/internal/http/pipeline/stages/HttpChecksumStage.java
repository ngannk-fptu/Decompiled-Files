...
implements MutableRequestToRequestPipeline {
    ...
    private void addMd5ChecksumInHeader(SdkHttpFullRequest.Builder request) {
        try {
            String payloadMd5 = Md5Utils.md5AsBase64((InputStream)request.contentStreamProvider().newStream());
    ...
    private void addFlexibleChecksumInTrailer(SdkHttpFullRequest.Builder request, RequestExecutionContext context, ChecksumSpecs checksumSpecs) {
        ...
        int chunkSize = 0;
        if (this.clientType == ClientType.SYNC) {
            request.contentStreamProvider((ContentStreamProvider)new ChecksumCalculatingStreamProvider(request.contentStreamProvider(), checksumSpecs));
    ...
    private void addFlexibleChecksumInHeader(SdkHttpFullRequest.Builder request, RequestExecutionContext context, ChecksumSpecs checksumSpecs) {
        try {
            String payloadChecksum = BinaryUtils.toBase64((byte[])HttpChecksumUtils.computeChecksum(context.executionContext().interceptorContext().requestBody().get().contentStreamProvider().newStream(), checksumSpecs.algorithm()));
    ...
    implements ContentStreamProvider {
        ...
        private void closeCurrentStream() {
            ...
            if (this.currentStream != null) {
                IoUtils.closeQuietly((AutoCloseable)this.currentStream, null);
