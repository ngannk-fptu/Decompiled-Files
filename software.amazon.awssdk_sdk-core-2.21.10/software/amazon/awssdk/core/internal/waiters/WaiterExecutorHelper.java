...
public final class WaiterExecutorHelper<T> {
    ...
    public WaiterResponse<T> createWaiterResponse(Either<T, Throwable> responseOrException, int attempts) {
        return (WaiterResponse)responseOrException.map(r -> DefaultWaiterResponse.builder().response(r).attemptsExecuted(attempts).build(), e -> DefaultWaiterResponse.builder().exception((Throwable)e).attemptsExecuted(attempts).build());
    }

    public Optional<WaiterAcceptor<? super T>> firstWaiterAcceptorIfMatched(Either<T, Throwable> responseOrException) {
        return (Optional)responseOrException.map(this::responseMatches, this::exceptionMatches);
    }
    ...
    public Either<Long, SdkClientException> nextDelayOrUnretryableException(int attemptNumber, long startTime) {
        if (attemptNumber >= this.maxAttempts) {
            return Either.right((Object)SdkClientException.create("The waiter has exceeded the max retry attempts: " + this.maxAttempts));
        }
        long nextDelay = this.computeNextDelayInMills(attemptNumber);
        if (this.exceedsMaxWaitTime(startTime, nextDelay)) {
            return Either.right((Object)SdkClientException.create("The waiter has exceeded the max wait time or the next retry will exceed the max wait time + " + this.waitTimeout));
        }
        return Either.left((Object)nextDelay);
    }

    public SdkClientException noneMatchException(Either<T, Throwable> responseOrException) {
        return (SdkClientException)responseOrException.map(r -> SdkClientException.create("No acceptor was matched for the response: " + r), t -> SdkClientException.create("An exception was thrown and did not match any waiter acceptors", t));
    }
