...
implements ServletActionContextCompat {
    ...
    private ServletActionContextCompat initialiseServletActionContextCompat(ClassLoader classLoader) {
        ...
        try {
            Class.forName(STRUTS_SERVLET_ACTION_CONTEXT, false, classLoader);
            internalDelegate = new ServletActionContextStruts2AndWWCompat(STRUTS_SERVLET_ACTION_CONTEXT, classLoader);
        }
        catch (ClassNotFoundException e) {
            log.debug("Could not find Struts2 ServletActionContext, falling back to WebWork ServletActionContext", (Throwable)e);
            try {
                internalDelegate = new ServletActionContextStruts2AndWWCompat(WW_SERVLET_ACTION_CONTEXT, classLoader);
            }
