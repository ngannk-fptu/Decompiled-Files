...
public abstract class LCS {
    ...
    private int find_middle_snake(int bottoml1, int topl1, int bottoml2, int topl2, int[][] V, int[] snake) {
        int N2 = topl1 - bottoml1 + 1;
        int M = topl2 - bottoml2 + 1;
        int delta = N2 - M;
        boolean isEven = (delta & 1) != 1;
        int limit = Math.min(this.max_differences, (N2 + M + 1) / 2);
        int value_to_add_forward = (M & 1) == 1 ? 1 : 0;
        int value_to_add_backward = (N2 & 1) == 1 ? 1 : 0;
        int start_forward = -M;
        int end_forward = N2;
        int start_backward = -N2;
        int end_backward = M;
        V[0][limit + 1] = 0;
        V[1][limit - 1] = N2;
        for (int d = 0; d <= limit; ++d) {
            ...
            for (k = start_diag; k <= end_diag; k += 2) {
                ...
                snake[1] = y + bottoml2;
                snake[2] = 0;
                for (y = x - k; x < N2 && y < M && this.isRangeEqual(x + bottoml1, y + bottoml2); ++x, ++y) {
                    snake[2] = snake[2] + 1;
                }
                V[0][limit + k] = x;
        ...
        int[] most_progress = LCS.findMostProgress(M, N2, limit, V);
    ...
    private static int[] findMostProgress(int M, int N2, int limit, int[][] V) {
        int delta = N2 - M;
        int forward_start_diag = (M & 1) == (limit & 1) ? Math.max(-M, -limit) : Math.max(1 - M, -limit);
        int forward_end_diag = Math.min(N2, limit);
        int backward_start_diag = (N2 & 1) == (limit & 1) ? Math.max(-N2, -limit) : Math.max(1 - N2, -limit);
