...
implements ManagedConnPool<T, C> {
    ...
    public Future<PoolEntry<T, C>> lease(T route, Object state, Timeout requestTimeout, FutureCallback<PoolEntry<T, C>> callback) {
        ...
        BasicFuture future = new BasicFuture<PoolEntry<T, C>>(callback){
            ...
            public synchronized PoolEntry<T, C> get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
                try {
                    return (PoolEntry)super.get(timeout, unit);
                }
        ...
        if (acquiredLock) {
            try {
                LeaseRequest request = new LeaseRequest(route, state, requestTimeout, future);
    ...
    private void processPendingRequests() {
        ListIterator it = this.pendingRequests.listIterator();
        while (it.hasNext()) {
    ...
    public PoolStats getTotalStats() {
        this.lock.lock();
        try {
            PoolStats poolStats = new PoolStats(this.leased.size(), this.pendingRequests.size(), this.available.size(), this.maxTotal);
            return poolStats;
        }
        finally {
            this.lock.unlock();
    ...
    public PoolStats getStats(T route) {
        ...
        try {
            PerRoutePool<T, C> pool = this.getPool(route);
            int pendingCount = 0;
            for (LeaseRequest leaseRequest : this.pendingRequests) {
                if (!Objects.equals(route, leaseRequest.getRoute())) continue;
                ++pendingCount;
            }
