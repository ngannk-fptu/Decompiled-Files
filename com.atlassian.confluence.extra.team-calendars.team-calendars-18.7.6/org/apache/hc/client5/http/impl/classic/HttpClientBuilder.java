...
public class HttpClientBuilder {
    ...
    public CloseableHttpClient build() {
        ...
        namedElementChain.addLast(new MainClientExec(connManagerCopy, httpProcessor, reuseStrategyCopy, keepAliveStrategyCopy, userTokenHandlerCopy), ChainElement.MAIN_TRANSPORT.name());
        namedElementChain.addFirst(new ConnectExec(reuseStrategyCopy, new DefaultHttpProcessor(new RequestTargetHost(), new RequestUserAgent(userAgentCopy)), proxyAuthStrategyCopy, this.schemePortResolver != null ? this.schemePortResolver : DefaultSchemePortResolver.INSTANCE, this.authCachingDisabled), ChainElement.CONNECT.name());
        namedElementChain.addFirst(new ProtocolExec(targetAuthStrategyCopy, proxyAuthStrategyCopy, this.schemePortResolver != null ? this.schemePortResolver : DefaultSchemePortResolver.INSTANCE, this.authCachingDisabled), ChainElement.PROTOCOL.name());
        if (!this.automaticRetriesDisabled) {
            ...
            namedElementChain.addFirst(new HttpRequestRetryExec(retryStrategyCopy), ChainElement.RETRY.name());
        }
        ...
        if (!this.contentCompressionDisabled) {
            if (this.contentDecoderMap != null) {
                ...
                namedElementChain.addFirst(new ContentCompressionExec(encodings, decoderRegistry, true), ChainElement.COMPRESS.name());
            } else {
        ...
        if (!this.redirectHandlingDisabled) {
            ...
            namedElementChain.addFirst(new RedirectExec(routePlannerCopy, (RedirectStrategy)redirectStrategyCopy), ChainElement.REDIRECT.name());
        }
        ...
        return new InternalHttpClient(connManagerCopy, requestExecCopy, execChain, routePlannerCopy, cookieSpecRegistryCopy, authSchemeRegistryCopy, defaultCookieStore, defaultCredentialsProvider, this.defaultRequestConfig != null ? this.defaultRequestConfig : RequestConfig.DEFAULT, closeablesCopy);
    }
