...
ConnPoolControl<HttpRoute> {
    ...
    public PoolingAsyncClientConnectionManager() {
        this(RegistryBuilder.create().register(URIScheme.HTTPS.getId(), DefaultClientTlsStrategy.getDefault()).build());
    }
                ...
                this.leaseFuture = PoolingAsyncClientConnectionManager.this.pool.lease(route, state, requestTimeout, new FutureCallback<PoolEntry<HttpRoute, ManagedAsyncClientConnection>>(){
                    ...
                    public void completed(PoolEntry<HttpRoute, ManagedAsyncClientConnection> poolEntry) {
                        ...
                        if (poolEntry.hasConnection()) {
                            ...
                            if (connection.isOpen() && TimeValue.isNonNegative(timeValue) && (deadline2 = Deadline.calculate(poolEntry.getUpdated(), timeValue)).isExpired()) {
    ...
    public void upgrade(final AsyncConnectionEndpoint endpoint, Object attachment, HttpContext context, final FutureCallback<AsyncConnectionEndpoint> callback) {
        ...
        HttpHost host = route.getProxyHost() != null ? route.getProxyHost() : route.getTargetHost();
        TlsConfig tlsConfig = this.resolveTlsConfig(host, attachment);
        this.connectionOperator.upgrade(poolEntry.getConnection(), route.getTargetHost(), attachment != null ? attachment : tlsConfig, context, (FutureCallback<ManagedAsyncClientConnection>)new CallbackContribution<ManagedAsyncClientConnection>(callback){
