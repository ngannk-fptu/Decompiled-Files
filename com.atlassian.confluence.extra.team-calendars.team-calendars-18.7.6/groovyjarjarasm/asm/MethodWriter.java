...
import groovyjarjarasm.asm.AnnotationWriter;
import groovyjarjarasm.asm.Attribute;
import groovyjarjarasm.asm.ByteVector;
import groovyjarjarasm.asm.ClassWriter;
import groovyjarjarasm.asm.CurrentFrame;
import groovyjarjarasm.asm.Edge;
import groovyjarjarasm.asm.Frame;
import groovyjarjarasm.asm.Handle;
import groovyjarjarasm.asm.Handler;
...
extends MethodVisitor {
        ...
        if (this.compute == 1) {
            ...
            for (i = 0; i < nLocal; ++i) {
                if (local[i] instanceof String) {
                    desc = Type.getObjectType((String)local[i]).getDescriptor();
                    this.frame[frameIndex++] = Frame.type(this.cw, desc);
                    continue;
                }
                this.frame[frameIndex++] = local[i] instanceof Integer ? 0x1000000 | (Integer)local[i] : 0x1800000 | this.cw.addUninitializedType("", ((Label)local[i]).position);
            }
            for (i = 0; i < nStack; ++i) {
                if (stack[i] instanceof String) {
                    desc = Type.getObjectType((String)stack[i]).getDescriptor();
                    this.frame[frameIndex++] = Frame.type(this.cw, desc);
                    continue;
                }
                this.frame[frameIndex++] = stack[i] instanceof Integer ? 0x1000000 | (Integer)stack[i] : 0x1800000 | this.cw.addUninitializedType("", ((Label)stack[i]).position);
            }
            this.endFrame();
        } else {
            int delta;
    ...
    public void visitTypeInsn(int opcode, String type) {
        this.lastCodeOffset = this.code.length;
        Item i = this.cw.newStringishItem(7, type);
        if (this.currentBlock != null) {
            if (this.compute == 0 || this.compute == 1) {
    ...
    public void visitMultiANewArrayInsn(String desc, int dims) {
        this.lastCodeOffset = this.code.length;
        Item i = this.cw.newStringishItem(7, desc);
        if (this.currentBlock != null) {
            if (this.compute == 0 || this.compute == 1) {
    ...
    private void visitImplicitFirstFrame() {
        ...
        if ((this.access & 8) == 0) {
            this.frame[frameIndex++] = (this.access & 0x80000) == 0 ? 0x1700000 | this.cw.addType(this.cw.thisName) : 0x1000006;
        }
        int i = 1;
        block8: while (true) {
            int j = i;
            switch (this.descriptor.charAt(i++)) {
                ...
                case 'S': 
                case 'Z': {
                    this.frame[frameIndex++] = 0x1000001;
                    continue block8;
                }
                case 'F': {
                    this.frame[frameIndex++] = 0x1000002;
                    continue block8;
                }
                case 'J': {
                    this.frame[frameIndex++] = 0x1000004;
                    continue block8;
                }
                case 'D': {
                    this.frame[frameIndex++] = 0x1000003;
                    continue block8;
                }
                case '[': {
                    ...
                    this.frame[frameIndex++] = Frame.type(this.cw, this.descriptor.substring(j, ++i));
                    continue block8;
                }
                case 'L': {
                    while (this.descriptor.charAt(i) != ';') {
