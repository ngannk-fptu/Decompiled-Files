...
import groovyjarjarasm.asm.MethodWriter;
...
public class ClassReader {
    ...
    private void readModule(ClassVisitor classVisitor, Context context, int u, String mainClass, int packages) {
        ...
        String name = this.readModule(u, buffer);
        ...
        if (packages != 0) {
            for (i = this.readUnsignedShort(packages - 2); i > 0; --i) {
    ...
    private void readCode(MethodVisitor mv, Context context, int u) {
        ...
        block32: while (u < codeEnd) {
            ...
            switch (ClassWriter.TYPE[opcode]) {
                ...
                case 9: {
                    this.createLabel(offset + this.readShort(u + 1), labels);
                    u += 3;
                    continue block32;
                }
                case 18: {
                    this.createLabel(offset + this.readUnsignedShort(u + 1), labels);
                    u += 3;
                    continue block32;
                }
                case 10: 
                case 19: {
                    this.createLabel(offset + this.readInt(u + 1), labels);
                    u += 5;
                    continue block32;
                }
                case 17: {
                ...
                case 14: {
                    int i2;
                    u = u + 4 - (offset & 3);
                    this.createLabel(offset + this.readInt(u), labels);
                    for (i2 = this.readInt(u + 8) - this.readInt(u + 4) + 1; i2 > 0; --i2) {
                        this.createLabel(offset + this.readInt(u + 12), labels);
                        u += 4;
                    }
                    u += 12;
                    continue block32;
        ...
        for (int i3 = this.readUnsignedShort(u); i3 > 0; --i3) {
            Label start = this.createLabel(this.readUnsignedShort(u + 2), labels);
            Label end = this.createLabel(this.readUnsignedShort(u + 4), labels);
            ...
            if ("LocalVariableTable".equals(attrName)) {
                if ((context.flags & 2) == 0) {
                    ...
                    for (j = this.readUnsignedShort(u + 8); j > 0; --j) {
                        ...
                        this.createDebugLabel(label, labels);
                        this.createDebugLabel(label += this.readUnsignedShort(v + 12), labels);
                        v += 10;
                    }
                }
            ...
            switch (ClassWriter.TYPE[opcode]) {
                ...
                case 18: {
                    ...
                    if (opcode == 167 || opcode == 168) {
                        ...
                        Label endif = this.createLabel(offset + 3, labels);
                ...
                case 19: {
                    mv.visitJumpInsn(200, labels[offset + this.readInt(u + 1)]);
                    insertFrame = true;
                    u += 5;
                    break;
                }
    ...
    private int readFrameType(Object[] frame, int index, int v, char[] buf, Label[] labels) {
        ...
        switch (type) {
            ...
            default: {
                frame[index] = this.createLabel(this.readUnsignedShort(v), labels);
                v += 2;
            }
        }
        return v;
    ...
    private Label createLabel(int offset, Label[] labels) {
        Label label = this.readLabel(offset, labels);
    ...
    private void createDebugLabel(int offset, Label[] labels) {
        if (labels[offset] == null) {
            this.readLabel((int)offset, (Label[])labels).status |= 1;
        }
    }
    ...
    public String readClass(int index, char[] buf) {
        return this.readStringish(index, buf);
    }
