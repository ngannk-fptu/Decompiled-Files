...
implements ApfloatImpl {
    ...
    private IntApfloatImpl(int sign, long precision, long exponent, DataStorage dataStorage, int radix) {
        ...
        assert (sign == 0 || dataStorage != null);
        assert (exponent <= IntRadixConstants.MAX_EXPONENT[radix] && exponent >= -IntRadixConstants.MAX_EXPONENT[radix]);
    ...
    public IntApfloatImpl(String value, long precision, int radix, boolean isInteger) throws NumberFormatException, ApfloatRuntimeException {
        super(IntApfloatImpl.checkRadix(radix));
        int slack;
        ...
        if (integerSize >= -(slack = IntRadixConstants.BASE_DIGITS[radix]) && this.exponent >= Long.MAX_VALUE - (long)integerSize - (long)slack) {
        ...
        long baseExp = (this.exponent + (long)(this.exponent > 0L ? IntRadixConstants.BASE_DIGITS[radix] - 1 : 0)) / (long)IntRadixConstants.BASE_DIGITS[radix];
        if (baseExp > IntRadixConstants.MAX_EXPONENT[this.radix]) {
        ...
        int digitsInBase = (int)(baseExp * (long)IntRadixConstants.BASE_DIGITS[radix] - this.exponent);
        this.exponent = baseExp;
        digitSize -= leadingZeros + trailingZeros;
        digitSize = (int)Math.min((long)digitSize, precision);
        int size = (int)this.getBasePrecision(digitSize, IntRadixConstants.BASE_DIGITS[radix] - digitsInBase);
        this.dataStorage = IntApfloatImpl.createDataStorage(size);
        ...
        while (digitSize > 0) {
            ...
            if (c != '.') {
                ...
                word += digit;
                if (digitSize == 1) {
                    while (digitsInBase < IntRadixConstants.BASE_DIGITS[radix] - 1) {
                        word *= radix;
                        ++digitsInBase;
                    }
                }
                if (++digitsInBase == IntRadixConstants.BASE_DIGITS[radix]) {
                    digitsInBase = 0;
    ...
    public IntApfloatImpl(long value, long precision, int radix) throws NumberFormatException, ApfloatRuntimeException {
        super(IntApfloatImpl.checkRadix(radix));
        int size;
        assert (precision > 0L);
        this.radix = radix;
        ...
        this.precision = precision;
        int[] data = new int[4];
        long longBase = IntRadixConstants.BASE[radix];
        if (-longBase < value) {
            size = 1;
            data[3] = (int)(-value);
        } else {
            ...
            while (value != 0L) {
                ...
                data[3 - size] = (int)(newValue * longBase - value);
                value = newValue;
                ++size;
            }
        }
        ...
        while (data[3 - (int)this.exponent + size] == 0) {
            --size;
        }
        this.dataStorage = IntApfloatImpl.createDataStorage(size);
        this.dataStorage.setSize(size);
    ...
    public IntApfloatImpl(double value, long precision, int radix) throws NumberFormatException, ApfloatRuntimeException {
        super(IntApfloatImpl.checkRadix(radix));
        int size;
        ...
        this.precision = precision;
        int[] data = new int[4];
        double doubleBase = IntRadixConstants.BASE[radix];
     ...
     */
    public IntApfloatImpl(PushbackReader in, long precision, int radix, boolean isInteger) throws IOException, NumberFormatException, ApfloatRuntimeException {
        super(IntApfloatImpl.checkRadix(radix));
        int slack;
        int input;
        assert (precision == Long.MIN_VALUE || precision > 0L);
        this.radix = radix;
        this.sign = 1;
        long initialSize = IntApfloatImpl.getBlockSize();
        long previousAllocatedSize = 0L;
        long allocatedSize = initialSize;
        this.dataStorage = IntApfloatImpl.createDataStorage(initialSize);
                ...
                block26: {
                    ...
                    if (i == 0L && (c == '-' || c == '+')) {
                        ...
                        if (!(isInteger || digitSize <= 0L || c != 'e' && c != 'E')) {
                            this.exponent = IntApfloatImpl.readExponent(in);
                            break;
                        }
                        in.unread(input);
                        break;
                ...
                if (leadingZeros == digitSize && digit == 0) {
                    ...
                    if (actualSize == allocatedSize) {
                        if (actualSize == initialSize) {
                            DataStorage dataStorage = IntApfloatImpl.createDataStorage(0x1FFFFFFFFFFFFFFFL);
                            dataStorage.copyFrom(this.dataStorage, actualSize);
                            this.dataStorage = dataStorage;
                        }
                        previousAllocatedSize = allocatedSize;
                        this.dataStorage.setSize(allocatedSize += (long)IntApfloatImpl.getBlockSize());
                        iterator.close();
        ...
        if (digitsInBase > 0 && word != 0) {
            while (digitsInBase < IntRadixConstants.BASE_DIGITS[radix]) {
                word *= radix;
                ++digitsInBase;
            }
            iterator.setInt(word);
        ...
        if (integerSize >= (long)(-(slack = IntRadixConstants.BASE_DIGITS[radix])) && this.exponent >= Long.MAX_VALUE - integerSize - (long)slack) {
            throw new NumberFormatException("Exponent overflow");
        }
        ...
        int bias = (int)(this.exponent - baseExp * (long)IntRadixConstants.BASE_DIGITS[radix]);
        this.exponent = baseExp;
        digitSize -= leadingZeros + trailingZeros;
        digitSize = Math.min(digitSize, precision);
        actualSize = (digitSize + (long)IntRadixConstants.BASE_DIGITS[radix] - 1L) / (long)IntRadixConstants.BASE_DIGITS[radix];
        ...
        if (bias != 0) {
            ...
            IntApfloatImpl tmp = (IntApfloatImpl)this.multiply(new IntApfloatImpl(factor, Long.MAX_VALUE, radix));
    ...
    public ApfloatImpl addOrSubtract(ApfloatImpl x, boolean subtract) throws ApfloatRuntimeException {
        ...
        if (this == that) {
            ...
            dataStorage = IntApfloatImpl.createDataStorage(size);
            ...
            try (DataStorage.Iterator dst = dataStorage.iterator(2, size, 0L);){
                carry = additionStrategy.add(src1, src2, 0, dst, size - 1L);
                dst.setInt(carry);
            }
            size -= IntApfloatImpl.getTrailingZeros(dataStorage, size);
            int carrySize = carry;
            ...
            if (this.exponent == IntRadixConstants.MAX_EXPONENT[this.radix] && carrySize > 0) {
                throw new OverflowException("Overflow");
            }
            ...
            if (scaleDifference < 0L) {
                ...
                long basePrecision = Math.min(IntRadixConstants.MAX_EXPONENT[this.radix], this.getBasePrecision(precision, big.getInitialDigits()));
                exponent = big.exponent;
            ...
            dataStorage = IntApfloatImpl.createDataStorage(dstSize);
            ...
            try (DataStorage.Iterator dst = dataStorage.iterator(2, dstSize, 0L);){
                ...
                if (size > bigSize) {
                    ...
                    carry = reallySubtract ? additionStrategy.subtract(null, src2, carry, dst, blockSize).intValue() : additionStrategy.add(null, src2, carry, dst, blockSize).intValue();
                } else if (size > exponentDifference + smallSize) {
                    blockSize = size - exponentDifference - smallSize;
                    carry = reallySubtract ? additionStrategy.subtract(src1, null, carry, dst, blockSize).intValue() : additionStrategy.add(src1, null, carry, dst, blockSize).intValue();
                }
                if (exponentDifference > bigSize) {
                    blockSize = exponentDifference - bigSize;
                    carry = reallySubtract ? additionStrategy.subtract(null, null, carry, dst, blockSize).intValue() : additionStrategy.add(null, null, carry, dst, blockSize).intValue();
                } else if (bigSize > exponentDifference) {
                    blockSize = Math.min(bigSize - exponentDifference, smallSize);
                    carry = reallySubtract ? additionStrategy.subtract(src1, src2, carry, dst, blockSize).intValue() : additionStrategy.add(src1, src2, carry, dst, blockSize).intValue();
                }
            ...
            if (reallySubtract) {
                leadingZeros = IntApfloatImpl.getLeadingZeros(dataStorage, 0L);
                assert (leadingZeros <= size);
            } else {
                leadingZeros = carry == 0 ? 1 : 0;
                if (this.exponent == IntRadixConstants.MAX_EXPONENT[this.radix] && leadingZeros == 0L) {
                    throw new OverflowException("Overflow");
                }
            }
            dstSize -= IntApfloatImpl.getTrailingZeros(dataStorage, dstSize);
            dataStorage = dataStorage.subsequence(leadingZeros, dstSize - leadingZeros);
            if ((exponent += 1L - leadingZeros) < -IntRadixConstants.MAX_EXPONENT[this.radix]) {
                return this.zero();
            }
            if (precision != Long.MAX_VALUE) {
                long scaleChange = (1L - leadingZeros) * (long)IntRadixConstants.BASE_DIGITS[this.radix] + (long)this.getInitialDigits(dataStorage) - (long)big.getInitialDigits();
    ...
    public ApfloatImpl multiply(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        if (exponent > IntRadixConstants.MAX_EXPONENT[this.radix]) {
            throw new OverflowException("Overflow");
        }
        if (exponent < -IntRadixConstants.MAX_EXPONENT[this.radix]) {
            return this.zero();
        ...
        int n = leadingZeros = IntApfloatImpl.getMostSignificantWord(dataStorage) == 0 ? 1 : 0;
        if ((exponent -= (long)leadingZeros) < -IntRadixConstants.MAX_EXPONENT[this.radix]) {
            return this.zero();
        }
        dataStorage = dataStorage.subsequence(leadingZeros, size -= (long)leadingZeros);
    ...
    public ApfloatImpl divideShort(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        int divisor = IntApfloatImpl.getMostSignificantWord(that.dataStorage);
        if (divisor == 1) {
            long size = thisDataSize - IntApfloatImpl.getTrailingZeros(this.dataStorage, thisDataSize);
            ...
            for (int i = 0; i < RadixConstants.RADIX_FACTORS[this.radix].length; ++i) {
                int quotient;
                int factor = RadixConstants.RADIX_FACTORS[this.radix][i];
    ...
    public ApfloatImpl absFloor() throws ApfloatRuntimeException {
        ...
        size -= IntApfloatImpl.getTrailingZeros(this.dataStorage, size);
        DataStorage dataStorage = this.dataStorage.subsequence(0L, size);
        IntApfloatImpl apfloatImpl = new IntApfloatImpl(this.sign, Long.MAX_VALUE, this.exponent, dataStorage, this.radix);
        return apfloatImpl;
    }
    ...
    public ApfloatImpl absCeil() throws ApfloatRuntimeException {
        ...
        if (this.exponent <= 0L) {
            ...
            AdditionStrategy<Integer> additionStrategy = additionBuilder.createAddition(this.radix);
            long size = this.exponent;
            dataStorage = IntApfloatImpl.createDataStorage(size + 1L);
            ...
            try (DataStorage.Iterator src = this.dataStorage.iterator(1, size, 0L);
                ...
                carry = additionStrategy.add(src, null, 1, dst, size);
                dst.setInt(carry);
            }
            int carrySize = carry;
            size -= IntApfloatImpl.getTrailingZeros(dataStorage, size + 1L);
            dataStorage = dataStorage.subsequence(1 - carrySize, size + (long)carrySize);
            exponent = this.exponent + (long)carrySize;
        }
        dataStorage.setReadOnly();
        IntApfloatImpl apfloatImpl = new IntApfloatImpl(this.sign, Long.MAX_VALUE, exponent, dataStorage, this.radix);
        return apfloatImpl;
    }
    ...
    public ApfloatImpl frac() throws ApfloatRuntimeException {
        ...
        long leadingZeros = IntApfloatImpl.getLeadingZeros(this.dataStorage, this.exponent);
        ...
        if (this.precision != Long.MAX_VALUE) {
            precision = this.precision - (long)this.getInitialDigits() - (this.exponent + leadingZeros) * (long)IntRadixConstants.BASE_DIGITS[this.radix] + (long)this.getInitialDigits(dataStorage);
            if (precision <= 0L) {
                return this.zero();
            }
        } else {
        ...
        IntApfloatImpl apfloatImpl = new IntApfloatImpl(this.sign, precision, exponent, dataStorage, this.radix);
        return apfloatImpl;
    }
    ...
    public long size() throws ApfloatRuntimeException {
        ...
        if (this.size == 0L) {
            this.size = (long)this.getInitialDigits() + (this.getSize() - 1L) * (long)IntRadixConstants.BASE_DIGITS[this.radix] - this.getLeastZeros();
        }
        return this.size;
    }
    ...
    private long getLeastZeros() throws ApfloatRuntimeException {
        if (this.leastZeros == Integer.MIN_VALUE) {
            ...
            int word = this.getWord(index);
            word = this.getLeastSignificantWord(index, word);
            long leastZeros = 0L;
            if (word == 0) {
                long trailingZeros = IntApfloatImpl.getTrailingZeros(this.dataStorage, index) + 1L;
                word = this.getWord(index -= trailingZeros);
                word = this.getLeastSignificantWord(index, word);
                leastZeros += trailingZeros * (long)IntRadixConstants.BASE_DIGITS[this.radix];
            }
            assert (word != 0);
    ...
    public long scale() throws ApfloatRuntimeException {
        assert (this.dataStorage != null);
        return (this.exponent - 1L) * (long)IntRadixConstants.BASE_DIGITS[this.radix] + (long)this.getInitialDigits();
    }
    ...
    public double doubleValue() {
        ...
        double value = 0.0;
        double doubleBase = IntRadixConstants.BASE[this.radix];
        ...
        if (this.exponent > 0L) {
            return (double)this.sign * value * Math.pow(IntRadixConstants.BASE[this.radix], this.exponent - 1L) * (double)IntRadixConstants.BASE[this.radix];
        }
        return (double)this.sign * value * Math.pow(IntRadixConstants.BASE[this.radix], this.exponent);
    }
    ...
    public long longValue() {
        ...
        long value = 0L;
        long longBase = IntRadixConstants.BASE[this.radix];
    ...
    public boolean isOne() throws ApfloatRuntimeException {
        if (this.isOne == Integer.MIN_VALUE) {
            this.isOne = this.sign == 1 && this.exponent == 1L && this.getSize() == 1L && this.getMostSignificantWord() == 1 ? 1 : 0;
        }
        return this.isOne == 1;
    }
    ...
    public long equalDigits(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        try (DataStorage.Iterator thisIterator = this.getZeroPaddedIterator(0L, thisSize);
            ...
            for (index = 0L; index < size; ++index) {
                value = thisIterator.getInt() - thatIterator.getInt() + carry;
                if (value == 0) {
                    carry = 0;
                } else {
                    if (Math.abs(value) > 1) {
                        ...
                        lastMatchingDigits = IntRadixConstants.BASE_DIGITS[this.radix] - this.getDigits(Math.abs(value));
                        break;
                    }
                    if (value == 1) {
                        carry = base;
            ...
            if (index < size || carry != 0) {
                long initialMatchingDigits = this.exponent == that.exponent ? Math.min(this.getInitialDigits(), that.getInitialDigits()) : IntRadixConstants.BASE_DIGITS[this.radix];
                long middleMatchingDigits = (index - 1L) * (long)IntRadixConstants.BASE_DIGITS[this.radix];
            ...
            public int getInt() throws ApfloatRuntimeException {
                ...
                if (this.index < end) {
                    ...
                    if (this.index == end - 1L) {
                        value = IntApfloatImpl.this.getLeastSignificantWord(this.index, value);
                    }
                } else {
                    value = 0;
                }
    ...
    private int compareMantissaTo(IntApfloatImpl that) throws ApfloatRuntimeException {
        int result = 0;
        long thisSize = this.getSize();
        long thatSize = that.getSize();
        long size = Math.max(thisSize, thatSize);
        try (DataStorage.Iterator thisIterator = this.getZeroPaddedIterator(0L, thisSize);
            ...
            if (index >= 0L) {
                ...
                if (thisValue < (thatValue = thatIterator.getInt())) {
                    result = -1;
    ...
    private int getLeastSignificantWord(long index, int word) throws ApfloatRuntimeException {
        if (this.precision == Long.MAX_VALUE) {
            return word;
        }
        long digits = (long)this.getInitialDigits() + index * (long)IntRadixConstants.BASE_DIGITS[this.radix];
        if (this.precision >= digits) {
            return word;
        }
        int divisor = IntRadixConstants.MINIMUM_FOR_DIGITS[this.radix][(int)(digits - this.precision)];
    ...
    public String toString(boolean pretty) throws ApfloatRuntimeException {
        ...
        long size = this.getSize() * (long)IntRadixConstants.BASE_DIGITS[this.radix];
        if (pretty) {
    ...
    public void writeTo(Writer out, boolean pretty) throws IOException, ApfloatRuntimeException {
        ...
        if (pretty) {
            if (this.exponent <= 0L) {
                out.write("0.");
                IntApfloatImpl.writeZeros(out, -this.scale());
                integerDigits = -1L;
            } else {
        ...
        long digitsToWrite = Math.min(this.precision, (long)this.getInitialDigits() + (size - 1L) * (long)IntRadixConstants.BASE_DIGITS[this.radix]);
        long digitsWritten = 0L;
        long trailingZeros = 0L;
        DataStorage.Iterator iterator = this.dataStorage.iterator(1, 0L, size);
        char[] buffer = new char[IntRadixConstants.BASE_DIGITS[this.radix]];
        for (size = this.getSize(); size > 0L; --size) {
            int start = leftPadZeros ? 0 : IntRadixConstants.BASE_DIGITS[this.radix] - this.getInitialDigits();
            int digits = (int)Math.min(digitsToWrite, (long)(IntRadixConstants.BASE_DIGITS[this.radix] - start));
            this.formatWord(buffer, iterator.getInt());
        ...
        IntApfloatImpl.writeZeros(out, integerDigits - digitsWritten);
    }

    private void formatWord(char[] buffer, int word) {
        int position = IntRadixConstants.BASE_DIGITS[this.radix];
    ...
    private int getMostSignificantWord() throws ApfloatRuntimeException {
        return IntApfloatImpl.getMostSignificantWord(this.dataStorage);
    }
    ...
    private int getInitialDigits(DataStorage dataStorage) throws ApfloatRuntimeException {
        return this.getDigits(IntApfloatImpl.getMostSignificantWord(dataStorage));
    }

    private int getDigits(int x) {
        assert (x > 0);
        int[] minimums = IntRadixConstants.MINIMUM_FOR_DIGITS[this.radix];
    ...
    private long getBasePrecision(long precision, int mswDigits) {
        ...
        return (precision + (long)IntRadixConstants.BASE_DIGITS[this.radix] - (long)mswDigits - 1L) / (long)IntRadixConstants.BASE_DIGITS[this.radix] + 1L;
    }

    private int getWord(long index) {
        int word;
        try (ArrayAccess arrayAccess = this.dataStorage.getArray(1, index, 1);){
            word = arrayAccess.getIntData()[arrayAccess.getOffset()];
        }
        return word;
    }
    ...
    private static DataStorage createDataStorage(long size) throws ApfloatRuntimeException {
        ...
        return dataStorageBuilder.createDataStorage(size * 4L);
    }
