...
Parallelizable {
    ...
    implements Runnable {
        ...
        public void run() {
            ...
            while (this.length > 0L) {
                ...
                iterator.setDouble(DoubleNTTConvolutionStepStrategy.this.modMultiply(value, value));
                iterator.next();
                --this.length;
            }
        }
    ...
    implements Runnable {
        ...
        public void run() {
            ...
            while (this.length > 0L) {
                dest.setDouble(DoubleNTTConvolutionStepStrategy.this.modMultiply(dest.getDouble(), src.getDouble()));
                dest.next();
                src.next();
                --this.length;
            }
