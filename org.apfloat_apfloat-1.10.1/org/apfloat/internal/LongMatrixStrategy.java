...
implements MatrixStrategy {
    ...
    public void transpose(ArrayAccess arrayAccess, int n1, int n2) throws ApfloatRuntimeException {
        ...
        if (n1 == n2) {
            LongMatrixStrategy.transposeSquare(data, offset, n1, n1);
        } else if (n2 == 2 * n1) {
            LongMatrixStrategy.transposeSquare(data, offset, n1, n2);
            LongMatrixStrategy.transposeSquare(data, offset + n1, n1, n2);
            LongMatrixStrategy.permuteToHalfWidth(data, offset, n1, n2);
        } else if (n1 == 2 * n2) {
            LongMatrixStrategy.permuteToDoubleWidth(data, offset, n1, n2);
            LongMatrixStrategy.transposeSquare(data, offset, n2, n1);
            LongMatrixStrategy.transposeSquare(data, offset + n2, n2, n1);
        } else {
    ...
    private static void transposeSquare(long[] data, int offset, int n1, int n2) {
        ...
        if (n1 <= cacheBurstBlockSize || n1 <= cacheBlockSize) {
            LongMatrixStrategy.transposeBlock(data, offset, n2, n1);
        } else if (n1 * n2 <= cacheThreshold) {
            int b = cacheBurstBlockSize;
            int i = 0;
            int position1 = offset;
            while (i < n1) {
                LongMatrixStrategy.transposeBlock(data, position1 + i, n2, b);
                int j = i + b;
                int position2 = offset + j * n2 + i;
                while (j < n1) {
                    LongMatrixStrategy.transpose2blocks(data, position1 + j, position2, n2, b);
                    j += b;
                    position2 += b * n2;
                }
                i += b;
            ...
            int position1 = offset;
            while (i < n1) {
                LongMatrixStrategy.moveBlock(data, position1 + i, n2, tmp1, 0, b, b);
                LongMatrixStrategy.transposeBlock(tmp1, 0, b, b);
                LongMatrixStrategy.moveBlock(tmp1, 0, b, data, position1 + i, n2, b);
                int j = i + b;
                int position2 = offset + j * n2 + i;
                while (j < n1) {
                    LongMatrixStrategy.moveBlock(data, position1 + j, n2, tmp1, 0, b, b);
                    LongMatrixStrategy.transposeBlock(tmp1, 0, b, b);
                    LongMatrixStrategy.moveBlock(data, position2, n2, tmp2, 0, b, b);
                    LongMatrixStrategy.transposeBlock(tmp2, 0, b, b);
                    LongMatrixStrategy.moveBlock(tmp2, 0, b, data, position1 + j, n2, b);
                    LongMatrixStrategy.moveBlock(tmp1, 0, b, data, position2, n2, b);
                    j += b;
                    position2 += b * n2;
                }
                i += b;
