...
implements ApfloatImpl {
    ...
    private FloatApfloatImpl(int sign, long precision, long exponent, DataStorage dataStorage, int radix) {
        ...
        assert (sign == 0 || dataStorage != null);
        assert (exponent <= FloatRadixConstants.MAX_EXPONENT[radix] && exponent >= -FloatRadixConstants.MAX_EXPONENT[radix]);
    ...
    public FloatApfloatImpl(String value, long precision, int radix, boolean isInteger) throws NumberFormatException, ApfloatRuntimeException {
        ...
        if (baseExp > FloatRadixConstants.MAX_EXPONENT[this.radix]) {
        ...
        int size = (int)this.getBasePrecision(digitSize, FloatRadixConstants.BASE_DIGITS[radix] - digitsInBase);
        this.dataStorage = FloatApfloatImpl.createDataStorage(size);
    ...
    public FloatApfloatImpl(long value, long precision, int radix) throws NumberFormatException, ApfloatRuntimeException {
        super(FloatApfloatImpl.checkRadix(radix));
        int size;
        assert (precision > 0L);
        this.radix = radix;
        ...
        while (data[3 - (int)this.exponent + size] == 0.0f) {
            --size;
        }
        this.dataStorage = FloatApfloatImpl.createDataStorage(size);
        this.dataStorage.setSize(size);
    ...
    public FloatApfloatImpl(double value, long precision, int radix) throws NumberFormatException, ApfloatRuntimeException {
        super(FloatApfloatImpl.checkRadix(radix));
        int size;
    ...
    public FloatApfloatImpl(PushbackReader in, long precision, int radix, boolean isInteger) throws IOException, NumberFormatException, ApfloatRuntimeException {
        ...
        this.radix = radix;
        this.sign = 1;
        long initialSize = FloatApfloatImpl.getBlockSize();
    ...
    public ApfloatImpl addOrSubtract(ApfloatImpl x, boolean subtract) throws ApfloatRuntimeException {
        ...
        if (this == that) {
            ...
            dataStorage = FloatApfloatImpl.createDataStorage(size);
            ...
            try (DataStorage.Iterator dst = dataStorage.iterator(2, size, 0L);){
                carry = additionStrategy.add(src1, src2, Float.valueOf(0.0f), dst, size - 1L).floatValue();
                dst.setFloat(carry);
            }
            size -= FloatApfloatImpl.getTrailingZeros(dataStorage, size);
            ...
            try (DataStorage.Iterator dst = dataStorage.iterator(2, dstSize, 0L);){
                ...
                if (size > bigSize) {
                    ...
                    carry = reallySubtract ? additionStrategy.subtract(null, src2, Float.valueOf(carry), dst, blockSize).floatValue() : additionStrategy.add(null, src2, Float.valueOf(carry), dst, blockSize).floatValue();
                } else if (size > exponentDifference + smallSize) {
                    blockSize = size - exponentDifference - smallSize;
                    carry = reallySubtract ? additionStrategy.subtract(src1, null, Float.valueOf(carry), dst, blockSize).floatValue() : additionStrategy.add(src1, null, Float.valueOf(carry), dst, blockSize).floatValue();
                }
                if (exponentDifference > bigSize) {
                    blockSize = exponentDifference - bigSize;
                    carry = reallySubtract ? additionStrategy.subtract(null, null, Float.valueOf(carry), dst, blockSize).floatValue() : additionStrategy.add(null, null, Float.valueOf(carry), dst, blockSize).floatValue();
                } else if (bigSize > exponentDifference) {
                    blockSize = Math.min(bigSize - exponentDifference, smallSize);
                    carry = reallySubtract ? additionStrategy.subtract(src1, src2, Float.valueOf(carry), dst, blockSize).floatValue() : additionStrategy.add(src1, src2, Float.valueOf(carry), dst, blockSize).floatValue();
                }
            ...
            if (reallySubtract) {
                leadingZeros = FloatApfloatImpl.getLeadingZeros(dataStorage, 0L);
            ...
            if ((exponent += 1L - leadingZeros) < -FloatRadixConstants.MAX_EXPONENT[this.radix]) {
                return this.zero();
            }
    ...
    public ApfloatImpl multiply(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        if (exponent > FloatRadixConstants.MAX_EXPONENT[this.radix]) {
            throw new OverflowException("Overflow");
        }
        if (exponent < -FloatRadixConstants.MAX_EXPONENT[this.radix]) {
            return this.zero();
        ...
        if ((exponent -= (long)leadingZeros) < -FloatRadixConstants.MAX_EXPONENT[this.radix]) {
            return this.zero();
        }
        dataStorage = dataStorage.subsequence(leadingZeros, size -= (long)leadingZeros);
    ...
    public ApfloatImpl divideShort(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        if (divisor == 1.0f) {
            long size = thisDataSize - FloatApfloatImpl.getTrailingZeros(this.dataStorage, thisDataSize);
            ...
            if (dividend != 1.0f) {
                ...
                while (carry != 0.0f) {
                    carry = additionStrategy.divide(null, Float.valueOf(divisor), Float.valueOf(carry), dummy, 1L).floatValue();
                    ++sequenceSize;
                }
            ...
            carry = additionStrategy.divide(src, Float.valueOf(divisor), Float.valueOf(0.0f), dst, thisDataSize).floatValue();
            carry = additionStrategy.divide(null, Float.valueOf(divisor), Float.valueOf(carry), dst, size - thisDataSize).floatValue();
            size -= FloatApfloatImpl.getTrailingZeros(dataStorage, size);
    ...
    public ApfloatImpl absFloor() throws ApfloatRuntimeException {
        ...
        size -= FloatApfloatImpl.getTrailingZeros(this.dataStorage, size);
    ...
    public ApfloatImpl absCeil() throws ApfloatRuntimeException {
        ...
        if (this.exponent <= 0L) {
            ...
            AdditionStrategy<Float> additionStrategy = additionBuilder.createAddition(this.radix);
            long size = this.exponent;
            dataStorage = FloatApfloatImpl.createDataStorage(size + 1L);
            ...
            try (DataStorage.Iterator src = this.dataStorage.iterator(1, size, 0L);
                ...
                carry = additionStrategy.add(src, null, Float.valueOf(1.0f), dst, size).floatValue();
                dst.setFloat(carry);
            }
            int carrySize = (int)carry;
            size -= FloatApfloatImpl.getTrailingZeros(dataStorage, size + 1L);
            dataStorage = dataStorage.subsequence(1 - carrySize, size + (long)carrySize);
            exponent = this.exponent + (long)carrySize;
        }
        dataStorage.setReadOnly();
        FloatApfloatImpl apfloatImpl = new FloatApfloatImpl(this.sign, Long.MAX_VALUE, exponent, dataStorage, this.radix);
        return apfloatImpl;
    }
    ...
    public ApfloatImpl frac() throws ApfloatRuntimeException {
        ...
        long leadingZeros = FloatApfloatImpl.getLeadingZeros(this.dataStorage, this.exponent);
        ...
        if (this.precision != Long.MAX_VALUE) {
            precision = this.precision - (long)this.getInitialDigits() - (this.exponent + leadingZeros) * (long)FloatRadixConstants.BASE_DIGITS[this.radix] + (long)this.getInitialDigits(dataStorage);
            if (precision <= 0L) {
                return this.zero();
            }
        } else {
        ...
        FloatApfloatImpl apfloatImpl = new FloatApfloatImpl(this.sign, precision, exponent, dataStorage, this.radix);
        return apfloatImpl;
    }
    ...
    public long size() throws ApfloatRuntimeException {
        ...
        if (this.size == 0L) {
            this.size = (long)this.getInitialDigits() + (this.getSize() - 1L) * (long)FloatRadixConstants.BASE_DIGITS[this.radix] - this.getLeastZeros();
        }
        return this.size;
    }
    ...
    public double doubleValue() {
        ...
        if (this.exponent > 0L) {
            return (double)this.sign * value * Math.pow(FloatRadixConstants.BASE[this.radix], this.exponent - 1L) * (double)FloatRadixConstants.BASE[this.radix];
        }
        return (double)this.sign * value * Math.pow(FloatRadixConstants.BASE[this.radix], this.exponent);
    }
    ...
    public boolean isOne() throws ApfloatRuntimeException {
        if (this.isOne == Integer.MIN_VALUE) {
            this.isOne = this.sign == 1 && this.exponent == 1L && this.getSize() == 1L && this.getMostSignificantWord() == 1.0f ? 1 : 0;
        }
        return this.isOne == 1;
    }
    ...
    public long equalDigits(ApfloatImpl x) throws ApfloatRuntimeException {
        ...
        try (DataStorage.Iterator thisIterator = this.getZeroPaddedIterator(0L, thisSize);
            ...
            for (index = 0L; index < size; ++index) {
                value = thisIterator.getFloat() - thatIterator.getFloat() + carry;
                if (value == 0.0f) {
                    carry = 0.0f;
                } else {
                    if (Math.abs(value) > 1.0f) {
                        ...
                        lastMatchingDigits = FloatRadixConstants.BASE_DIGITS[this.radix] - this.getDigits(Math.abs(value));
                        break;
                    }
                    if (value == 1.0f) {
                        carry = base;
            ...
            public float getFloat() throws ApfloatRuntimeException {
                ...
                if (this.index < end) {
                    ...
                    if (this.index == end - 1L) {
                        value = FloatApfloatImpl.this.getLeastSignificantWord(this.index, value);
                    }
                } else {
                    value = 0.0f;
                }
    ...
    public String toString(boolean pretty) throws ApfloatRuntimeException {
        ...
        long size = this.getSize() * (long)FloatRadixConstants.BASE_DIGITS[this.radix];
        if (pretty) {
    ...
    public void writeTo(Writer out, boolean pretty) throws IOException, ApfloatRuntimeException {
        ...
        long digitsToWrite = Math.min(this.precision, (long)this.getInitialDigits() + (size - 1L) * (long)FloatRadixConstants.BASE_DIGITS[this.radix]);
        ...
        for (size = this.getSize(); size > 0L; --size) {
            ...
            this.formatWord(buffer, iterator.getFloat());
    ...
    private void formatWord(char[] buffer, float word) {
        ...
        while (position > 0 && word > 0.0f) {
            ...
            int digit = (int)(word - newWord * (float)this.radix);
            word = newWord;
    ...
    private float getWord(long index) {
        ...
        try (ArrayAccess arrayAccess = this.dataStorage.getArray(1, index, 1);){
            word = arrayAccess.getFloatData()[arrayAccess.getOffset()];
        }
        return word;
    }
