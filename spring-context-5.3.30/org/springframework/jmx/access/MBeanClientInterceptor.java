...
DisposableBean {
    ...
    public void prepare() {
        ...
        synchronized (object) {
            ...
            if (this.useStrictCasing) {
                Assert.state((this.objectName != null ? 1 : 0) != 0, (String)"No ObjectName set");
    ...
    private void retrieveMBeanInfo(MBeanServerConnection server) throws MBeanInfoRetrievalException {
        try {
            ...
            this.allowedAttributes = CollectionUtils.newHashMap((int)attributeInfo.length);
            for (MBeanAttributeInfo infoEle : attributeInfo) {
                this.allowedAttributes.put(infoEle.getName(), infoEle);
            }
            MBeanOperationInfo[] operationInfo = info.getOperations();
            this.allowedOperations = CollectionUtils.newHashMap((int)operationInfo.length);
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        try {
            ...
            return this.handleConnectFailure(invocation, (Exception)ex);
        }
    }
    ...
    protected Object doInvoke(MethodInvocation invocation) throws Throwable {
        ...
        try {
            ...
            Object result = this.invocationHandler != null ? this.invocationHandler.invoke(invocation.getThis(), method, invocation.getArguments()) : ((pd = BeanUtils.findPropertyForMethod((Method)method)) != null ? this.invokeAttribute(pd, invocation) : this.invokeOperation(method, invocation.getArguments()));
            ...
            if (ReflectionUtils.declaresException((Method)method, ex.getClass())) {
                throw ex;
            }
    ...
    private Object invokeAttribute(PropertyDescriptor pd, MethodInvocation invocation) throws JMException, IOException {
        Assert.state((this.serverToUse != null ? 1 : 0) != 0, (String)"No MBeanServerConnection available");
    ...
    protected Object convertResultValueIfNecessary(@Nullable Object result, MethodParameter parameter) {
        ...
        try {
            ...
            if (ClassUtils.isAssignableValue((Class)targetClass, (Object)result)) {
                return result;
            }
            if (result instanceof CompositeData) {
                Method fromMethod = targetClass.getMethod("from", CompositeData.class);
                return ReflectionUtils.invokeMethod((Method)fromMethod, null, (Object[])new Object[]{result});
            }
            if (result instanceof CompositeData[]) {
                Class elementType;
                ...
                if (Collection.class.isAssignableFrom(targetClass) && (elementType = ResolvableType.forMethodParameter((MethodParameter)parameter).asCollection().resolveGeneric(new int[0])) != null) {
    ...
    private Object convertDataArrayToTargetArray(Object[] array, Class<?> targetClass) throws NoSuchMethodException {
        ...
        for (int i = 0; i < array.length; ++i) {
            Array.set(resultArray, i, ReflectionUtils.invokeMethod((Method)fromMethod, null, (Object[])new Object[]{array[i]}));
        }
        return resultArray;
    }
    ...
    private Collection<?> convertDataArrayToTargetCollection(Object[] array, Class<?> collectionType, Class<?> elementType) throws NoSuchMethodException {
        ...
        Collection resultColl = CollectionFactory.createCollection(collectionType, (int)Array.getLength(array));
        for (int i = 0; i < array.length; ++i) {
            resultColl.add(ReflectionUtils.invokeMethod((Method)fromMethod, null, (Object[])new Object[]{array[i]}));
        }
        return resultColl;
    }
    ...
    implements Comparable<MethodCacheKey> {
        ...
        public String toString() {
            return this.name + "(" + StringUtils.arrayToCommaDelimitedString((Object[])this.parameterTypes) + ")";
        }
