...
public class HttpStatus {
    ...
    private static final Code[] codeMap = new Code[512];
    ...
    public static String getMessage(int code) {
        Code codeEnum = HttpStatus.getCode(code);
        if (codeEnum != null) {
    ...
    public static boolean isInterim(int code) {
        return HttpStatus.isInformational(code) && code != 101;
    }
    ...
    static {
        Code[] codeArray = Code.values();
        int n = codeArray.length;
        for (int i = 0; i < n; ++i) {
            Code code;
            HttpStatus.codeMap[code._code] = code = codeArray[i];
        }
    }
