...
@LuceneIndependent
@Internal
public class ReindexAllSpacesChangeIndexTask
implements ConfluenceIndexTask {
    private static final Logger log = LoggerFactory.getLogger(ReindexAllSpacesChangeIndexTask.class);
    private static final JournalEntryType journalEntryType = JournalEntryType.REINDEX_ALL_SPACES_CHANGE;
    private final SpaceDao spaceDao;
    private final IndexTaskFactoryInternal indexTaskFactory;

    public ReindexAllSpacesChangeIndexTask(SpaceDao spaceDao, IndexTaskFactoryInternal indexTaskFactory) {
    ...
    public String getDescription() {
        return "index.task.reindex.allspaces.change";
    }
        ...
        this.spaceDao.performOnAll(space -> {
            try {
                if (ChangeDocumentIndexPolicy.shouldIndex(space)) {
                    this.indexTaskFactory.createRebuildChangeDocumentsIndexTask((Searchable)space).perform(writer);
                }
            }
            catch (IOException e) {
