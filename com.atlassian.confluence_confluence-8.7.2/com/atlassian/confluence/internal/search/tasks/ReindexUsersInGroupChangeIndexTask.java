...
HandleAware {
    private static final Logger log = LoggerFactory.getLogger(ReindexUsersInGroupChangeIndexTask.class);
    private static final JournalEntryType journalEntryType = JournalEntryType.REINDEX_USERS_IN_GROUP_CHANGE;
    ...
    public ReindexUsersInGroupChangeIndexTask(BatchOperationManager batchOperationManager, PersonalInformationManager personalInformationManager, GroupResolver groupResolver, GroupMembershipAccessor groupMembershipAccessor, IndexTaskFactoryInternal indexTaskFactory, String groupName) {
    ...
    public String getDescription() {
        return "index.task.reindex.users.in.group.change";
    }
        ...
        try {
            this.batchOperationManager.applyInBatches(personalInfoNamesToReindex, personalInfoNamesToReindex.size(), new Function<String, Void>(){
                ...
                public Void apply(String username) {
                    PersonalInformation personalInformation = ReindexUsersInGroupChangeIndexTask.this.personalInformationManager.getOrCreatePersonalInformation((User)new DefaultUser(username));
                    try {
                        if (ChangeDocumentIndexPolicy.shouldIndex(personalInformation)) {
                            ReindexUsersInGroupChangeIndexTask.this.indexTaskFactory.createRebuildChangeDocumentsIndexTask(personalInformation).perform(writer);
                        }
                    }
                    catch (IOException e) {
                ...
                public String toString() {
                    return "PersonalInfo reindexing for group '" + ReindexUsersInGroupChangeIndexTask.this.groupName + "'";
                }
            });
        }
