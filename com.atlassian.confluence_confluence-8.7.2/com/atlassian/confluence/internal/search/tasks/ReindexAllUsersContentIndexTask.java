...
implements ConfluenceIndexTask {
    private static final Logger log = LoggerFactory.getLogger(ReindexAllUsersContentIndexTask.class);
    private static final JournalEntryType journalEntryType = JournalEntryType.REINDEX_ALL_USERS;
    ...
    public String getDescription() {
        return "index.task.reindex.users.content";
    }
        ...
        try {
            this.batchOperationManager.applyInBatches(listOfInfoId, listOfInfoId.size(), new Function<Long, Void>(){
                ...
                public Void apply(Long id) {
                    PersonalInformation personalInformation = ReindexAllUsersContentIndexTask.this.personalInformationDao.getById(id);
                    try {
                        ReindexAllUsersContentIndexTask.this.indexTaskFactory.createUpdateDocumentTask(personalInformation).perform(writer);
                    }
                    catch (IOException e) {
