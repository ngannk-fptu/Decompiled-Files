...
class CustomCategorySectionSpec
implements ContentNameSearchSectionSpec {
    private static final Set<String> FIELD_NAMES = ImmutableSet.builder().addAll(FieldValuesTransformers.DEFAULT_FIELD_NAMES).add((Object)SearchFieldNames.CONTENT_PLUGIN_KEY).build();
    private final Function<SearchResult, ContentNameMatch> searchResultTransformer;

    CustomCategorySectionSpec(Function<SearchResult, ContentNameMatch> searchResultTransformer) {
    ...
    public SearchQuery getFilter() {
        return new ContentCategoryQuery(Category.CUSTOM);
    }
    ...
    public Function<List<Map<String, String>>, List<SearchResult>> getFieldValuesTransformer() {
        return FieldValuesTransformers.customTransformer();
    }
