...
public class ViewChangesSinceLastEditAction
...
implements ContentDetailAction {
    ...
    public AbstractPage getOriginalPage() {
        if (this.originalPage == null) {
            ...
            for (VersionHistorySummary previousVersion : previousVersions) {
                if (!previousVersion.getLastModifierName().equals(this.getAuthenticatedUser().getName())) continue;
                this.originalPage = this.pageManager.getPage(previousVersion.getId());
                break;
            }
            if (this.originalPage == null) {
