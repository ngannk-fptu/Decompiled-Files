...
public class MsSqlCollationRule
extends AbstractDatabaseCollationRule {
    @VisibleForTesting
    static final URL KB_URL = UrlBuilder.createURL("https://confluence.atlassian.com/doc/database-setup-for-sql-server-9258.html");

    public MsSqlCollationRule(ErrorMessageProvider errorMessageProvider, DatabaseCollationVerifier databaseCollationVerifier, SingleConnectionProvider databaseHelper, HibernateConfig hibernateConfig, String[] supportedCollations) {
    ...
    protected String getCollationScript() {
        return "SELECT collation_name FROM sys.databases\nWHERE name = ? \nAND collation_name NOT IN (" + StringUtils.repeat((String)"?", (String)",", (int)this.supportedCollations.length) + ")";
    }
