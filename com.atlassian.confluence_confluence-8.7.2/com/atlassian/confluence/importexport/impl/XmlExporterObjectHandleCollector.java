    ...
    private Iterable<TransientHibernateHandle> getLikesFromContent(List<ContentEntityObject> contentList) {
        ...
        for (List contentSublist : Lists.partition(contentList, (int)500)) {
            ...
            for (ContentEntityObject obj : contentSublist) {
                ...
                for (List commentsSubList : Lists.partition(new ArrayList<Comment>(comments), (int)500)) {
                    likeHandles.addAll(this.likeDao.getLikeEntities(commentsSubList).stream().map(item -> this.translator.objectToHandle(item)).collect(Collectors.toList()));
                }
            }
        }
        return likeHandles;
