...
extends AbstractSpacePermissionManager {
    ...
    public Map<String, Long> getUsersForPermissionType(String permissionType, @Nullable Space space) {
        ...
        for (SpacePermission spacePermission : permissions) {
            ConfluenceUser user;
            String username;
            if (!spacePermission.isUserPermission() || !spacePermission.getType().equalsIgnoreCase(permissionType) || result.get(username = spacePermission.getUserName()) != null || (user = this.getUserAccessor().getUserByName(username)) == null) continue;
