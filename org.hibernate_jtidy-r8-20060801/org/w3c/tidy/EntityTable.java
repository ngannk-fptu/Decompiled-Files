...
public final class EntityTable {
    ...
    public int entityCode(String name) {
        ...
        Entity ent = this.lookup(name.substring(1));
        if (ent != null) {
            return ent.getCode();
        }
        return 0;
    ...
    public String entityName(short code) {
        ...
        while (en.hasNext()) {
            Entity ent = (Entity)en.next();
            if (ent.getCode() != code) continue;
            name = ent.getName();
            break;
        }
        return name;
