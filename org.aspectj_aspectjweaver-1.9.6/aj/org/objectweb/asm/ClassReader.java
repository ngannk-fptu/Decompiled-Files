...
import aj.org.objectweb.asm.Context;
import aj.org.objectweb.asm.FieldVisitor;
import aj.org.objectweb.asm.Handle;
import aj.org.objectweb.asm.Label;
import aj.org.objectweb.asm.MethodVisitor;
import aj.org.objectweb.asm.MethodWriter;
...
public class ClassReader {
    ...
    ClassReader(byte[] classFileBuffer, int classFileOffset, boolean checkClassVersion) {
        ...
        if (checkClassVersion && this.readShort(classFileOffset + 6) > 59) {
    ...
    public void accept(ClassVisitor classVisitor, Attribute[] attributePrototypes, int parsingOptions) {
        ...
        for (int i = this.readUnsignedShort(currentAttributeOffset - 2); i > 0; --i) {
            ...
            } else if ("PermittedSubtypes".equals(attributeName)) {
        ...
        if (permittedSubtypesOffset != 0) {
            int numberOfPermittedSubtypes = this.readUnsignedShort(permittedSubtypesOffset);
            int currentPermittedSubtypeOffset = permittedSubtypesOffset + 2;
            while (numberOfPermittedSubtypes-- > 0) {
                classVisitor.visitPermittedSubtypeExperimental(this.readClass(currentPermittedSubtypeOffset, charBuffer));
    ...
    private void readCode(MethodVisitor methodVisitor, Context context, int codeOffset) {
        ...
        int bytecodeStartOffset = currentOffset += 8;
        ...
        boolean bl = expandFrames = (context.parsingOptions & 8) != 0;
            ...
            while (localVariableTableLength-- > 0) {
                ...
                if (typeTable != null) {
                    for (int i = 0; i < typeTable.length; i += 3) {
                        if (typeTable[i] != startPc || typeTable[i + 1] != index) continue;
        ...
        if (visibleTypeAnnotationOffsets != null) {
            ...
            for (int i = 0; i < n; ++i) {
                int typeAnnotationOffset = typeTable[i];
