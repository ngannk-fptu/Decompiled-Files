...
final class Constants {
    ...
    static boolean isWhitelisted(String internalName) {
        if (!internalName.startsWith("aj/org/objectweb/asm/")) {
            return false;
        }
        String member = "(Annotation|Class|Field|Method|Module|RecordComponent|Signature)";
        return internalName.contains("Test$") || Pattern.matches("aj/org/objectweb/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName) || Pattern.matches("aj/org/objectweb/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
    }
