...
implements ByteBufAllocator {
    ...
    protected static ByteBuf toLeakAwareBuffer(ByteBuf buf) {
        switch (ResourceLeakDetector.getLevel()) {
            ...
            case PARANOID: {
                ResourceLeakTracker leak = AbstractByteBuf.leakDetector.track((Object)buf);
                if (leak == null) break;
                buf = new AdvancedLeakAwareByteBuf(buf, (ResourceLeakTracker<ByteBuf>)leak);
                break;
            }
        }
        return buf;
    ...
    protected static CompositeByteBuf toLeakAwareBuffer(CompositeByteBuf buf) {
        switch (ResourceLeakDetector.getLevel()) {
            ...
            case PARANOID: {
                ResourceLeakTracker leak = AbstractByteBuf.leakDetector.track((Object)buf);
                if (leak == null) break;
                buf = new AdvancedLeakAwareCompositeByteBuf(buf, (ResourceLeakTracker<ByteBuf>)leak);
                break;
            }
        }
        return buf;
    ...
    public int calculateNewCapacity(int minNewCapacity, int maxCapacity) {
        ...
        int newCapacity = MathUtil.findNextPositivePowerOfTwo((int)Math.max(minNewCapacity, 64));
