...
implements EventStreamResponseHandler<ResponseT, EventT> {
    ...
    protected EventStreamResponseHandlerFromBuilder(DefaultEventStreamResponseHandlerBuilder<ResponseT, EventT, ?> builder) {
        Validate.mutuallyExclusive((String)"onEventStream and subscriber are mutually exclusive, set only one on the Builder", (Object[])new Object[]{builder.onEventStream(), builder.subscriber()});
        Supplier subscriber = builder.subscriber();
        Consumer<Object> consumer = this.onEventStream = subscriber != null ? p -> p.subscribe((Subscriber)subscriber.get()) : builder.onEventStream();
        ...
        this.responseConsumer = (Consumer)Validate.getOrDefault(builder.onResponse(), FunctionalUtils::noOpConsumer);
        this.errorConsumer = (Consumer)Validate.getOrDefault(builder.onError(), FunctionalUtils::noOpConsumer);
        this.onComplete = (Runnable)Validate.getOrDefault((Object)builder.onComplete(), FunctionalUtils::noOpRunnable);
        this.publisherTransformer = (Function)Validate.getOrDefault(builder.publisherTransformer(), Function::identity);
    }
