...
extends ParserBase {
    ...
    private final void _isNextTokenNameYes(int i) throws IOException {
        ...
        switch (i) {
            ...
            case 43: {
                this._nextToken = this.isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature()) ? this._parseSignedNumber(false) : this._handleOddValue(i);
                return;
            }
            case 46: {
    ...
    private final JsonToken _parseSignedNumber(boolean negative) throws IOException {
        ...
        if ((ch = this._inputBuffer[ptr++]) > '9' || ch < '0') {
            this._inputPtr = ptr;
            if (ch == '.') {
                return this._parseFloatThatStartsWithPeriod(negative);
            }
            return this._handleInvalidNumberStart(ch, negative, true);
        }
        if (ch == '0') {
            return this._parseNumber2(negative, startPtr);
        }
    ...
    private final JsonToken _parseNumber2(boolean neg, int startPtr) throws IOException {
        ...
        if (c == 'e' || c == 'E') {
            ...
            outBuf[outPtr++] = c;
            char c3 = c = this._inputPtr < this._inputEnd ? this._inputBuffer[this._inputPtr++] : this.getNextChar("expected a digit for number exponent", JsonToken.VALUE_NUMBER_FLOAT);
            if (c == '-' || c == '+') {
                if (outPtr >= outBuf.length) {
                    outBuf = this._textBuffer.finishCurrentSegment();
                    outPtr = 0;
                ...
                outBuf[outPtr++] = c;
                char c4 = c = this._inputPtr < this._inputEnd ? this._inputBuffer[this._inputPtr++] : this.getNextChar("expected a digit for number exponent", JsonToken.VALUE_NUMBER_FLOAT);
            }
            while (c <= '9' && c >= '0') {
                ++expLen;
                if (outPtr >= outBuf.length) {
        ...
        if (fractLen < 0 && expLen < 0) {
            return this.resetInt(neg, intLen);
        }
        return this.resetFloat(neg, intLen, fractLen, expLen);
    }
