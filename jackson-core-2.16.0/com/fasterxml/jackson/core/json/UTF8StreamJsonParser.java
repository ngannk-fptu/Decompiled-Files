...
extends ParserBase {
            ...
            if ((bits = b64variant.decodeBase64Char(ch = this._inputBuffer[this._inputPtr++] & 0xFF)) < 0) {
                if (bits != -2) {
                    if (ch == 34) {
                        ...
                        if (!b64variant.requiresPaddingOnRead()) break;
                        --this._inputPtr;
    ...
    public JsonToken nextToken() throws IOException {
        ...
        switch (i) {
            ...
            case 46: {
                t = this._parseFloatThatStartsWithPeriod(false, false);
                break;
            }
            case 48: 
            case 49: 
    ...
    private final JsonToken _nextTokenNotInObject(int i) throws IOException {
        ...
        switch (i) {
            ...
            case 43: {
                ...
                this._currToken = this._parseSignedNumber(false);
                return this._currToken;
            }
            case 46: {
            ...
            case 56: 
            case 57: {
                this._currToken = this._parseUnsignedNumber(i);
                return this._currToken;
            }
        }
        this._currToken = this._handleUnexpectedValue(i);
    ...
    private final void _isNextTokenNameYes(int i) throws IOException {
        ...
        switch (i) {
            ...
            case 43: {
                this._nextToken = this.isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature()) ? this._parseSignedNumber(false) : this._handleUnexpectedValue(i);
                return;
            }
            case 46: {
                this._nextToken = this._parseFloatThatStartsWithPeriod(false, false);
                return;
            }
            case 48: 
            case 49: 
    ...
    protected final JsonToken _parseFloatThatStartsWithPeriod(boolean neg, boolean hasSign) throws IOException {
        ...
        return this._parseFloat(outBuf, outPtr, 46, neg, 0);
    }
    ...
    private final JsonToken _parseSignedNumber(boolean negative) throws IOException {
        ...
        if ((c = this._inputBuffer[this._inputPtr++] & 0xFF) <= 48) {
            if (c != 48) {
                if (c == 46) {
                    return this._parseFloatThatStartsWithPeriod(negative, true);
                }
                return this._handleInvalidNumberStart(c, negative, true);
            }
            c = this._verifyNoLeadingZeroes();
        } else if (c > 57) {
            return this._handleInvalidNumberStart(c, negative, true);
        ...
        while (true) {
            if (this._inputPtr >= end) {
                return this._parseNumber2(outBuf, outPtr, negative, intLen);
            }
            if ((c = this._inputBuffer[this._inputPtr++] & 0xFF) < 48 || c > 57) break;
            ++intLen;
            outBuf[outPtr++] = (char)c;
        ...
        if (c == 46 || c == 101 || c == 69) {
            return this._parseFloat(outBuf, outPtr, c, negative, intLen);
        }
        --this._inputPtr;
        this._textBuffer.setCurrentLength(outPtr);
        if (this._parsingContext.inRoot()) {
    ...
    protected final String parseLongName(int q, int q2, int q3) throws IOException {
        ...
        while (this._inputPtr + 4 <= this._inputEnd) {
            ...
            if (qlen >= this._quadBuffer.length) {
                this._quadBuffer = this._growNameDecodeBuffer(this._quadBuffer, qlen);
            }
                ...
                if (ch > 127) {
                    if (currQuadBytes >= 4) {
                        if (qlen >= quads.length) {
                            quads = this._growNameDecodeBuffer(quads, quads.length);
    ...
    protected JsonToken _handleApos() throws IOException {
        ...
        block6: while (true) {
            ...
            while (true) {
                ...
                if ((c = inputBuffer[this._inputPtr++] & 0xFF) == 39) break block6;
                if (codes[c] != 0 && c != 34) break;
                outBuf[outPtr++] = (char)c;
            }
            switch (codes[c]) {
                case 1: {
    ...
    protected JsonToken _handleInvalidNumberStart(int ch, boolean neg) throws IOException {
        return this._handleInvalidNumberStart(ch, neg, false);
    }
    ...
    protected JsonToken _handleInvalidNumberStart(int ch, boolean neg, boolean hasSign) throws IOException {
        while (ch == 73) {
            ...
            this._reportError("Non-standard token '%s': enable `JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to allow", match);
        }
        if (!this.isEnabled(JsonReadFeature.ALLOW_LEADING_PLUS_SIGN_FOR_NUMBERS.mappedFeature()) && hasSign && !neg) {
    ...
    protected void _reportInvalidToken(String matchedPart, String msg) throws IOException {
        ...
        while ((this._inputPtr < this._inputEnd || this._loadMore()) && Character.isJavaIdentifierPart(c = (char)this._decodeCharForError(i = this._inputBuffer[this._inputPtr++]))) {
            sb.append(c);
            if (sb.length() < this._ioContext.errorReportConfiguration().getMaxErrorTokenLength()) continue;
            sb.append("...");
            break;
        }
    ...
    public JsonLocation getTokenLocation() {
        if (this._currToken == JsonToken.FIELD_NAME) {
            ...
            return new JsonLocation(this._contentReference(), total, -1L, this._nameStartRow, this._nameStartCol);
        }
    ...
    public JsonLocation getCurrentLocation() {
        ...
        return new JsonLocation(this._contentReference(), this._currInputProcessed + (long)this._inputPtr, -1L, this._currInputRow, col);
    }
