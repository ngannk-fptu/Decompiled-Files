...
extends ParserBase {
    ...
    protected JsonToken _parseUnsignedNumber(int c) throws IOException {
        ...
        if (c == 48) {
            ...
            if (c <= 57 && c >= 48) {
                ...
                if (c == 120 || c == 88) {
                    return this._handleInvalidNumberStart(c, false);
                }
                outBuf[0] = 48;
                outPtr = 1;
            }
    ...
    private final String _parseLongName(int q, int q2, int q3) throws IOException {
        ...
        while (true) {
            ...
            if (qlen >= this._quadBuffer.length) {
                this._quadBuffer = UTF8DataInputJsonParser.growArrayBy(this._quadBuffer, qlen);
            }
                ...
                if (ch > 127) {
                    if (currQuadBytes >= 4) {
                        if (qlen >= quads.length) {
                            quads = UTF8DataInputJsonParser.growArrayBy(quads, quads.length);
    ...
    protected String _handleOddName(int ch) throws IOException {
        ...
        do {
            ...
            if (qlen >= quads.length) {
                this._quadBuffer = quads = UTF8DataInputJsonParser.growArrayBy(quads, quads.length);
            }
            quads[qlen++] = currQuad;
            currQuad = ch;
            currQuadBytes = 1;
