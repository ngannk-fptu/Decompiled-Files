...
class JavaMembers {
    private static final boolean STRICT_REFLECTIVE_ACCESS = SourceVersion.latestSupported().ordinal() > 8;
                    ...
                    while (clazz != null) {
                        ...
                        try {
                            ...
                            for (Method method : methods = clazz.getDeclaredMethods()) {
                                ...
                                if (!Modifier.isPublic(n) && !Modifier.isProtected(n) && !includePrivate || map.containsKey(sig = new MethodSignature(method))) continue;
                            ...
                            for (GenericDeclaration genericDeclaration : interfaces2) {
                                this.discoverAccessibleMethods((Class<?>)genericDeclaration, map, includeProtected, includePrivate);
                            }
    ...
    private void reflect(Scriptable scope, boolean includeProtected, boolean includePrivate) {
        ...
        Object method;
        Method[] methods;
        for (Method method2 : methods = this.discoverAccessibleMethods(this.cl, includeProtected, includePrivate)) {
            ObjArray overloadedMethods;
            String name;
            int n = method2.getModifiers();
            boolean isStatic = Modifier.isStatic(n);
            Map<String, Object> ht = isStatic ? this.staticMembers : this.members;
        ...
        for (int tableCursor = 0; tableCursor != 2; ++tableCursor) {
            ...
            for (String name : ht.keySet()) {
                ...
                if (hashMap.containsKey(beanPropertyName) || (v = ht.get(beanPropertyName)) != null && (!includePrivate || !(v instanceof Member) || !Modifier.isPrivate(((Member)v).getModifiers()))) continue;
                MemberBox getter = null;
                getter = JavaMembers.findGetter(isStatic, ht, "get", nameComponent);
                if (getter == null) {
                ...
                hashMap.put(beanPropertyName, bp);
            }
            ht.putAll(hashMap);
        }
