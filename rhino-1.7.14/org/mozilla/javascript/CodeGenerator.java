...
extends Icode {
    ...
    private void addBigInt(BigInteger n) {
        int index = this.bigInts.get(n, -1);
        if (index == -1) {
            index = this.bigInts.size();
            this.bigInts.put(n, index);
        }
        if (index < 4) {
            this.addIcode(-67 - index);
        } else if (index <= 255) {
            this.addIcode(-71);
            this.addUint8(index);
        } else if (index <= 65535) {
