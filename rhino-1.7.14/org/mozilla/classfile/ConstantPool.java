...
final class ConstantPool {
    ...
    int addConstant(Object value) {
        ...
        if (value instanceof Boolean) {
            return this.addConstant((Boolean)value != false ? 1 : 0);
        }
    ...
    short addMethodHandle(ClassFileWriter.MHandle mh) {
        ...
        if (theIndex == -1) {
            short ref = mh.tag <= 4 ? this.addFieldRef(mh.owner, mh.name, mh.desc) : (mh.tag == 9 ? this.addInterfaceMethodRef(mh.owner, mh.name, mh.desc) : this.addMethodRef(mh.owner, mh.name, mh.desc));
            this.ensure(4);
