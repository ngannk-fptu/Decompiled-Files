...
public class ClassFileWriter {
    ...
    public void addInvokeDynamic(String methodName, String methodType, MHandle bsm, Object ... bsmArgs) {
        ...
        if ((bootstrapIndex = this.itsBootstrapMethods.indexOf(bsmEntry)) == -1) {
    ...
    public byte[] toByteArray() {
        ...
        offset = ClassFileWriter.putInt16(attributeCount, data, offset);
        if (this.itsBootstrapMethods != null) {
            offset = ClassFileWriter.putInt16(bootstrapMethodsAttrNameIndex, data, offset);
            offset = ClassFileWriter.putInt32(this.itsBootstrapMethodsLength + 2, data, offset);
            offset = ClassFileWriter.putInt16(this.itsBootstrapMethods.size(), data, offset);
            for (i = 0; i < this.itsBootstrapMethods.size(); ++i) {
                BootstrapEntry entry = (BootstrapEntry)this.itsBootstrapMethods.get(i);
    ...
    final class StackMapTable {
        ...
        private SuperBlock getSuperBlockFromOffset(int offset) {
            int startIdx = Arrays.binarySearch(ClassFileWriter.this.itsSuperBlockStarts, 0, ClassFileWriter.this.itsSuperBlockStartsTop, offset);
            if (startIdx < 0) {
                startIdx = -startIdx - 2;
            }
            if (startIdx < ClassFileWriter.this.itsSuperBlockStartsTop) {
                SuperBlock sb = this.superBlocks[startIdx];
        ...
        private void killSuperBlock(SuperBlock sb) {
            ...
            for (i = 0; i < ClassFileWriter.this.itsExceptionTableTop; ++i) {
                SuperBlock handlerSB;
                int handlerPC;
                ete = ClassFileWriter.this.itsExceptionTable[i];
                eteStart = ClassFileWriter.this.getLabelPC(ete.itsStartLabel);
                if (sbStart > eteStart && sbStart < ClassFileWriter.this.getLabelPC(ete.itsEndLabel)) {
                    handlerPC = ClassFileWriter.this.getLabelPC(ete.itsHandlerLabel);
                    handlerSB = this.getSuperBlockFromOffset(handlerPC);
                    locals = handlerSB.getLocals();
                    if (!handlerSB.isInitialized()) continue;
                    break;
                }
                if (eteStart <= sbStart || eteStart >= sb.getEnd() || !(handlerSB = this.getSuperBlockFromOffset(handlerPC = ClassFileWriter.this.getLabelPC(ete.itsHandlerLabel))).isInitialized()) continue;
                locals = handlerSB.getLocals();
                break;
            }
            for (i = 0; i < ClassFileWriter.this.itsExceptionTableTop; ++i) {
                ete = ClassFileWriter.this.itsExceptionTable[i];
                eteStart = ClassFileWriter.this.getLabelPC(ete.itsStartLabel);
                if (eteStart != sb.getStart() && ClassFileWriter.this.getLabelPC(ete.itsHandlerLabel) != sb.getStart()) continue;
        ...
        private void executeBlock(SuperBlock work) {
            ...
            int next = 0;
            int etStart = 0;
            int etEnd = ClassFileWriter.this.itsExceptionTableTop;
            if (ClassFileWriter.this.itsExceptionTableTop > 1) {
                ...
                for (int i = 0; i < ClassFileWriter.this.itsExceptionTableTop; ++i) {
                    ...
                    if (work.getEnd() < ClassFileWriter.this.getLabelPC(ete.itsStartLabel) || work.getStart() >= ClassFileWriter.this.getLabelPC(ete.itsEndLabel)) continue;
                ...
                case 185: {
                    ...
                    if (bc != 184 && ((tag = TypeInfo.getTag(instType = this.pop())) == TypeInfo.UNINITIALIZED_VARIABLE(0) || tag == 6)) {
                        if ("<init>".equals(methodName)) {
                            int newType = tag == TypeInfo.UNINITIALIZED_VARIABLE(0) ? TypeInfo.OBJECT(m.getClassName(), ClassFileWriter.this.itsConstantPool) : TypeInfo.OBJECT(ClassFileWriter.this.itsThisClassIndex);
