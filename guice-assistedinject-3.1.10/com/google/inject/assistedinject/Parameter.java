...
class Parameter {
    ...
    public Key<?> fixAnnotations(Key<?> key) {
        return key.getAnnotation() == null ? key : Key.get((TypeLiteral)key.getTypeLiteral(), key.getAnnotation().annotationType());
    }
    ...
    private Key<?> getBindingForType(Type type) {
        return this.bindingAnnotation != null ? Key.get((Type)type, (Annotation)this.bindingAnnotation) : Key.get((Type)type);
    }
    ...
    private Annotation getBindingAnnotation(Annotation[] annotations) {
        ...
        for (Annotation annotation : annotations) {
            ...
            Preconditions.checkArgument((bindingAnnotation == null ? 1 : 0) != 0, (String)"Parameter has multiple binding annotations: %s and %s", (Object[])new Object[]{bindingAnnotation, annotation});
