...
implements Evaluator {
    ...
    private void transform(ScriptNode tree) {
        ...
        if (optLevel > 0 && tree.getType() == 140) {
    ...
    private byte[] generateCode(String encodedSource) {
        boolean hasScript = this.scriptOrFnNodes[0].getType() == 140;
        ...
        if (hasScript) {
            ...
            Codegen.generateScriptCtor(cfw);
            this.generateMain(cfw);
            Codegen.generateExecute(cfw);
        }
    ...
    static boolean isGenerator(ScriptNode node) {
        return node.getType() == 113 && ((FunctionNode)node).isGenerator();
    }
    ...
    private void generateCallMethod(ClassFileWriter cfw, boolean isStrictMode) {
        ...
        for (int i = 0; i != end; ++i) {
            ...
            if (n.getType() == 113 && (ofn = OptFunctionNode.get(n)).isTargetOfDirectCall() && (pcount = ofn.fnode.getParamCount()) != 0) {
    ...
    private void generateMain(ClassFileWriter cfw) {
        ...
        cfw.add(89);
        cfw.addInvoke(183, cfw.getClassName(), "<init>", TEMPLATE_LITERAL_INIT_METHOD_SIGNATURE);
        cfw.add(42);
    ...
    private static void generateExecute(ClassFileWriter cfw) {
        ...
        cfw.addALoad(1);
        cfw.addInvoke(182, "org.mozilla.javascript.Context", "processMicrotasks", TEMPLATE_LITERAL_INIT_METHOD_SIGNATURE);
        cfw.add(176);
    ...
    private static void generateScriptCtor(ClassFileWriter cfw) {
        ...
        cfw.addLoadThis();
        cfw.addInvoke(183, SUPER_CLASS_NAME, "<init>", TEMPLATE_LITERAL_INIT_METHOD_SIGNATURE);
        cfw.addLoadThis();
        cfw.addPush(0);
    ...
    private void generateFunctionConstructor(ClassFileWriter cfw) {
        ...
        cfw.addALoad(2);
        cfw.addALoad(1);
        int start = this.scriptOrFnNodes[0].getType() == 140 ? 1 : 0;
        int end = this.scriptOrFnNodes.length;
        if (start == end) {
    ...
    String getBodyMethodSignature(ScriptNode n) {
        ...
        if (n.getType() == 113 && (ofn = OptFunctionNode.get(n)).isTargetOfDirectCall()) {
