...
class BodyCodegen {
    ...
    private void generatePrologue() {
        ...
        if (this.fnCurrent != null) {
            ...
            this.cfw.addPush(this.scriptOrFn.isInStrictMode());
            this.addScriptRuntimeInvoke(methodName, "(Lorg/mozilla/javascript/NativeFunction;Lorg/mozilla/javascript/Scriptable;[Ljava/lang/Object;Z)Lorg/mozilla/javascript/Scriptable;");
            this.cfw.addAStore(this.variableObjectLocal);
            this.cfw.addALoad(this.contextLocal);
            this.cfw.addALoad(this.variableObjectLocal);
    ...
    private void generateEpilogue() {
        ...
        if (this.isGenerator) {
            ...
            if (liveLocals != null) {
                ...
                boolean bl = false;
                while (var3_6 < nodes.size()) {
                    Node node = (Node)nodes.get((int)var3_6);
    ...
    private void generateStatement(Node node) {
        ...
        switch (type) {
            ...
            case 137: {
                ...
                if (child.getType() == 73 || child.getType() == 169) {
    ...
    private void generateExpression(Node node, Node parent) {
        ...
        block0 : switch (type) {
            ...
            case 113: {
                if (this.fnCurrent == null && parent.getType() == 140) break;
                int fnIndex = node.getExistingIntProp(1);
                OptFunctionNode ofn = OptFunctionNode.get(this.scriptOrFn, fnIndex);
                int t = ofn.fnode.getFunctionType();
                if (t != 2 && t != 4) {
    ...
    private void addLoadPropertyValues(Node node, Node child, int count) {
        if (this.isGenerator) {
            ...
            for (i = 0; i != count; ++i) {
                ...
                if (childType == 155 || childType == 156 || childType == 167) {
    ...
    private void visitObjectLiteral(Node node, Node child, boolean topLevel) {
        ...
        for (i = 0; i != count; ++i) {
            ...
            if (childType == 155 || childType == 156) {
    ...
    private static Node getFinallyAtTarget(Node node) {
        ...
        if (node.getType() == 135 && (fBlock = node.getNext()) != null && fBlock.getType() == 129) {
            return fBlock;
        }
    ...
    private void visitIfJumpEqOp(Node node, Node child, int trueGOTO, int falseGOTO) {
        ...
        if (child.getType() == 42 || rChild.getType() == 42) {
            ...
            if (child_dcp_register != -1 && rChild.getType() == 153 && (convertChild = rChild.getFirstChild()).getType() == 40) {
