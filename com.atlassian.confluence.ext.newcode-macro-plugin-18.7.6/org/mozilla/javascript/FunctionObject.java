...
extends BaseFunction {
        ...
        if (arity == 4 && (types[1].isArray() || types[2].isArray())) {
            ...
            if (arity > 0) {
                ...
                for (int i = 0; i != arity; ++i) {
                    ...
                    if (tag == 0) {
                        throw Context.reportRuntimeErrorById("msg.bad.parms", types[i].getName(), methodName);
                    }
    ...
    static Method findSingleMethod(Method[] methods, String name) {
        ...
        for (int i = 0; i != N; ++i) {
            ...
            if (found != null) {
                throw Context.reportRuntimeErrorById("msg.no.overload", name, method.getDeclaringClass().getName());
            }
            found = method;
        }
        return found;
        ...
        if (this.parmsLength < 0) {
            ...
            if (!this.isStatic) {
                ...
                if (!clazz.isInstance(thisObj)) {
                    ...
                    if ((thisObj == scope || thisObj instanceof ModuleScope) && scope != (parentScope = this.getParentScope()) && (compatible = clazz.isInstance(parentScope))) {
                        thisObj = parentScope;
                    }
                    if (!compatible) {
