    ...
    public Object execIdCall(IdFunctionObject f, Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
        if (f.hasTag(FTAG)) {
            ...
            switch (methodId) {
                ...
                case 5: {
                    return NativeGlobal.js_escape(args);
                }
                case 6: {
                    return NativeGlobal.js_eval(cx, scope, args);
                }
                case 7: {
                    if (args.length < 1) {
                ...
                case 12: {
                    return NativeGlobal.js_unescape(args);
                }
                case 13: {
        ...
        if (radix == -1) {
            ...
            if (len - start > 1 && s.charAt(start) == '0') {
                ...
                if (c == 'x' || c == 'X') {
                    radix = 16;
                    start += 2;
                } else if ('0' <= c && c <= '9' && ((cx = Context.getCurrentContext()) == null || cx.getLanguageVersion() < 150)) {
                    radix = 8;
                    ++start;
                }
            }
