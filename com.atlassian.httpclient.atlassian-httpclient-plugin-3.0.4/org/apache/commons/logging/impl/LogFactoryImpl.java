...
extends LogFactory {
    ...
    protected Log newInstance(String name) throws LogConfigurationException {
        try {
            ...
            LogFactoryImpl.handleThrowable(t);
    ...
    private Log createLogFromClass(String logAdapterClassName, String logCategory, boolean affectState) throws LogConfigurationException {
        ...
        while (true) {
            ...
            try {
                ...
                try {
                    ...
                    this.logDiagnostic("The log adapter '" + logAdapterClassName + "' is not available via classloader " + LogFactoryImpl.objectId(currentCL) + ": " + msg2.trim());
                    try {
        ...
        if (logAdapterClass != null && affectState) {
            ...
            this.logDiagnostic("Log adapter '" + logAdapterClassName + "' from classloader " + LogFactoryImpl.objectId(logAdapterClass.getClassLoader()) + " has been selected for use.");
        }
        return logAdapter;
    }
        ...
        if (implementsLog) {
            if (LogFactoryImpl.isDiagnosticsEnabled()) {
                try {
                    ...
                    this.logDiagnostic("Class '" + badClass.getName() + "' was found in classloader " + LogFactoryImpl.objectId(badClassLoader) + ". It is bound to a Log interface which is not" + " the one loaded from classloader " + LogFactoryImpl.objectId(logInterfaceClassLoader));
                }
                catch (Throwable t) {
                    LogFactoryImpl.handleThrowable(t);
