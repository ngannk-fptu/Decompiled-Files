...
public class ContinueIfNotFoundValidator
implements QueryParamValidator {
    @VisibleForTesting
    public static final String BAD_REQUEST_INVALID_CONTINUE_IF_NOT_FOUND_KEY = "data-pipeline.api.rest.queryparam.continueifnotfound.invalid.should.be.boolean";
    ...
    public void validate(@Nonnull String fieldValue, @Nonnull ValidationResult validationResult) {
        if (!"true".equals(fieldValue) && !"false".equals(fieldValue)) {
            validationResult.add(BAD_REQUEST_INVALID_CONTINUE_IF_NOT_FOUND_KEY);
        }
    }
}
