...
public class PluginLicenseFactory
extends AbstractProductLicenseFactory {
    private final Product product;

    public PluginLicenseFactory(Product product) {
        this.product = product;
    }

    @Override
    public ProductLicense getLicenseInternal(Product product, LicenseProperties licenseProperties) {
        if (this.product.equals((Object)product)) {
            return new DefaultPluginLicense(product, licenseProperties);
        }
