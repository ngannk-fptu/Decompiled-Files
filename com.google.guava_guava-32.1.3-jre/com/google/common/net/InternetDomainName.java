...
public final class InternetDomainName {
    ...
    private int findSuffixOfType(Optional<PublicSuffixType> desiredType) {
        ...
        for (int i = 0; i < partsSize; ++i) {
            ...
            if (i > 0 && InternetDomainName.matchesType(desiredType, Optional.fromNullable(PublicSuffixPatterns.UNDER.get(ancestorName)))) {
                return i - 1;
            }
            if (InternetDomainName.matchesType(desiredType, Optional.fromNullable(PublicSuffixPatterns.EXACT.get(ancestorName)))) {
                return i;
            }
            if (!PublicSuffixPatterns.EXCLUDED.containsKey(ancestorName)) continue;
            return i + 1;
        }
        return -1;
    }
