...
final class TopKSelector<T> {
    ...
    private TopKSelector(Comparator<? super T> comparator, int k) {
        ...
        this.buffer = new Object[IntMath.checkedMultiply(k, 2)];
        this.bufferSize = 0;
        this.threshold = null;
    }
    ...
    private int partition(int left, int right, int pivotIndex) {
        ...
        for (int i = left; i < right; ++i) {
            if (this.comparator.compare(NullnessCasts.uncheckedCastNullableTToT(this.buffer[i]), pivotValue) >= 0) continue;
