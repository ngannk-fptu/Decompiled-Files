...
extends AbstractQueue<E> {
    ...
    private class Heap {
        ...
        int bubbleUpAlternatingLevels(int index, E x) {
            ...
            ((MinMaxPriorityQueue)MinMaxPriorityQueue.this).queue[index] = x;
            return index;
        }
        ...
        int crossOverUp(int index, E x) {
            ...
            if (parentIndex != 0 && (auntIndex = this.getRightChildIndex(grandparentIndex = this.getParentIndex(parentIndex))) != parentIndex && this.getLeftChildIndex(auntIndex) >= MinMaxPriorityQueue.this.size && this.ordering.compare(auntElement = MinMaxPriorityQueue.this.elementData(auntIndex), parentElement) < 0) {
        ...
        int swapWithConceptuallyLastElement(E actualLastElement) {
            ...
            if (parentIndex != 0 && (auntIndex = this.getRightChildIndex(grandparentIndex = this.getParentIndex(parentIndex))) != parentIndex && this.getLeftChildIndex(auntIndex) >= MinMaxPriorityQueue.this.size && this.ordering.compare(auntElement = MinMaxPriorityQueue.this.elementData(auntIndex), actualLastElement) < 0) {
                ((MinMaxPriorityQueue)MinMaxPriorityQueue.this).queue[auntIndex] = actualLastElement;
                ((MinMaxPriorityQueue)MinMaxPriorityQueue.this).queue[((MinMaxPriorityQueue)MinMaxPriorityQueue.this).size] = auntElement;
                return auntIndex;
            }
