...
implements ConcurrentMap<K, V> {
    ...
    public boolean isEmpty() {
        ...
        long sum = 0L;
        for (Segment<K, V> segment : segments = this.segments) {
            if (segment.count != 0) {
                return false;
            }
    ...
    @Override
    @CheckForNull
    public V compute(K key, BiFunction<? super K, ? super @Nullable V, ? extends @Nullable V> function) {
        Preconditions.checkNotNull(key);
    ...
    public V computeIfAbsent(K key, Function<? super K, ? extends V> function) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k, oldValue) -> oldValue == null ? function.apply((Object)key) : oldValue);
    }

    @Override
    @CheckForNull
    public V computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends @Nullable V> function) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k, oldValue) -> oldValue == null ? null : function.apply((Object)k, (Object)oldValue));
    }

    @Override
    @CheckForNull
    public V merge(K key, V newValue, BiFunction<? super V, ? super V, ? extends @Nullable V> function) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(newValue);
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k, oldValue) -> oldValue == null ? newValue : function.apply((Object)oldValue, (Object)newValue));
    }
    ...
    implements Serializable {
        ...
        CacheBuilder<K, V> recreateCacheBuilder() {
            ...
            if (this.weigher != CacheBuilder.OneWeigher.INSTANCE) {
                CacheBuilder<K, V> unused = builder.weigher(this.weigher);
    ...
    extends AbstractCacheSet<Map.Entry<K, V>> {
        ...
        public boolean removeIf(Predicate<? super Map.Entry<K, V>> filter) {
            Preconditions.checkNotNull(filter);
            return LocalCache.this.removeIf((? super K k, ? super V v) -> filter.test(Maps.immutableEntry(k, v)));
        }
    ...
    extends AbstractCollection<V> {
        ...
        public boolean removeIf(Predicate<? super V> filter) {
            Preconditions.checkNotNull(filter);
            return LocalCache.this.removeIf((? super K k, ? super V v) -> filter.test((Object)v));
        }
    ...
    implements ValueReference<K, V> {
        ...
        public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
            try {
                ...
                return Futures.transform(newValue, newResult -> {
                    this.set(newResult);
                    return newResult;
    ...
    extends ReentrantLock {
         ...
         */
        @CheckForNull
        V compute(K key, int hash, BiFunction<? super K, ? super @Nullable V, ? extends @Nullable V> function) {
            ValueReference<K, V> valueReference = null;
            ComputingValueReference<? super K, Object> computingValueReference = null;
            boolean createNewEntry = true;
            this.lock();
            try {
                V v;
                ...
                computingValueReference = new ComputingValueReference<K, Object>(valueReference);
                if (e == null) {
                    createNewEntry = true;
                    e = this.newEntry(key, hash, first);
                ...
                if (newValue != null) {
                    ...
                    entryKey = this.getAndRecordStats(key, hash, computingValueReference, Futures.immediateFuture(newValue));
                    return (V)entryKey;
                }
    ...
    static interface ValueReference<K, V> {
        ...
        public ValueReference<K, V> copyFor(ReferenceQueue<V> var1, @CheckForNull V var2, ReferenceEntry<K, V> var3);

        public void notifyNewValue(@CheckForNull V var1);
    ...
    static enum EntryFactory {
        ...
        abstract <K, V> ReferenceEntry<K, V> newEntry(Segment<K, V> var1, K var2, int var3, @CheckForNull ReferenceEntry<K, V> var4);
