...
final class BulkOperationPacked3
extends BulkOperationPacked {
    ...
    public void decode(byte[] blocks, int blocksOffset, long[] values, int valuesOffset, int iterations) {
        for (int i = 0; i < iterations; ++i) {
            ...
            values[valuesOffset++] = byte0 >>> 5;
            values[valuesOffset++] = byte0 >>> 2 & 7L;
            long byte1 = blocks[blocksOffset++] & 0xFF;
            values[valuesOffset++] = (byte0 & 3L) << 1 | byte1 >>> 7;
            values[valuesOffset++] = byte1 >>> 4 & 7L;
            values[valuesOffset++] = byte1 >>> 1 & 7L;
            long byte2 = blocks[blocksOffset++] & 0xFF;
            values[valuesOffset++] = (byte1 & 1L) << 2 | byte2 >>> 6;
            values[valuesOffset++] = byte2 >>> 3 & 7L;
            values[valuesOffset++] = byte2 & 7L;
        }
    }
}
