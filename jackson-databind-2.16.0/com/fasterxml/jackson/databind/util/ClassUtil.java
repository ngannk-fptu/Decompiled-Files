...
public final class ClassUtil {
    ...
    public static <T> T throwAsMappingException(DeserializationContext ctxt, IOException e0) throws JsonMappingException {
        if (e0 instanceof JsonMappingException) {
            throw (JsonMappingException)((Object)e0);
        }
    ...
    public static void checkAndFixAccess(Member member, boolean evenIfAlreadyPublic) {
        ...
        try {
            ...
            boolean bl = isPublic = Modifier.isPublic(member.getModifiers()) && Modifier.isPublic(declaringClass.getModifiers());
            ...
            if ("InaccessibleObjectException".equals(se.getClass().getSimpleName())) {
                throw new IllegalArgumentException(String.format("Failed to call `setAccess()` on %s '%s' (of class %s) due to `%s`, problem: %s", member.getClass().getSimpleName(), member.getName(), ClassUtil.nameOf(member.getDeclaringClass()), se.getClass().getName(), se.getMessage()), se);
            }
            throw se;
        }
    }
    ...
    private static class EnumTypeLocator {
        ...
        private static Field locateField(Class<?> fromClass, String expectedName, Class<?> type) throws Exception {
            ...
            throw new IllegalStateException(String.format("No field named '%s' in class '%s'", expectedName, fromClass.getName()));
        }
    }
}
