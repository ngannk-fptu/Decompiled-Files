...
extends StdScalarDeserializer<T> {
    ...
    public T deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
        ...
        if (this._shouldTrim()) {
            String old = text;
            if ((text = text.trim()) != old && text.isEmpty()) {
                return (T)this._deserializeFromEmptyString(ctxt);
            }
        }
    ...
    extends FromStringDeserializer<Object> {
        ...
        private Locale _deSerializeBCP47Locale(String value, int ix, String first, String second, int extMarkerIx) {
            ...
            try {
                ...
                if ((underscoreIx = (value = value.substring(extMarkerIx + 2)).indexOf(95)) < 0) {
                    ...
                    return new Locale.Builder().setLanguage(first).setRegion(second).setVariant(third).setExtension(value.charAt(0), value.substring(hyphenIx + 1)).build();
                }
