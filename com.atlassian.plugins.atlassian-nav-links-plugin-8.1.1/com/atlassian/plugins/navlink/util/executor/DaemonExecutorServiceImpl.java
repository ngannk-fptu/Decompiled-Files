...
DisposableBean {
    ...
    public DaemonExecutorServiceImpl(ThreadLocalDelegateExecutorFactory delegateExecutorFactory) {
        ThreadFactory factory = ThreadFactories.namedThreadFactory((String)"Navlink Plugin Executor", (ThreadFactories.Type)ThreadFactories.Type.DAEMON, (int)THREAD_POOL_PRIORITY);
        ScheduledThreadPoolExecutor delegate = new ScheduledThreadPoolExecutor(THREAD_POOL_SIZE, factory);
        delegate.setKeepAliveTime(60L, TimeUnit.SECONDS);
    ...
    @Override
    public <O, I extends Collection<O>> List<O> invokeAllAndGet(Collection<? extends Callable<I>> callables, long timeout, TimeUnit unit) throws ExecutionException, InterruptedException {
        ArrayList builder = new ArrayList();
        List futures = this.executor.invokeAll(callables, timeout, unit);
        for (Future future : futures) {
            if (future.isCancelled()) continue;
            builder.addAll((Collection)future.get());
        }
        return builder;
    }
