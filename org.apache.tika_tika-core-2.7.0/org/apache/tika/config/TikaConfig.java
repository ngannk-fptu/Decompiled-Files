...
public class TikaConfig {
    ...
    private TikaConfig(Element element, ServiceLoader loader) throws TikaException, IOException {
        ...
        this.renderer = (Renderer)rendererXmlLoader.loadOverall(element, this.mimeTypes, loader);
        ParserXmlLoader parserLoader = new ParserXmlLoader(this.encodingDetector, this.renderer);
    ...
    public TikaConfig(ClassLoader loader) throws MimeTypeException, IOException {
        ...
        this.renderer = TikaConfig.getDefaultRenderer(this.serviceLoader);
        this.parser = TikaConfig.getDefaultParser(this.mimeTypes, this.serviceLoader, this.encodingDetector, this.renderer);
    ...
    public TikaConfig() throws TikaException, IOException {
        ...
        if ((config == null || config.trim().equals("")) && !StringUtils.isBlank(config = System.getenv("TIKA_CONFIG"))) {
        ...
        if (config == null || config.trim().equals("")) {
            ...
            try (InputStream stream = TikaConfig.getConfigInputStream(config, tmpServiceLoader);){
                ...
                this.renderer = (Renderer)rendererLoader.loadOverall(element, this.mimeTypes, this.serviceLoader);
                ParserXmlLoader parserLoader = new ParserXmlLoader(this.encodingDetector, this.renderer);
    ...
    private static CompositeParser getDefaultParser(MimeTypes types, ServiceLoader loader, EncodingDetector encodingDetector, Renderer renderer) {
    ...
    extends XmlLoader<Renderer, Renderer> {
        ...
        @Override
        Renderer preLoadOne(Class<? extends Renderer> loadedClass, String classname, MimeTypes mimeTypes) throws TikaException {
            return null;
        }

        @Override
        Renderer createDefault(MimeTypes mimeTypes, ServiceLoader loader) {
            return TikaConfig.getDefaultRenderer(loader);
        }

        @Override
        Renderer createComposite(List<Renderer> renderers, MimeTypes mimeTypes, ServiceLoader loader) {
            return new CompositeRenderer(renderers);
        }

        @Override
        Renderer createComposite(Class<? extends Renderer> rendererClass, List<Renderer> childRenderers, Set<Class<? extends Renderer>> excludeRenderers, Map<String, Param> params, MimeTypes mimeTypes, ServiceLoader loader) throws InvocationTargetException, IllegalAccessException, InstantiationException {
            ...
            if (renderer == null) {
                try {
                    ...
                    LOG.debug("couldn't find constructor for service loader + collection for {}", renderer);
                }
            }
            if (renderer == null) {
                try {
        ...
        EncodingDetector createComposite(Class<? extends EncodingDetector> encodingDetectorClass, List<EncodingDetector> childEncodingDetectors, Set<Class<? extends EncodingDetector>> excludeDetectors, Map<String, Param> params, MimeTypes mimeTypes, ServiceLoader loader) throws InvocationTargetException, IllegalAccessException, InstantiationException {
            ...
            if (encodingDetector == null) {
                try {
                    ...
                    LOG.debug("couldn't find constructor for service loader + collection for {}", encodingDetectorClass);
                }
            }
            if (encodingDetector == null) {
                try {
                    ...
                    LOG.debug("couldn't find constructor for EncodingDetector(List) for {}", encodingDetectorClass);
                }
            }
            return encodingDetector;
        }
    ...
    extends XmlLoader<CompositeParser, Parser> {
        ...
        CompositeParser createDefault(MimeTypes mimeTypes, ServiceLoader loader) {
            return TikaConfig.getDefaultParser(mimeTypes, loader, this.encodingDetector, this.renderer);
        }
        ...
        Parser createComposite(Class<? extends Parser> parserClass, List<Parser> childParsers, Set<Class<? extends Parser>> excludeParsers, Map<String, Param> params, MimeTypes mimeTypes, ServiceLoader loader) throws InvocationTargetException, IllegalAccessException, InstantiationException {
            ...
            if (parser == null) {
                try {
                    c = parserClass.getConstructor(MediaTypeRegistry.class, ServiceLoader.class, Collection.class, EncodingDetector.class, Renderer.class);
                    parser = c.newInstance(registry, loader, excludeParsers, this.encodingDetector, this.renderer);
                }
                catch (NoSuchMethodException noSuchMethodException) {
                    // empty catch block
                }
        ...
        Parser newInstance(Class<? extends Parser> loadedClass) throws IllegalAccessException, InstantiationException, NoSuchMethodException, InvocationTargetException {
            ...
            if (AbstractEncodingDetectorParser.class.isAssignableFrom(loadedClass)) {
                ...
                parser = ctor.newInstance(this.encodingDetector);
            } else {
                parser = loadedClass.newInstance();
            }
            if (parser instanceof RenderingParser) {
                ((RenderingParser)((Object)parser)).setRenderer(this.renderer);
            }
            return parser;
        }
        ...
        T loadOne(Element element, MimeTypes mimeTypes, ServiceLoader loader) throws TikaException, IOException {
            ...
            try {
                ...
                if (loader.getLoadErrorHandler() == LoadErrorHandler.THROW) {
                    throw new TikaConfigException("Unable to find a " + this.getLoaderTagName() + " class: " + name, e);
                }
