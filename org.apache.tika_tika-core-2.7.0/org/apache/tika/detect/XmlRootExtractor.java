...
public class XmlRootExtractor {
    ...
    public QName extractRootElement(byte[] data) {
        while (true) {
            try {
                return this.extractRootElement((InputStream)new UnsynchronizedByteArrayInputStream(data), true);
            }
    ...
    private QName extractRootElement(InputStream stream, boolean throwMalformed) {
        ...
        block3: {
            ...
            try {
                XMLReaderUtils.parseSAX((InputStream)new CloseShieldInputStream(stream), (ContentHandler)handler, EMPTY_CONTEXT);
            }
