...
public enum PGProperty {
    ADAPTIVE_FETCH("adaptiveFetch", "false", "Specifies if number of rows fetched in ResultSet should be adaptive to maxResultBuffer and max row size."),
    ADAPTIVE_FETCH_MAXIMUM("adaptiveFetchMaximum", "-1", "Specifies maximum number of rows used by adaptive fetch."),
    ADAPTIVE_FETCH_MINIMUM("adaptiveFetchMinimum", "0", "Specifies minimum number of rows used by adaptive fetch."),
    ...
    AUTHENTICATION_PLUGIN_CLASS_NAME("authenticationPluginClassName", null, "Name of class which implements AuthenticationPlugin"),
    ...
    CONNECT_TIMEOUT("connectTimeout", "10", "The timeout value in seconds used for socket connect operations."),
    ...
    GROUP_STARTUP_PARAMETERS("groupStartupParameters", "false", "This is important in pool-by-transaction scenarios in order to make sure that all the statements reaches the same connection that is being initialized."),
    ...
    GSS_RESPONSE_TIMEOUT("gssResponseTimeout", "5000", "Time in milliseconds we wait for a response from the server after requesting a GSS upgrade"),
    ...
    JAAS_APPLICATION_NAME("jaasApplicationName", "pgjdbc", "Specifies the name of the JAAS system or application login configuration."),
    ...
    LOCAL_SOCKET_ADDRESS("localSocketAddress", null, "Local Socket address, if set bind the client side of the socket to this address"),
    ...
    LOGIN_TIMEOUT("loginTimeout", "0", "Specify how long in seconds to wait for establishment of a database connection."),
    ...
    PG_HOST("PGHOST", "localhost", "Hostname of the PostgreSQL server (may be specified directly in the JDBC URL)", false),
    PG_PORT("PGPORT", "5432", "Port of the PostgreSQL server (may be specified directly in the JDBC URL)"),
    ...
    QUOTE_RETURNING_IDENTIFIERS("quoteReturningIdentifiers", "true", "Quote identifiers provided in returning array", false),
    ...
    SERVICE("service", null, "Service name to be searched in pg_service.conf resource"),
    ...
    SOCKET_TIMEOUT("socketTimeout", "0", "The timeout value in seconds max(2147484) used for socket read operations."),
    ...
    SSL_FACTORY("sslfactory", "org.postgresql.ssl.LibPQFactory", "Provide a SSLSocketFactory class when using SSL."),
    ...
    SSL_PASSWORD_CALLBACK("sslpasswordcallback", null, "A class, implementing javax.security.auth.callback.CallbackHandler that can handle PasswordCallback for the ssl password."),
    SSL_RESPONSE_TIMEOUT("sslResponseTimeout", "5000", "Time in milliseconds we wait for a response from the server after requesting SSL upgrade"),
    ...
    TARGET_SERVER_TYPE("targetServerType", "any", "Specifies what kind of server to connect", false, new String[]{"any", "primary", "master", "slave", "secondary", "preferSlave", "preferSecondary", "preferPrimary"}),
    TCP_KEEP_ALIVE("tcpKeepAlive", "false", "Enable or disable TCP keep-alive. The default is {@code false}."),
    TCP_NO_DELAY("tcpNoDelay", "true", "Enable or disable TCP no delay. The default is (@code true}."),
    ...
    private PGProperty(String name, String defaultValue, @Nullable String description, boolean required, String[] choices) {
        this.name = name;
    ...
    public @Nullable String get(Properties properties) {
        return this.getOrDefault(properties);
    }
    ...
    public boolean getBoolean(Properties properties) {
        return Boolean.parseBoolean(this.getOrDefault(properties));
    }

    public int getIntNoCheck(Properties properties) {
        String value = this.getOrDefault(properties);
    ...
    public DriverPropertyInfo toDriverPropertyInfo(Properties properties) {
        DriverPropertyInfo propertyInfo = new DriverPropertyInfo(this.name, this.getOrDefault(properties));
