...
implements Freezable<GlobalizationPreferences> {
    ...
    public GlobalizationPreferences setLocales(String acceptLanguageString) {
        ...
        this.locales = this.processLocales(new ArrayList<ULocale>(acceptSet));
        return this;
    }
        ...
        block11: for (ULocale locale : this.getLocales()) {
            ...
            switch (type) {
                ...
                case 8: {
                    ...
                    result = temp.getName(locale, type == 7 ? 1 : 0, null);
                    break;
                }
                case 9: {
    ...
    protected List<ULocale> processLocales(List<ULocale> inputLocales) {
        ...
        for (ULocale uloc2 : inputLocales) {
            String language = uloc2.getLanguage();
            String script = uloc2.getScript();
            String country = uloc2.getCountry();
            String variant = uloc2.getVariant();
            boolean bInserted = false;
            for (int j = 0; j < result.size(); ++j) {
                ULocale u = (ULocale)result.get(j);
                if (!u.getLanguage().equals(language)) continue;
                ...
                if (!s.equals(script)) {
                    ...
                    if (script.length() != 0 || country.length() <= 0 || c.length() != 0) continue;
                    result.add(j, uloc2);
                    bInserted = true;
                    break;
                }
                ...
                if (v.equals(variant) || v.length() != 0) continue;
                result.add(j, uloc2);
                bInserted = true;
                break;
            }
        ...
        for (index = 0; index < result.size(); ++index) {
            ...
            while ((uloc2 = uloc2.getFallback()) != null && uloc2.getLanguage().length() != 0) {
        ...
        while (index < result.size() - 1) {
            ...
            for (int i = index + 1; i < result.size(); ++i) {
                if (!uloc2.equals(result.get(i))) continue;
                result.remove(index);
                bRemoved = true;
                break;
            }
    ...
    private ULocale getAvailableLocale(int type) {
        ...
        for (int i = 0; i < locs.size(); ++i) {
            ...
            if (!this.isAvailableLocale(l, type)) continue;
            result = l;
            break;
        }
        return result;
    ...
    static {
        ...
        for (int i2 = 0; i2 < allLocales.length; ++i2) {
            bits = new BitSet(6);
            available_locales.put(allLocales[i2], bits);
            bits.set(0);
        }
        ULocale[] calLocales = Calendar.getAvailableULocales();
        for (int i3 = 0; i3 < calLocales.length; ++i3) {
            bits = available_locales.get(calLocales[i3]);
            if (bits == null) {
                bits = new BitSet(6);
        ...
        for (int i4 = 0; i4 < dateLocales.length; ++i4) {
            bits = available_locales.get(dateLocales[i4]);
            if (bits == null) {
                bits = new BitSet(6);
        ...
        for (int i5 = 0; i5 < numLocales.length; ++i5) {
            bits = available_locales.get(numLocales[i5]);
            if (bits == null) {
                bits = new BitSet(6);
        ...
        for (int i6 = 0; i6 < collLocales.length; ++i6) {
            bits = available_locales.get(collLocales[i6]);
            if (bits == null) {
                bits = new BitSet(6);
        ...
        for (int i7 = 0; i7 < brkLocales.length; ++i7) {
            bits = available_locales.get(brkLocales[i7]);
            bits.set(5);
        }
