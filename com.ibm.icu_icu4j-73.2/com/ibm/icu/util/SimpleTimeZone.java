...
extends BasicTimeZone {
    ...
    public void getOffsetFromLocal(long date, BasicTimeZone.LocalOption nonExistingTimeOpt, BasicTimeZone.LocalOption duplicatedTimeOpt, int[] offsets) {
        int nonExistingTimeOptVal = SimpleTimeZone.getLocalOptionValue(nonExistingTimeOpt);
        int duplicatedTimeOptVal = SimpleTimeZone.getLocalOptionValue(duplicatedTimeOpt);
        ...
        if (offsets[1] > 0) {
            if ((nonExistingTimeOptVal & 3) == 1 || (nonExistingTimeOptVal & 3) != 3 && (nonExistingTimeOptVal & 0xC) != 12) {
                date -= (long)this.getDSTSavings();
                recalc = true;
            }
        } else if ((duplicatedTimeOptVal & 3) == 3 || (duplicatedTimeOptVal & 3) != 1 && (duplicatedTimeOptVal & 0xC) == 4) {
            date -= (long)this.getDSTSavings();
            recalc = true;
        }
        if (recalc) {
