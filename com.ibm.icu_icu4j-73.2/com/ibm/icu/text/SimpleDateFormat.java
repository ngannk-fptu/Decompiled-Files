...
extends DateFormat {
    ...
    private static synchronized String getDefaultPattern() {
        ...
        if (!defaultLocale.equals(cachedDefaultLocale)) {
            ...
            try {
                ICUResourceBundle rb = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt73b", cachedDefaultLocale);
    ...
    public StringBuffer format(Calendar cal, StringBuffer toAppendTo, FieldPosition pos) {
        return this.format(cal, toAppendTo, pos, null);
    }
    ...
    protected void subFormat(StringBuffer buf, char ch, int count, int beginOffset, int fieldNum, DisplayContext capitalizationContext, FieldPosition pos, char patternCharToOutput, Calendar cal) {
        ...
        switch (patternCharIndex) {
            ...
            case 27: {
                if (count >= 5) {
                    SimpleDateFormat.safeAppend(this.formatData.narrowQuarters, value / 3, buf);
                    break;
                }
                if (count == 4) {
            ...
            case 28: {
                if (count >= 5) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneNarrowQuarters, value / 3, buf);
                    break;
                }
                if (count == 4) {
        ...
        if (fieldNum == 0 && capitalizationContext != null && buf.length() > bufstart && UCharacter.isLowerCase(buf.codePointAt(bufstart))) {
            ...
            if (titlecase) {
                ...
                String firstFieldTitleCase = UCharacter.toTitleCase(this.locale, firstField, mutableCapitalizationBrkIter, 768);
                buf.replace(bufstart, buf.length(), firstFieldTitleCase);
            }
        }
        int outputCharIndex = SimpleDateFormat.getIndexFromChar(patternCharToOutput);
    ...
    public void parse(String text, Calendar cal, ParsePosition parsePos) {
        ...
        block55: {
                ...
                block59: {
                    block57: {
                        ...
                        block60: {
                            PatternItem field;
                            block58: {
                                if (!(items[i] instanceof PatternItem)) break block57;
                                ...
                                if (numericFieldStart == -1) break block58;
                            ...
                            if (field.type == 'l') break block59;
                            numericFieldStart = -1;
                            s = pos;
                            if ((pos = this.subParse(text, pos, field.type, field.length, false, true, ambiguousYear, cal, numericLeapMonthFormatter, tzTimeType, dayPeriod)) >= 0) break block59;
                            if (pos != -32000) break block60;
                            pos = s;
                            if (i + 1 < items.length) {
                                char pch;
                                int idx;
                                ...
                                for (idx = 0; idx < plen && (PatternProps.isWhiteSpace(pch = patl.charAt(idx)) || UCharacter.isUWhiteSpace(pch)); ++idx) {
                                }
                                if (idx == plen) {
                                    ++i;
                                }
            ...
            try {
                ...
                if (!ambiguousYear[0] && tztype == TimeZoneFormat.TimeType.UNKNOWN) break block55;
                ...
                if (tztype == TimeZoneFormat.TimeType.UNKNOWN) break block55;
                ...
                if (tztype == TimeZoneFormat.TimeType.STANDARD) {
                    ...
                    if (btz == null) {
                        ...
                        long baseTime;
                        long time = baseTime = localMillis + (long)offsets[0];
                        long limit = baseTime + 946080000000L;
                        TimeZoneTransition trs = null;
                        while (time < limit && (trs = btz.getNextTransition(time, false)) != null && (resolvedSavings = trs.getTo().getDSTSavings()) == 0) {
                            time = trs.getTime();
                        }
                        if (resolvedSavings == 0) {
                            time = baseTime;
                            limit = baseTime - 946080000000L;
                            while (time > limit && (trs = btz.getPreviousTransition(time, true)) != null && (resolvedSavings = trs.getFrom().getDSTSavings()) == 0) {
                                time = trs.getTime() - 1L;
                            }
                            if (resolvedSavings == 0) {
    ...
    private int matchAlphaMonthStrings(String text, int start, String[] wideData, String[] shortData, Calendar cal) {
        ...
        for (i = 0; i < wideData.length; ++i) {
            length = wideData[i].length();
            matchLength = 0;
            if (length <= bestMatchLength || (matchLength = this.regionMatchesWithOptionalDot(text, start, wideData[i], length)) < 0) continue;
            bestMatch = i;
            bestMatchLength = matchLength;
        }
        for (i = 0; i < shortData.length; ++i) {
            length = shortData[i].length();
            matchLength = 0;
            if (length <= bestMatchLength || (matchLength = this.regionMatchesWithOptionalDot(text, start, shortData[i], length)) < 0) continue;
            bestMatch = i;
            bestMatchLength = matchLength;
        }
        if (bestMatch >= 0) {
        ...
        switch (patternCharIndex) {
            ...
            case 26: {
                ...
                if (this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) && count >= 3 && count <= 4 && !haveMonthPat) {
                    int n = newStart = patternCharIndex == 2 ? this.matchAlphaMonthStrings(text, start, this.formatData.months, this.formatData.shortMonths, cal) : this.matchAlphaMonthStrings(text, start, this.formatData.standaloneMonths, this.formatData.standaloneShortMonths, cal);
                    if (newStart > 0) {
                        return newStart;
                    }
                }
