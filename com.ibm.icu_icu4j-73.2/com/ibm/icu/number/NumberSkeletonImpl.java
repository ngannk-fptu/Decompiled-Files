...
class NumberSkeletonImpl {
    ...
    private static ParseState parseStem(StringSegment segment, CharsTrie stemTrie, MacroProps macros) {
        ...
        switch (stem) {
            ...
            case STEM_UNIT_USAGE: {
                NumberSkeletonImpl.checkNull(macros.usage, segment);
    ...
    private static ParseState parseOption(ParseState stem, StringSegment segment, MacroProps macros) {
        ...
        switch (stem) {
            case STATE_PRECISION: {
                if (!BlueprintHelpers.parseTrailingZeroOption(segment, macros)) break;
                return ParseState.STATE_NULL;
            }
        }
    ...
    static final class GeneratorHelpers {
        ...
        private static boolean unit(MacroProps macros, StringBuilder sb) {
            ...
            if (unit.equals(MeasureUnit.PERCENT)) {
                sb.append("percent");
                return true;
            }
            if (unit.equals(MeasureUnit.PERMILLE)) {
                sb.append("permille");
                return true;
            }
            sb.append("unit/");
    ...
    static final class BlueprintHelpers {
        ...
        private static void parseFractionStem(StringSegment segment, MacroProps macros) {
            ...
            macros.precision = maxFrac == -1 ? (minFrac == 0 ? Precision.unlimited() : Precision.minFraction(minFrac)) : Precision.minMaxFraction(minFrac, maxFrac);
        }
                    ...
                    block10: {
                        block9: {
                            block7: {
                                ...
                                offset = 1;
                                if (segment.length() == offset) break block6;
                                isEngineering = false;
                                if (segment.charAt(offset) != 'E') break block7;
                                isEngineering = true;
                                if (segment.length() == ++offset) break block6;
                            }
                            signDisplay = NumberFormatter.SignDisplay.AUTO;
                            if (segment.charAt(offset) != '+') break block8;
                            if (segment.length() == ++offset) break block6;
                            if (segment.charAt(offset) != '!') break block9;
                            signDisplay = NumberFormatter.SignDisplay.ALWAYS;
                            break block10;
                        }
                        if (segment.charAt(offset) != '?') break block6;
        ...
        private static void parseIntegerStem(StringSegment segment, MacroProps macros) {
            ...
            for (offset = 1; offset < segment.length(); ++offset) {
                if (segment.charAt(offset) == '0') continue;
                --offset;
                break;
            }
            if (offset < segment.length()) {
