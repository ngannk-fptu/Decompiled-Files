...
extends Http2ChannelDuplexHandler {
    ...
    public Http2MultiplexHandler(ChannelHandler inboundStreamHandler, ChannelHandler upgradeStreamHandler) {
        this.inboundStreamHandler = (ChannelHandler)ObjectUtil.checkNotNull((Object)inboundStreamHandler, (String)"inboundStreamHandler");
            ...
            if (event.type() == Http2FrameStreamEvent.Type.State) {
                switch (stream.state()) {
                    ...
                    case OPEN: {
                        ...
                        ChannelFuture future = ctx.channel().eventLoop().register((Channel)ch);
                        if (future.isDone()) {
                            Http2MultiplexHandler.registerDone(future);
                            break;
                        }
