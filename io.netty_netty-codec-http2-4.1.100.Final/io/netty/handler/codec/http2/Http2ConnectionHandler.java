...
ChannelOutboundHandler {
    ...
    protected Http2ConnectionHandler(Http2ConnectionDecoder decoder, Http2ConnectionEncoder encoder, Http2Settings initialSettings, boolean decoupleCloseAndGoAway, boolean flushPreface) {
        this.initialSettings = (Http2Settings)((Object)ObjectUtil.checkNotNull((Object)((Object)initialSettings), (String)"initialSettings"));
    ...
    protected void onConnectionError(ChannelHandlerContext ctx, boolean outbound, Throwable cause, Http2Exception http2Ex) {
        ...
        if (http2Ex.shutdownHint() == Http2Exception.ShutdownHint.GRACEFUL_SHUTDOWN) {
            ...
            future.addListener((GenericFutureListener)this.newClosingChannelFutureListener(ctx, promise));
        }
    }
    ...
    private ChannelFuture resetStream(final ChannelHandlerContext ctx, final Http2Stream stream, long errorCode, ChannelPromise promise) {
        ...
        stream.resetSent();
        Object future = stream.state() == Http2Stream.State.IDLE || this.connection().local().created(stream) && !stream.isHeadersSent() && !stream.isPushPromiseSent() ? promise.setSuccess() : this.frameWriter().writeRstStream(ctx, stream.id(), errorCode, promise);
        if (future.isDone()) {
    ...
    extends BaseDecoder {
        ...
        private boolean readClientPrefaceString(ByteBuf in) throws Http2Exception {
            ...
            if (bytesRead == 0 || !ByteBufUtil.equals((ByteBuf)in, (int)in.readerIndex(), (ByteBuf)this.clientPrefaceString, (int)this.clientPrefaceString.readerIndex(), (int)bytesRead)) {
                ...
                String receivedBytes = ByteBufUtil.hexDump((ByteBuf)in, (int)in.readerIndex(), (int)Math.min(in.readableBytes(), this.clientPrefaceString.readableBytes()));
        ...
        private boolean verifyFirstFrameIsSettings(ByteBuf in) throws Http2Exception {
            ...
            if (frameType != 4 || (flags & 1) != 0) {
                throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "First received frame was not SETTINGS. Hex dump for first 5 bytes: %s", ByteBufUtil.hexDump((ByteBuf)in, (int)in.readerIndex(), (int)5));
            }
            return true;
        }
