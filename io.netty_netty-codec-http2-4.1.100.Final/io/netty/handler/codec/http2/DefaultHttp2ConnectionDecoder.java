...
implements Http2ConnectionDecoder {
    ...
    implements Http2FrameListener {
        ...
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding, boolean endOfStream) throws Http2Exception {
            ...
            boolean bl = isInformational = !DefaultHttp2ConnectionDecoder.this.connection.isServer() && HttpStatusClass.valueOf((CharSequence)headers.status()) == HttpStatusClass.INFORMATIONAL;
            ...
            if (!isTrailers && (contentLength = headers.getAll(HttpHeaderNames.CONTENT_LENGTH)) != null && !contentLength.isEmpty()) {
                try {
                    long cLength = HttpUtil.normalizeAndGetContentLength((List)contentLength, (boolean)false, (boolean)true);
                    if (cLength != -1L) {
        ...
        private boolean shouldIgnoreHeadersOrDataFrame(ChannelHandlerContext ctx, int streamId, Http2Stream stream, String frameName) throws Http2Exception {
            ...
            if (stream.isResetSent() || this.streamCreatedAfterGoAwaySent(streamId)) {
                if (logger.isInfoEnabled()) {
                    logger.info("{} ignoring {} frame for stream {}", new Object[]{ctx.channel(), frameName, stream.isResetSent() ? "RST_STREAM sent." : "Stream created after GOAWAY sent. Last known stream by peer " + DefaultHttp2ConnectionDecoder.this.connection.remote().lastStreamKnownByPeer()});
                }
                return true;
            }
            return false;
