...
public abstract class BeanUtils {
    ...
    public static <T> T instantiateClass(Constructor<T> ctor, Object ... args) throws BeanInstantiationException {
        ...
        try {
            ...
            Assert.isTrue((args.length <= parameterTypes.length ? 1 : 0) != 0, (String)"Can't specify more arguments than constructor parameters");
    ...
    private static boolean isAssignable(Method writeMethod, Method readMethod) {
        ...
        if (paramType instanceof Class) {
            return ClassUtils.isAssignable((Class)((Class)paramType), readMethod.getReturnType());
        }
        if (paramType.equals(readMethod.getGenericReturnType())) {
            return true;
        }
        ResolvableType sourceType = ResolvableType.forMethodReturnType((Method)readMethod);
        ResolvableType targetType = ResolvableType.forMethodParameter((Method)writeMethod, (int)0);
    ...
    private static class KotlinDelegate {
        ...
        public static <T> T instantiateClass(Constructor<T> ctor, Object ... args) throws IllegalAccessException, InvocationTargetException, InstantiationException {
            ...
            Assert.isTrue((args.length <= parameters.size() ? 1 : 0) != 0, (String)"Number of provided arguments should be less of equals than number of constructor parameters");
            for (int i = 0; i < args.length; ++i) {
