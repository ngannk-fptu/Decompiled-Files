...
import net.sourceforge.htmlunit.cyberneko.LostText;
...
HTMLComponent {
    ...
    public void startDocument(XMLLocator locator, String encoding, NamespaceContext nscontext, Augmentations augs) throws XNIException {
        ...
        if (this.fragmentContextStack_ != null) {
            ...
            for (QName name : this.fragmentContextStack_) {
                HTMLElements.Element elt = this.htmlConfiguration_.htmlElements_.getElement(name.localpart);
    ...
    public void startElement(QName elem, XMLAttributes attrs, Augmentations augs) throws XNIException {
        ...
        if (isForcedCreation && (elementCode == 116 || elementCode == 103)) {
            return;
        }
        if (this.fSeenRootElement && elementCode == 54 && !this.fOpenedSvg) {
            this.notifyDiscardedStartElement(elem, attrs, augs);
            return;
        }
        if (this.fSeenFramesetElement && elementCode != 43 && elementCode != 44 && elementCode != 81) {
            this.notifyDiscardedStartElement(elem, attrs, augs);
            return;
        }
        if (elementCode == 51) {
        ...
        } else if (!this.fOpenedSvg && elementCode == 44) {
            ...
            if (!this.fSeenHeadElement) {
                ...
                this.forceStartElement(head, this.emptyAttributes(), this.synthesizedAugs());
        ...
        } else if (elementCode == 134) {
            this.consumeBufferedEndElements();
        } else if (elementCode == 116) {
            for (int i = this.fElementStack.top - 1; i >= 0; --i) {
                Info info = this.fElementStack.data[i];
                if (info.element.code == 118 || info.element.code == 122 || info.element.code == 19) break;
                if (info.element.code != 125 && info.element.code != 123 && info.element.code != 117 && info.element.code != 121 && info.element.code != 116) continue;
                QName head = this.createQName("table");
                this.endElement(head, this.synthesizedAugs());
                break;
            }
        }
        if (element.parent != null && !this.fOpenedSvg) {
            ...
            if ((!this.fDocumentFragment || preferedParent.code != 51 && preferedParent.code != 16) && !this.fTemplateFragment) {
                int depth;
                if (!this.fSeenRootElement && !this.fDocumentFragment) {
                    QName qname;
                    ...
                    if (!(parentCreated = this.forceStartElement(qname = this.createQName(pname), this.emptyAttributes(), this.synthesizedAugs()))) {
                        if (!isForcedCreation) {
                            this.notifyDiscardedStartElement(elem, attrs, augs);
                        }
                        return;
                    }
                } else if ((preferedParent.code != 51 || !this.fSeenBodyElement && !this.fDocumentFragment) && (depth = this.getParentDepth(element.parent, element.bounds)) == -1) {
                    boolean parentCreated;
                    ...
                    if (!(parentCreated = this.forceStartElement(qname, this.emptyAttributes(), this.synthesizedAugs()))) {
                        if (!isForcedCreation) {
                            this.notifyDiscardedStartElement(elem, attrs, augs);
                        }
                        return;
        ...
        if (this.fElementStack.top > 1 && this.fElementStack.peek().element.code == 101 || this.fElementStack.top > 2 && this.fElementStack.data[this.fElementStack.top - 2].element.code == 51) {
            Info info = this.fElementStack.pop();
            if (this.fDocumentHandler != null) {
                this.callEndElement(info.qname, this.synthesizedAugs());
            }
    ...
    public void startGeneralEntity(String name, XMLResourceIdentifier id, String encoding, Augmentations augs) throws XNIException {
        ...
        if (!this.fDocumentFragment) {
            ...
            if (!insertBody) {
                ...
                if (info.element.code == 51 || info.element.code == 54) {
    ...
    private void forceStartBody() {
        ...
        this.forceStartElement(body, this.emptyAttributes(), this.synthesizedAugs());
    }
    ...
    public void characters(XMLString text, Augmentations augs) throws XNIException {
        ...
        int limit = text.offset + text.length;
    ...
    public void endElement(QName element, Augmentations augs) throws XNIException {
        ...
        if (!(this.fIgnoreOutsideContent || elementCode != 16 && elementCode != 54)) {
        ...
        if (this.fSeenFramesetElement && elementCode != 43 && elementCode != 44) {
            this.notifyDiscardedEndElement(element, augs);
            return;
        }
        if (elementCode == 54) {
        ...
        } else if (elementCode == 115) {
            this.fOpenedSvg = false;
        } else if (elementCode == 51 && !forcedEndElement) {
            this.endElementsBuffer_.add(new ElementEntry(element, augs));
            return;
        }
        ...
        if (depth > 1 && elem.isInline()) {
            ...
            for (i = 0; i < depth - 1; ++i) {
                ...
                if (!pelem.isInline() && pelem.code != 40) continue;
                this.fInlineStack.push(info);
            }
        }
    ...
    protected HTMLElements.Element getElement(QName elementName) {
        ...
        return this.htmlConfiguration_.htmlElements_.getElement(name);
    }
    ...
    private void addBodyIfNeeded(short element) {
        if (!this.fDocumentFragment && !this.fSeenFramesetElement && element == 54) {
            if (!this.fSeenHeadElement) {
                QName head = this.createQName("head");
                this.callStartElement(head, this.emptyAttributes(), this.synthesizedAugs());
                this.callEndElement(head, this.synthesizedAugs());
            }
            if (!this.fSeenBodyElement) {
                QName body = this.createQName("body");
                this.callStartElement(body, this.emptyAttributes(), this.synthesizedAugs());
    ...
    protected final int getElementDepth(HTMLElements.Element element) {
        ...
        short elementCode = element.code;
        boolean tableBodyOrHtml = elementCode == 116 || elementCode == 16 || elementCode == 54;
        int depth = -1;
        for (int i = this.fElementStack.top - 1; i >= this.fragmentContextStackSize_; --i) {
            Info info = this.fElementStack.data[i];
            if (info.element.code == element.code && (elementCode != 134 || elementCode == 134 && element.name.equals(info.element.name))) {
                depth = this.fElementStack.top - i;
                break;
            }
            if (!container && info.element.isBlock()) break;
            if (info.element.code == 116 && !tableBodyOrHtml) {
                return -1;
            }
