    ...
    FingerprintTrustManagerFactory(final String algorithm, byte[][] fingerprints) {
        ...
        for (byte[] f : fingerprints) {
            ...
            if (f.length != hashLength) {
                throw new IllegalArgumentException(String.format("malformed fingerprint (length is %d but expected %d): %s", f.length, hashLength, ByteBufUtil.hexDump((ByteBuf)Unpooled.wrappedBuffer((byte[])f))));
            }
            list.add(f.clone());
        }
