...
final class SslUtils {
    ...
    private static short shortBE(ByteBuffer buffer, int offset) {
        return buffer.order() == ByteOrder.BIG_ENDIAN ? buffer.getShort(offset) : ByteBufUtil.swapShort((short)buffer.getShort(offset));
    }
    ...
    static boolean isValidHostNameForSNI(String hostname) {
        return hostname != null && hostname.indexOf(46) > 0 && !hostname.endsWith(".") && !hostname.startsWith("/") && !NetUtil.isValidIpV4Address((String)hostname) && !NetUtil.isValidIpV6Address((String)hostname);
    }
