...
implements ChannelOutboundHandler {
    ...
    public SslHandler(SSLEngine engine, boolean startTls, Executor delegatedTaskExecutor) {
        ...
        this.delegatedTaskExecutor = (Executor)ObjectUtil.checkNotNull((Object)delegatedTaskExecutor, (String)"delegatedTaskExecutor");
        ...
        block23: {
            ...
            try {
                while (!ctx.isRemoved()) {
                    ...
                    if ((result = this.wrap(alloc, this.engine, Unpooled.EMPTY_BUFFER, out)).bytesProduced() > 0) {
    ...
    private boolean setHandshakeSuccess() {
        boolean notified = !this.handshakePromise.isDone() && this.handshakePromise.trySuccess((Object)this.ctx.channel());
        if (notified) {
            if (logger.isDebugEnabled()) {
        ...
        block7: {
            ...
            try {
                ...
                if (!this.isStateSet(64)) {
                    ...
                    this.sslClosePromise.addListener((GenericFutureListener)new FutureListener<Channel>(promise){
    ...
    private void applyHandshakeTimeout() {
        ...
        localHandshakePromise.addListener((GenericFutureListener)new FutureListener<Channel>((Future)timeoutFuture){
    ...
    private void safeClose(final ChannelHandlerContext ctx, final ChannelFuture flushFuture, final ChannelPromise promise) {
        ...
        ScheduledFuture timeoutFuture = !flushFuture.isDone() ? ((closeNotifyTimeout = this.closeNotifyFlushTimeoutMillis) > 0L ? ctx.executor().schedule(new Runnable(){
        ...
        flushFuture.addListener((GenericFutureListener)new ChannelFutureListener((Future)timeoutFuture, ctx, promise){
            final /* synthetic */ Future val$timeoutFuture;
            ...
            public void operationComplete(ChannelFuture f) {
                ...
                if ((closeNotifyReadTimeout = SslHandler.this.closeNotifyReadTimeoutMillis) <= 0L) {
                    ...
                    final ScheduledFuture closeNotifyReadTimeoutFuture = !SslHandler.this.sslClosePromise.isDone() ? this.val$ctx.executor().schedule(new Runnable(){

                        @Override
                        public void run() {
                            if (!SslHandler.this.sslClosePromise.isDone()) {
                                logger.debug("{} did not receive close_notify in {}ms; force-closing the connection.", (Object)val$ctx.channel(), (Object)closeNotifyReadTimeout);
    ...
    private static void addCloseListener(ChannelFuture future, ChannelPromise promise) {
        PromiseNotifier.cascade((boolean)false, (Future)future, (Promise)promise);
    }
    ...
    private static boolean attemptCopyToCumulation(ByteBuf cumulation, ByteBuf next, int wrapDataSize) {
        ...
        if (wrapDataSize - cumulation.readableBytes() >= inReadableBytes && (cumulation.isWritable(inReadableBytes) && cumulationCapacity >= wrapDataSize || cumulationCapacity < wrapDataSize && ByteBufUtil.ensureWritableSuccess((int)cumulation.ensureWritable(inReadableBytes, false)))) {
