...
ECPointEncoder {
    ...
    private void populateFromPrivKeyInfo(PrivateKeyInfo privateKeyInfo) throws IOException {
        ...
        if (aSN1Primitive instanceof ASN1Sequence && (ASN1Sequence.getInstance(aSN1Primitive).size() == 2 || ASN1Sequence.getInstance(aSN1Primitive).size() == 3)) {
            ...
            if (aSN1OctetString.getOctets().length == 32 || aSN1OctetString.getOctets().length == 64) {
                this.d = new BigInteger(1, Arrays.reverse(aSN1OctetString.getOctets()));
            } else {
                ...
                if (aSN1Encodable instanceof ASN1Integer) {
                    ...
                    this.d = new BigInteger(1, Arrays.reverse(byArray));
                }
            }
        } else {
            Object object;
            ...
            if (x962Parameters.isNamedCurve()) {
                ...
                if (object == null) {
                    X9ECParameters x9ECParameters = ECGOST3410NamedCurves.getByOIDX9((ASN1ObjectIdentifier)aSN1Encodable);
                    EllipticCurve ellipticCurve = EC5Util.convertCurve(x9ECParameters.getCurve(), x9ECParameters.getSeed());
                    this.ecSpec = new ECNamedCurveSpec(ECGOST3410NamedCurves.getName((ASN1ObjectIdentifier)aSN1Encodable), ellipticCurve, EC5Util.convertPoint(x9ECParameters.getG()), x9ECParameters.getN(), x9ECParameters.getH());
                } else {
    ...
    private DERBitString getPublicKeyDetails(BCECGOST3410_2012PublicKey bCECGOST3410_2012PublicKey) {
        SubjectPublicKeyInfo subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(bCECGOST3410_2012PublicKey.getEncoded());
