...
public class BufferedDecoder {
    ...
    public int processByte(byte by, byte[] byArray, int n) {
        ...
        if (this.bufOff == this.buf.length) {
            n2 = this.translator.decode(this.buf, 0, this.buf.length, byArray, n);
            this.bufOff = 0;
        }
        return n2;
    }
    ...
    public int processBytes(byte[] byArray, int n, int n2, byte[] byArray2, int n3) {
        ...
        if (n2 > n5) {
            ...
            n4 += this.translator.decode(this.buf, 0, this.buf.length, byArray2, n3);
            this.bufOff = 0;
            n3 += n4;
            int n6 = (n2 -= n5) - n2 % this.buf.length;
            n4 += this.translator.decode(byArray, n += n5, n6, byArray2, n3);
            n2 -= n6;
            n += n6;
        }
        if (n2 != 0) {
