...
implements ExtendedDigest {
    ...
    public int doFinal(byte[] byArray, int n) {
        ...
        for (int i = 0; i < this.chainValue.length && i * 8 < this.digestLength; ++i) {
            byte[] byArray2 = Pack.longToLittleEndian(this.chainValue[i]);
    ...
    private void compress(byte[] byArray, int n) {
        ...
        for (n2 = 0; n2 < 16; ++n2) {
            lArray[n2] = Pack.littleEndianToLong(byArray, n + n2 * 8);
        }
    ...
    private void G(long l, long l2, int n, int n2, int n3, int n4) {
        ...
        this.internalState[n4] = Longs.rotateRight(this.internalState[n4] ^ this.internalState[n], 32);
        this.internalState[n3] = this.internalState[n3] + this.internalState[n4];
        this.internalState[n2] = Longs.rotateRight(this.internalState[n2] ^ this.internalState[n3], 24);
        this.internalState[n] = this.internalState[n] + this.internalState[n2] + l2;
        this.internalState[n4] = Longs.rotateRight(this.internalState[n4] ^ this.internalState[n], 16);
        this.internalState[n3] = this.internalState[n3] + this.internalState[n4];
        this.internalState[n2] = Longs.rotateRight(this.internalState[n2] ^ this.internalState[n3], 63);
    }
