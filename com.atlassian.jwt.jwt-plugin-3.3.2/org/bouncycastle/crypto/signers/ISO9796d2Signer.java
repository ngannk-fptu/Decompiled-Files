...
implements SignerWithRecovery {
    ...
    public void updateWithRecoveredMessage(byte[] byArray) throws InvalidCipherTextException {
        ...
        int n5 = byArray2.length - n2 - this.digest.getDigestSize();
        if (n5 - ++n <= 0) {
            throw new InvalidCipherTextException("malformed block");
        }
        if ((byArray2[0] & 0x20) == 0) {
            this.fullMessage = true;
            this.recoveredMessage = new byte[n5 - n];
    ...
    public boolean verifySignature(byte[] byArray) {
        ...
        if ((byArray2[0] & 0x20) == 0) {
            this.fullMessage = true;
            if (this.messageLength > n - n2) {
                return this.returnFalse(byArray2);
            }
            this.digest.reset();
            this.digest.update(byArray2, n2, n - n2);
            ...
            this.recoveredMessage = new byte[n - n2];
