...
extends SerpentEngineBase {
    protected int[] makeWorkingKey(byte[] byArray) throws IllegalArgumentException {
        ...
        int n3 = 0;
        n2 = 0;
        while (n2 + 4 < byArray.length) {
            nArray[n3++] = Pack.littleEndianToInt(byArray, n2);
            n2 += 4;
        }
        if (n2 % 4 == 0) {
            nArray[n3++] = Pack.littleEndianToInt(byArray, n2);
        ...
        for (n = 8; n < 16; ++n) {
            nArray[n] = SerpentEngine.rotateLeft(nArray[n - 8] ^ nArray[n - 5] ^ nArray[n - 3] ^ nArray[n - 1] ^ 0x9E3779B9 ^ n - 8, 11);
        }
        System.arraycopy(nArray, 8, nArray2, 0, 8);
        for (n = 8; n < n4; ++n) {
            nArray2[n] = SerpentEngine.rotateLeft(nArray2[n - 8] ^ nArray2[n - 5] ^ nArray2[n - 3] ^ nArray2[n - 1] ^ 0x9E3779B9 ^ n, 11);
        }
    ...
    protected void encryptBlock(byte[] byArray, int n, byte[] byArray2, int n2) {
        this.X0 = Pack.littleEndianToInt(byArray, n);
        this.X1 = Pack.littleEndianToInt(byArray, n + 4);
        this.X2 = Pack.littleEndianToInt(byArray, n + 8);
        this.X3 = Pack.littleEndianToInt(byArray, n + 12);
        ...
        Pack.intToLittleEndian(this.wKey[128] ^ this.X0, byArray2, n2);
        Pack.intToLittleEndian(this.wKey[129] ^ this.X1, byArray2, n2 + 4);
        Pack.intToLittleEndian(this.wKey[130] ^ this.X2, byArray2, n2 + 8);
        Pack.intToLittleEndian(this.wKey[131] ^ this.X3, byArray2, n2 + 12);
    }

    protected void decryptBlock(byte[] byArray, int n, byte[] byArray2, int n2) {
        this.X0 = this.wKey[128] ^ Pack.littleEndianToInt(byArray, n);
        this.X1 = this.wKey[129] ^ Pack.littleEndianToInt(byArray, n + 4);
        this.X2 = this.wKey[130] ^ Pack.littleEndianToInt(byArray, n + 8);
        this.X3 = this.wKey[131] ^ Pack.littleEndianToInt(byArray, n + 12);
        ...
        this.inverseLT();
        this.ib0(this.X0, this.X1, this.X2, this.X3);
        Pack.intToLittleEndian(this.X0 ^ this.wKey[0], byArray2, n2);
        Pack.intToLittleEndian(this.X1 ^ this.wKey[1], byArray2, n2 + 4);
        Pack.intToLittleEndian(this.X2 ^ this.wKey[2], byArray2, n2 + 8);
        Pack.intToLittleEndian(this.X3 ^ this.wKey[3], byArray2, n2 + 12);
    }
}
