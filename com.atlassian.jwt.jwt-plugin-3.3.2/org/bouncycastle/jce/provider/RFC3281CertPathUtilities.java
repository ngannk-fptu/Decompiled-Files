...
class RFC3281CertPathUtilities {
    ...
    protected static void checkCRLs(X509AttributeCertificate x509AttributeCertificate, PKIXExtendedParameters pKIXExtendedParameters, Date date, Date date2, X509Certificate x509Certificate, List list, JcaJceHelper jcaJceHelper) throws CertPathValidatorException {
        if (pKIXExtendedParameters.isRevocationEnabled()) {
            if (x509AttributeCertificate.getExtensionValue(NO_REV_AVAIL) == null) {
                ...
                if (cRLDistPoint != null) {
                    ...
                    try {
                        for (int i = 0; i < ((DistributionPoint[])object).length && ((CertStatus)object2).getCertStatus() == 11 && !reasonsMask.isAllReasons(); ++i) {
                            ...
                            RFC3281CertPathUtilities.checkCRL(object[i], x509AttributeCertificate, pKIXExtendedParameters2, date, date2, x509Certificate, (CertStatus)object2, reasonsMask, list, jcaJceHelper);
                            bl = true;
                        }
                    }
                ...
                if (((CertStatus)object2).getCertStatus() == 11 && !reasonsMask.isAllReasons()) {
                    try {
                        try {
                            object = PrincipalUtils.getEncodedIssuerPrincipal(x509AttributeCertificate);
                        }
                        catch (Exception exception) {
                        ...
                        pKIXExtendedParameters2 = (PKIXExtendedParameters)pKIXExtendedParameters.clone();
                        RFC3281CertPathUtilities.checkCRL(distributionPoint, x509AttributeCertificate, pKIXExtendedParameters2, date, date2, x509Certificate, (CertStatus)object2, reasonsMask, list, jcaJceHelper);
                        bl = true;
                    }
    ...
    protected static CertPath processAttrCert1(X509AttributeCertificate x509AttributeCertificate, PKIXExtendedParameters pKIXExtendedParameters) throws CertPathValidatorException {
        ...
        if (x509AttributeCertificate.getHolder().getIssuer() != null) {
            ...
            for (n = 0; n < ((Principal[])object).length; ++n) {
                try {
                    ...
                    selector = new PKIXCertStoreSelector.Builder((CertSelector)object2).build();
                    CertPathValidatorUtilities.findCertificates(linkedHashSet, (PKIXCertStoreSelector)selector, pKIXExtendedParameters.getCertStores());
                    continue;
                }
                catch (AnnotatedException annotatedException) {
        ...
        while (iterator.hasNext()) {
            ...
            selector.setCertificate((X509Certificate)iterator.next());
            ((PKIXExtendedParameters.Builder)object2).setTargetConstraints(new PKIXCertStoreSelector.Builder((CertSelector)((Object)selector)).build());
    ...
    private static void checkCRL(DistributionPoint distributionPoint, X509AttributeCertificate x509AttributeCertificate, PKIXExtendedParameters pKIXExtendedParameters, Date date, Date date2, X509Certificate x509Certificate, CertStatus certStatus, ReasonsMask reasonsMask, List list, JcaJceHelper jcaJceHelper) throws AnnotatedException, RecoverableCertPathValidatorException {
        ...
        PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters = new PKIXCertRevocationCheckerParameters(pKIXExtendedParameters, date2, null, -1, x509Certificate, null);
        Set set = CertPathValidatorUtilities.getCompleteCRLs(pKIXCertRevocationCheckerParameters, distributionPoint, x509AttributeCertificate, pKIXExtendedParameters, date2);
        boolean bl = false;
        AnnotatedException annotatedException = null;
        Iterator iterator = set.iterator();
        while (iterator.hasNext() && certStatus.getCertStatus() == 11 && !reasonsMask.isAllReasons()) {
            try {
                ...
                if (pKIXExtendedParameters.isUseDeltasEnabled()) {
                    Set set3 = CertPathValidatorUtilities.getDeltaCRLs(date, x509CRL, pKIXExtendedParameters.getCertStores(), pKIXExtendedParameters.getCRLStores(), jcaJceHelper);
