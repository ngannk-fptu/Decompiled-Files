...
extends CertPathValidatorSpi {
    ...
    public PKIXCertPathValidatorSpi_8(boolean bl) {
        this.isForCRLCheck = bl;
    }
    ...
    public CertPathValidatorResult engineValidate(CertPath certPath, CertPathParameters certPathParameters) throws CertPathValidatorException, InvalidAlgorithmParameterException {
        ...
        Date date = new Date();
        Date date2 = CertPathValidatorUtilities.getValidityDate(pKIXExtendedParameters, date);
        ...
        try {
            ...
            PKIXCertPathValidatorSpi_8.checkCertificate(trustAnchor.getTrustedCert());
        }
        ...
        for (PKIXCertPathChecker pKIXCertPathChecker : pKIXExtendedParameters.getCertPathCheckers()) {
            pKIXCertPathChecker.init(false);
            if (pKIXCertPathChecker instanceof PKIXRevocationChecker) {
                if (pKIXCertRevocationChecker != null) {
                    throw new CertPathValidatorException("only one PKIXRevocationChecker allowed");
                }
                pKIXCertRevocationChecker = pKIXCertPathChecker instanceof PKIXCertRevocationChecker ? (PKIXCertRevocationChecker)((Object)pKIXCertPathChecker) : new WrappedRevocationChecker(pKIXCertPathChecker);
                continue;
            }
            arrayList.add(pKIXCertPathChecker);
        }
        if (pKIXExtendedParameters.isRevocationEnabled() && pKIXCertRevocationChecker == null) {
        ...
        try {
            ...
            throw new ExtCertPathValidatorException("Subject of trust anchor could not be (re)encoded.", (Throwable)runtimeException, certPath, -1);
        }
        ...
        for (n2 = object.size() - 1; n2 >= 0; --n2) {
            ...
            try {
                PKIXCertPathValidatorSpi_8.checkCertificate(x509Certificate2);
            }
            catch (AnnotatedException annotatedException) {
                throw new CertPathValidatorException(annotatedException.getMessage(), annotatedException.getUnderlyingException(), certPath, n2);
            }
            RFC3280CertPathUtilities.processCertA(certPath, pKIXExtendedParameters, date2, pKIXCertRevocationChecker, n2, publicKey, bl, x500Name, x509Certificate);
            RFC3280CertPathUtilities.processCertBC(certPath, n2, pKIXNameConstraintValidator, this.isForCRLCheck);
            pKIXPolicyNode = RFC3280CertPathUtilities.processCertD(certPath, n2, hashSet3, pKIXPolicyNode, listArray, n4, this.isForCRLCheck);
            ...
            RFC3280CertPathUtilities.prepareNextCertO(certPath, n2, hashSet, arrayList);
        ...
        RFC3280CertPathUtilities.wrapupCertF(certPath, n2 + 1, arrayList, set2);
    ...
    static void checkCertificate(X509Certificate x509Certificate) throws AnnotatedException {
        if (x509Certificate instanceof BCX509Certificate) {
            ...
            try {
                if (null != ((BCX509Certificate)((Object)x509Certificate)).getTBSCertificateNative()) {
                    return;
                }
            }
