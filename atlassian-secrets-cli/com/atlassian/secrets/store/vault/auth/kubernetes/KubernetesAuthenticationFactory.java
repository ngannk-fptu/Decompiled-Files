...
public class KubernetesAuthenticationFactory {
    ...
    public KubernetesAuthentication getAuthentication(VaultParams params) throws SecretStoreException {
        try {
            ...
            if (jwtPath != null) {
                builder.jwtSupplier(this.kubernetesServiceAccountTokenFileFactory.getKubernetesServiceAccountTokenFile(jwtPath));
            }
            if (kubeAuthPath != null) {
                builder.path(kubeAuthPath);
            }
            KubernetesAuthenticationOptions options = builder.build();
            VaultEndpoint vaultEndpoint = VaultEndpoint.from(URI.create(endpoint));
            RestTemplate restTemplate = new RestTemplate();
            restTemplate.setUriTemplateHandler(new DefaultUriBuilderFactory(String.format("%s/%s/", vaultEndpoint, vaultEndpoint.getPath())));
