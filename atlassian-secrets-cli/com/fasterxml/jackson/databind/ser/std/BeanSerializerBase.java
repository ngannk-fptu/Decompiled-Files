...
SchemaAware {
    ...
    protected BeanSerializerBase withIgnorals(Set<String> toIgnore) {
        return this.withByNameInclusion(toIgnore, null);
    }
    ...
    public void serializeWithType(Object bean2, JsonGenerator gen, SerializerProvider provider, TypeSerializer typeSer) throws IOException {
        ...
        gen.setCurrentValue(bean2);
        WritableTypeId typeIdDef = this._typeIdDef(typeSer, bean2, JsonToken.START_OBJECT);
        typeSer.writeTypePrefix(gen, typeIdDef);
        if (this._propertyFilterId != null) {
            this.serializeFieldsFiltered(bean2, gen, provider);
        } else {
            this.serializeFields(bean2, gen, provider);
        }
        typeSer.writeTypeSuffix(gen, typeIdDef);
    }

    protected final void _serializeWithObjectId(Object bean2, JsonGenerator gen, SerializerProvider provider, boolean startEndObject) throws IOException {
        ObjectIdWriter w = this._objectIdWriter;
        WritableObjectId objectId = provider.findObjectId(bean2, w.generator);
        if (objectId.writeAsId(gen, provider, w)) {
            return;
        }
        Object id = objectId.generateId(bean2);
        if (w.alwaysAsId) {
            w.serializer.serialize(id, gen, provider);
            return;
        }
        if (startEndObject) {
            gen.writeStartObject(bean2);
        }
    ...
    protected final void _serializeWithObjectId(Object bean2, JsonGenerator gen, SerializerProvider provider, TypeSerializer typeSer) throws IOException {
        ObjectIdWriter w = this._objectIdWriter;
        WritableObjectId objectId = provider.findObjectId(bean2, w.generator);
        if (objectId.writeAsId(gen, provider, w)) {
            return;
        ...
        this._serializeObjectId(bean2, gen, provider, typeSer, objectId);
    }

    protected void _serializeObjectId(Object bean2, JsonGenerator g, SerializerProvider provider, TypeSerializer typeSer, WritableObjectId objectId) throws IOException {
        ObjectIdWriter w = this._objectIdWriter;
        WritableTypeId typeIdDef = this._typeIdDef(typeSer, bean2, JsonToken.START_OBJECT);
        typeSer.writeTypePrefix(g, typeIdDef);
        objectId.writeAsField(g, provider, w);
        if (this._propertyFilterId != null) {
            this.serializeFieldsFiltered(bean2, g, provider);
        } else {
            this.serializeFields(bean2, g, provider);
        }
        typeSer.writeTypeSuffix(g, typeIdDef);
    }

    protected final WritableTypeId _typeIdDef(TypeSerializer typeSer, Object bean2, JsonToken valueShape) {
        if (this._typeId == null) {
            return typeSer.typeId(bean2, valueShape);
        }
        Object typeId = this._typeId.getValue(bean2);
        if (typeId == null) {
            typeId = "";
        }
        return typeSer.typeId(bean2, valueShape, typeId);
    }

    @Deprecated
    protected final String _customTypeId(Object bean2) {
        Object typeId = this._typeId.getValue(bean2);
        if (typeId == null) {
            return "";
        }
    ...
    protected void serializeFields(Object bean2, JsonGenerator gen, SerializerProvider provider) throws IOException {
        int i;
        ...
        try {
            for (BeanPropertyWriter prop : props) {
                ...
                prop.serializeAsField(bean2, gen, provider);
            }
            if (this._anyGetterWriter != null) {
                this._anyGetterWriter.getAndSerialize(bean2, gen, provider);
            }
        }
        catch (Exception e) {
            String name = i == props.length ? "[anySetter]" : props[i].getName();
            this.wrapAndThrow(provider, (Throwable)e, bean2, name);
        }
        catch (StackOverflowError e) {
            JsonMappingException mapE = new JsonMappingException((Closeable)gen, "Infinite recursion (StackOverflowError)", (Throwable)e);
            String name = i == props.length ? "[anySetter]" : props[i].getName();
            mapE.prependPath(new JsonMappingException.Reference(bean2, name));
            throw mapE;
        }
    }

    protected void serializeFieldsFiltered(Object bean2, JsonGenerator gen, SerializerProvider provider) throws IOException, JsonGenerationException {
        int i;
        BeanPropertyWriter[] props = this._filteredProps != null && provider.getActiveView() != null ? this._filteredProps : this._props;
        PropertyFilter filter = this.findPropertyFilter(provider, this._propertyFilterId, bean2);
        if (filter == null) {
            this.serializeFields(bean2, gen, provider);
            return;
        }
        try {
            for (BeanPropertyWriter prop : props) {
                if (prop == null) continue;
                filter.serializeAsField(bean2, gen, provider, prop);
            }
            if (this._anyGetterWriter != null) {
                this._anyGetterWriter.getAndFilter(bean2, gen, provider, filter);
            }
        }
        catch (Exception e) {
            String name = i == props.length ? "[anySetter]" : props[i].getName();
    ...
    public JsonNode getSchema(SerializerProvider provider, Type typeHint) throws JsonMappingException {
        ...
        if (ann != null && (id = ann.id()) != null && !id.isEmpty()) {
            o.put("id", id);
        }
