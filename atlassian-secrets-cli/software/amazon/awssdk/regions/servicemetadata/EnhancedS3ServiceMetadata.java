...
implements ServiceMetadata {
    ...
    private EnhancedS3ServiceMetadata(ServiceMetadataConfiguration config) {
        ...
        Supplier<String> profileName = config.profileName() != null ? () -> config.profileName() : ProfileFileSystemSetting.AWS_PROFILE::getStringValueOrThrow;
        this.useUsEast1RegionalEndpoint = new Lazy<Boolean>(() -> this.useUsEast1RegionalEndpoint(profileFile, profileName, config));
        this.s3ServiceMetadata = new S3ServiceMetadata().reconfigure(config);
    }

    @Override
    public URI endpointFor(ServiceEndpointKey key) {
        if (Region.US_EAST_1.equals(key.region()) && key.tags().isEmpty() && !this.useUsEast1RegionalEndpoint.getValue().booleanValue()) {
    ...
    private boolean useUsEast1RegionalEndpoint(Supplier<ProfileFile> profileFile, Supplier<String> profileName, ServiceMetadataConfiguration config) {
        ...
        String profile2 = this.profileFileSetting(profileFile, profileName);
        if (profile2 != null) {
            return REGIONAL_SETTING.equalsIgnoreCase(profile2);
        }
