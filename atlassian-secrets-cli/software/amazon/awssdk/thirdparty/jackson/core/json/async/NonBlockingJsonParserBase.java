...
extends ParserBase {
    ...
    public JsonLocation getCurrentLocation() {
        ...
        return new JsonLocation(this._contentReference(), this._currInputProcessed + (long)(this._inputPtr - this._currBufferStart), -1L, row, col);
    }
    ...
    protected final JsonToken _valueNonStdNumberComplete(int type) throws IOException {
        ...
        if (!this.isEnabled(JsonParser.Feature.ALLOW_NON_NUMERIC_NUMBERS)) {
            this._reportError("Non-standard token '%s': enable `JsonReadFeature.ALLOW_NON_NUMERIC_NUMBERS` to allow", tokenStr);
        }
        this._intLength = 0;
