...
implements AsyncRequestBodySigner {
    ...
    private String getHeaderSignature(SdkHttpFullRequest request) {
        Matcher matcher;
        Optional<String> authHeader = request.firstMatchingHeader("Authorization");
        if (authHeader.isPresent() && (matcher = AUTHENTICATION_HEADER_PATTERN.matcher(authHeader.get())).matches()) {
