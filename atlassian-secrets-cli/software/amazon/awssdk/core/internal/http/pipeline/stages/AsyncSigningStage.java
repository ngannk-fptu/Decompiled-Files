...
implements RequestPipeline<SdkHttpFullRequest, CompletableFuture<SdkHttpFullRequest>> {
    ...
    private <T extends Identity> CompletableFuture<SdkHttpFullRequest> doSraSign(SdkHttpFullRequest request, RequestExecutionContext context, SelectedAuthScheme<T> selectedAuthScheme, T identity) {
        ...
        if (context.requestProvider() == null) {
            SignRequest.Builder signRequestBuilder = (SignRequest.Builder)((SignRequest.Builder)((SignRequest.Builder)SignRequest.builder(identity).putProperty(HttpSigner.SIGNING_CLOCK, this.signingClock())).request(request)).payload(request.contentStreamProvider().orElse(null));
        ...
        AsyncSignRequest.Builder signRequestBuilder = (AsyncSignRequest.Builder)((AsyncSignRequest.Builder)((AsyncSignRequest.Builder)AsyncSignRequest.builder(identity).putProperty(HttpSigner.SIGNING_CLOCK, this.signingClock())).request(request)).payload(context.requestProvider());
        authSchemeOption.forEachSignerProperty(signRequestBuilder::putProperty);
        CompletableFuture<AsyncSignedRequest> signedRequestFuture = signer.signAsync((AsyncSignRequest)signRequestBuilder.build());
    ...
    private static void updateAsyncRequestBodyInContexts(RequestExecutionContext context, AsyncSignedRequest signedRequest) {
        ...
        AsyncRequestBody newAsyncRequestBody = optionalPayload.isPresent() ? ((signedPayload = (Publisher)optionalPayload.get()) instanceof AsyncRequestBody ? (AsyncRequestBody)signedPayload : AsyncRequestBody.fromPublisher(signedPayload)) : null;
    ...
    private SdkHttpFullRequest toSdkHttpFullRequest(SignedRequest signedRequest) {
        return this.toSdkHttpFullRequestBuilder(signedRequest).contentStreamProvider(signedRequest.payload().orElse(null)).build();
    }
    ...
    private SdkHttpFullRequest toSdkHttpFullRequest(AsyncSignedRequest signedRequest) {
        ...
        return this.toSdkHttpFullRequestBuilder(signedRequest).build();
    }
    ...
    private SdkHttpFullRequest.Builder toSdkHttpFullRequestBuilder(BaseSignedRequest<?> baseSignedRequest) {
        ...
        return SdkHttpFullRequest.builder().protocol(request.protocol()).method(request.method()).host(request.host()).port(request.port()).encodedPath(request.encodedPath()).applyMutation(r -> request.forEachHeader(r::putHeader)).applyMutation(r -> request.forEachRawQueryParameter(r::putRawQueryParameter));
    }
