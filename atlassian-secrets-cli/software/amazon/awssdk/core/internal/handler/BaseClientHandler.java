...
public abstract class BaseClientHandler {
    ...
    private static SdkHttpFullRequest modifyEndpointHostIfNeeded(SdkHttpFullRequest originalRequest, SdkClientConfiguration clientConfiguration, ClientExecutionParams executionParams) {
        ...
        if (disableHostPrefixInjection != null && disableHostPrefixInjection.equals(Boolean.TRUE) || StringUtils.isEmpty(executionParams.hostPrefixExpression())) {
    ...
    private static void addHttpRequest(ExecutionContext executionContext, SdkHttpFullRequest request) {
        ...
        interceptorContext = contentStreamProvider.isPresent() ? (InterceptorContext)interceptorContext.copy(b -> b.httpRequest(request).requestBody(BaseClientHandler.getBody(request))) : (InterceptorContext)interceptorContext.copy(b -> b.httpRequest(request));
