...
ToCopyableBuilder<Builder, FullJitterBackoffStrategy> {
    ...
    FullJitterBackoffStrategy(Duration baseDelay, Duration maxBackoffTime, Random random) {
        this.baseDelay = NumericUtils.min(Validate.isNotNegative(baseDelay, "baseDelay"), BASE_DELAY_CEILING);
        this.maxBackoffTime = NumericUtils.min(Validate.isNotNegative(maxBackoffTime, "maxBackoffTime"), MAX_BACKOFF_CEILING);
        this.random = random;
    }
    ...
    public Duration computeDelayBeforeNextRetry(RetryPolicyContext context) {
        ...
        return Duration.ofMillis((long)this.random.nextInt(ceil) + 1L);
    }
    ...
    extends CopyableBuilder<Builder, FullJitterBackoffStrategy> {
