...
public final class FnNode {
    ...
    public Fn validate() {
        ...
        throw RuleError.builder().cause(SourceException.builder().message(String.format("`%s` is not a valid function", this.fn)).build()).build();
    }
    ...
    public static FnNode fromNode(JsonNode node) {
        ...
        return FnNode.builder().fn(objNode.get(FN).asString()).argv(objNode.get(ARGV).asArray().stream().map(Expr::fromNode).collect(Collectors.toList())).build();
    }
