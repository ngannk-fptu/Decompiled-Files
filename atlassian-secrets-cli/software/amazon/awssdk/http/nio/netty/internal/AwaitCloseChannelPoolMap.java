...
extends SdkChannelPoolMap<URI, SimpleChannelPoolAwareChannelPool> {
    ...
    protected SimpleChannelPoolAwareChannelPool newPool(URI key) {
        ...
        if (this.shouldUseProxyForHost(key)) {
            ...
            baseChannelPool = new Http1TunnelConnectionPool(bootstrap.config().group().next(), tcpChannelPool, sslContext, this.proxyAddress(key), this.proxyConfiguration.username(), this.proxyConfiguration.password(), key, pipelineInitializer, this.configuration);
        } else {
