...
implements StreamByteDistributor {
    ...
    implements PriorityQueueNode {
        ...
        void activeCountChangeForTree(int increment) {
            ...
            if (this.parent != null) {
                assert (this.activeCountForTree != increment || this.pseudoTimeQueueIndex == -1 || this.parent.pseudoTimeQueue.containsTyped(this)) : "State[" + this.streamId + "].activeCountForTree changed from 0 to " + increment + " is in a pseudoTimeQueue, but not in parent[ " + this.parent.streamId + "]'s pseudoTimeQueue";
