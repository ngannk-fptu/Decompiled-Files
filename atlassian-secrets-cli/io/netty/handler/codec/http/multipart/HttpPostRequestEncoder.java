...
implements ChunkedInput<HttpContent> {
        ...
        if (data instanceof Attribute) {
            ...
            if (this.duringMixedMode) {
                ...
                if (!fileUpload.getFilename().isEmpty()) {
                    replacement.append("; ").append(HttpHeaderValues.FILENAME).append("=\"").append(this.currentFileUpload.getFilename()).append('\"');
                }
                replacement.append("\r\n");
    ...
    public HttpRequest finalizeRequest() throws ErrorDataEncoderException {
        ...
        if (realSize > 8096L || this.isMultipart) {
            ...
            if (transferEncoding != null) {
                ...
                for (CharSequence charSequence : transferEncoding) {
    ...
    private HttpContent encodeNextChunkUrlEncoded(int sizeleft) throws ErrorDataEncoderException {
        ...
        if (this.isKey) {
            ...
            buffer = Unpooled.wrappedBuffer(key.getBytes(this.charset));
            this.isKey = false;
            this.currentBuffer = this.currentBuffer == null ? Unpooled.wrappedBuffer(buffer, Unpooled.wrappedBuffer("=".getBytes(this.charset))) : Unpooled.wrappedBuffer(this.currentBuffer, buffer, Unpooled.wrappedBuffer("=".getBytes(this.charset)));
        ...
        if (buffer.readableBytes() < size) {
            this.isKey = true;
            ByteBuf byteBuf = delimiter = this.iterator.hasNext() ? Unpooled.wrappedBuffer("&".getBytes(this.charset)) : null;
        }
        if (buffer.capacity() == 0) {
            this.currentData = null;
