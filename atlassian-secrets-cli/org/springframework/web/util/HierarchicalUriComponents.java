...
extends UriComponents {
    ...
    implements PathComponent {
        ...
        @Override
        public List<String> getPathSegments() {
            String[] segments = StringUtils.tokenizeToStringArray(this.getPath(), HierarchicalUriComponents.PATH_DELIMITER_STRING);
    ...
    implements BiFunction<String, Type, String> {
        ...
        public String apply(String source, Type type) {
            ...
            for (char c : source.toCharArray()) {
                if (c == '{' && ++level == 1) {
                    this.encodeAndAppendCurrentLiteral(type);
                }
                if (c == '}' && level > 0) {
                    this.currentVariable.append('}');
                    if (--level != 0) continue;
                    this.output.append((CharSequence)this.currentVariable);
                    this.clear(this.currentVariable);
                    continue;
                }
                if (level > 0) {
