...
extends TemplateAwareExpressionParser {
    ...
    private SpelNodeImpl eatPrimaryExpression() {
        ...
        return new CompoundExpression(this.toPos(start.getStartPosition(), ((SpelNodeImpl)nodes.get(nodes.size() - 1)).getEndPosition()), nodes.toArray(new SpelNodeImpl[0]));
    }
    ...
    private boolean maybeEatInlineListOrMap() {
        ...
        if (this.peekToken(TokenKind.RCURLY, true)) {
            ...
            if (this.peekToken(TokenKind.RCURLY)) {
                ...
                closingCurly = this.eatToken(TokenKind.RCURLY);
                expr = new InlineList(this.toPos(t.startPos, closingCurly.endPos), elements.toArray(new SpelNodeImpl[0]));
                ...
                closingCurly = this.eatToken(TokenKind.RCURLY);
                expr = new InlineMap(this.toPos(t.startPos, closingCurly.endPos), elements.toArray(new SpelNodeImpl[0]));
            } else {
                throw this.internalException(t.startPos, SpelMessage.OOD, new Object[0]);
            }
        }
    ...
    private boolean maybeEatSelection(boolean nullSafeNavigation) {
        ...
        SpelNodeImpl expr = this.eatExpression();
        if (expr == null) {
            throw this.internalException(this.toPos(t), SpelMessage.MISSING_SELECTION_EXPRESSION, new Object[0]);
        }
        this.eatToken(TokenKind.RSQUARE);
    ...
    private boolean maybeEatConstructorReference() {
        if (this.peekIdentifierToken("new")) {
            ...
            if (this.peekToken(TokenKind.LSQUARE)) {
                ...
                this.push(new ConstructorReference(this.toPos(newToken), dimensions.toArray(new SpelNodeImpl[0]), nodes.toArray(new SpelNodeImpl[0])));
            } else {
                this.eatConstructorArgs(nodes);
                this.push(new ConstructorReference(this.toPos(newToken), nodes.toArray(new SpelNodeImpl[0])));
            }
            return true;
        }
        return false;
