...
implements ResourcePatternResolver {
                ...
                if (classLoader instanceof URLClassLoader) {
                    try {
                        for (URL url : ((URLClassLoader)classLoader).getURLs()) {
                            try {
                                UrlResource jarResource = new UrlResource("jar:" + url + "!/");
            ...
            try {
                ...
                for (String path : StringUtils.delimitedListToStringArray(javaClassPathProperty, System.getProperty("path.separator"))) {
                    try {
                        ...
                        if (result.contains(jarResource = new UrlResource("jar:file:" + filePath + "!/")) || this.hasDuplicate(filePath, result) || !jarResource.exists()) continue;
                        result.add(jarResource);
                    }
                    catch (MalformedURLException ex) {
                        if (!logger.isDebugEnabled()) continue;
    ...
    protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set<File> result) throws IOException {
        ...
        for (Object content : dirContents) {
            String currPath = StringUtils.replace(((File)content).getAbsolutePath(), File.separator, "/");
            if (((File)content).isDirectory() && this.getPathMatcher().matchStart(fullPattern, currPath + "/")) {
