...
BeanFactoryAware {
    ...
    public boolean matches(Method method, @Nullable Class<?> targetClass, boolean hasIntroductions) {
        ...
        return !walker.testsSubtypeSensitiveVars() || targetClass != null && walker.testTargetInstanceOfResidue(targetClass);
    }
    ...
    private ShadowMatch getTargetShadowMatch(Method method, @Nullable Class<?> targetClass) {
        ...
        if (targetClass != null && targetMethod.getDeclaringClass().isInterface() && (ifcs = ClassUtils.getAllInterfacesForClassAsSet(targetClass)).size() > 1) {
            try {
