...
implements VaultWrappingOperations {
    ...
    public WrappedMetadata lookup(VaultToken token) {
        Assert.notNull((Object)token, "token VaultToken not be null");
    ...
    public VaultResponse read(VaultToken token) {
        return this.doUnwrap(token, (restOperations, entity) -> (VaultResponse)restOperations.exchange("sys/wrapping/unwrap", HttpMethod.POST, (HttpEntity<?>)entity, VaultResponse.class, new Object[0]).getBody());
    }
    ...
    public <T> VaultResponseSupport<T> read(VaultToken token, Class<T> responseType) {
        ...
        return this.doUnwrap(token, (restOperations, entity) -> (VaultResponseSupport)restOperations.exchange("sys/wrapping/unwrap", HttpMethod.POST, (HttpEntity<?>)entity, ref, new Object[0]).getBody());
    }
    ...
    private <T extends VaultResponseSupport<?>> T doUnwrap(VaultToken token, BiFunction<RestOperations, HttpEntity<?>, T> requestFunction) {
        return (T)this.vaultOperations.doWithVault(restOperations -> {
            try {
                return (VaultResponseSupport)requestFunction.apply(restOperations, new HttpEntity(VaultHttpHeaders.from(token)));
            }
    ...
    public WrappedMetadata wrap(Object body, Duration duration) {
        ...
        Assert.notNull((Object)duration, "TTL duration must not be null");
        VaultResponse response = this.vaultOperations.doWithSession(restOperations -> {
            ...
            return (VaultResponse)restOperations.exchange("sys/wrapping/wrap", HttpMethod.POST, new HttpEntity<Object>(body, headers), VaultResponse.class, new Object[0]).getBody();
        });
        Map<String, String> wrapInfo = response.getWrapInfo();
        return VaultWrappingTemplate.getWrappedMetadata(wrapInfo, VaultToken.of(wrapInfo.get("token")));
    }
