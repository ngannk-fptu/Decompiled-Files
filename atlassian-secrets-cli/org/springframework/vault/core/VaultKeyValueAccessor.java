...
implements VaultKeyValueOperationsSupport {
    ...
    <I, T> T doRead(String path, Class<I> deserializeAs, BiFunction<VaultResponseSupport<?>, I, T> mappingFunction) {
        ...
        if (response != null) {
            ...
            response.setMetadata(this.mapper.convertValue((Object)jsonMeta, new TypeReference<Map<String, Object>>(){}));
    ...
    VaultResponse doWrite(String path, Object body) {
        Assert.hasText(path, "Path must not be empty");
        try {
            return this.vaultOperations.doWithSession(restOperations -> (VaultResponse)restOperations.exchange(path, HttpMethod.POST, new HttpEntity<Object>(body), VaultResponse.class, new Object[0]).getBody());
        }
        catch (HttpStatusCodeException e) {
