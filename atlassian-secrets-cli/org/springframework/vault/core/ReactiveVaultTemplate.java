...
implements ReactiveVaultOperations {
    ...
    public ReactiveVaultTemplate(VaultEndpointProvider endpointProvider, ClientHttpConnector connector, VaultTokenSupplier vaultTokenSupplier) {
        Assert.notNull((Object)endpointProvider, "VaultEndpointProvider must not be null");
        Assert.notNull((Object)connector, "ClientHttpConnector must not be null");
        Assert.notNull((Object)vaultTokenSupplier, "VaultTokenSupplier must not be null");
    ...
    public ReactiveVaultTemplate(WebClientBuilder webClientBuilder) {
        Assert.notNull((Object)webClientBuilder, "WebClientBuilder must not be null");
    ...
    private static <T> Mono<T> mapOtherwise(ClientResponse response, String path, HttpMethod method) {
        ...
        return response.bodyToMono(String.class).flatMap(body -> {
            ...
            return Mono.error((Throwable)VaultResponses.buildException(response.statusCode(), path, error));
        });
    }
