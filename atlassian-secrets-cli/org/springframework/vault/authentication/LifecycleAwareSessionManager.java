...
DisposableBean {
    ...
    protected void revoke(VaultToken token) {
        try {
            ...
            this.restOperations.postForObject("auth/token/revoke-self", new HttpEntity(VaultHttpHeaders.from(token)), Map.class, new Object[0]);
    ...
    private RenewOutcome tryRenewToken() {
        ...
        try {
            ...
            this.dispatch(new LoginTokenRenewalFailedEvent(tokenWrapper.getToken(), (Throwable)exception));
    ...
    private RenewOutcome doRenew(TokenWrapper wrapper) {
        ...
        VaultResponse vaultResponse = this.restOperations.postForObject("auth/token/renew-self", new HttpEntity(VaultHttpHeaders.from(wrapper.token)), VaultResponse.class, new Object[0]);
    ...
    private void doGetSessionToken() {
        ...
        try {
            ...
            this.dispatch(new LoginFailedEvent(this.clientAuthentication, e));
            throw e;
        }
        ...
        if (this.isTokenSelfLookupEnabled() && !ClassUtils.isAssignableValue(LoginToken.class, token)) {
            try {
                ...
                this.dispatch(new AuthenticationErrorEvent(token, e));
            }
        }
    ...
    private void scheduleRenewal() {
        ...
        Optional<TokenWrapper> token = this.getToken();
        token.ifPresent(tokenWrapper -> this.getTaskScheduler().schedule(task, this.createTrigger((TokenWrapper)tokenWrapper)));
    }
