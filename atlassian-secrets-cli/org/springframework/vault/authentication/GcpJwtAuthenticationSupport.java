...
public abstract class GcpJwtAuthenticationSupport {
    ...
    GcpJwtAuthenticationSupport(RestOperations restOperations) {
        Assert.notNull((Object)restOperations, "Vault RestOperations must not be null");
    ...
    VaultToken doLogin(String authenticationName, String signedJwt, String path, String role) {
        ...
        try {
            VaultResponse response = this.restOperations.postForObject(AuthenticationUtil.getLoginPath(path), login, VaultResponse.class, new Object[0]);
            Assert.state(response != null && response.getAuth() != null, "Auth field must not be null");
            if (logger.isDebugEnabled()) {
                if (response.getAuth().get("metadata") instanceof Map) {
                    Map metadata = (Map)response.getAuth().get("metadata");
                    logger.debug(String.format("Login successful using %s authentication for user id %s", authenticationName, metadata.get("service_account_email")));
                } else {
