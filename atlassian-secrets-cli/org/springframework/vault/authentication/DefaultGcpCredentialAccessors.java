...
GcpServiceAccountIdAccessor
{
    ...
    public String getServiceAccountId(GoogleCredential credential) {
        Assert.notNull((Object)credential, "GoogleCredential must not be null");
        Assert.notNull((Object)credential.getServiceAccountId(), "The configured GoogleCredential does not represent a service account. Configure the service account id with GcpIamAuthenticationOptionsBuilder#serviceAccountId(String).");
    ...
    public String getProjectId(GoogleCredential credential) {
        Assert.notNull((Object)credential, "GoogleCredential must not be null");
        return StringUtils.isEmpty(credential.getServiceAccountProjectId()) ? "-" : credential.getServiceAccountProjectId();
    }
}
