...
EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry registry) {
        ...
        if (!registry.isBeanNameInUse("VaultPropertySourceRegistrar")) {
            registry.registerBeanDefinition("VaultPropertySourceRegistrar", BeanDefinitionBuilder.rootBeanDefinition(VaultPropertySourceRegistrar.class).setRole(2).getBeanDefinition());
        }
        ...
        for (AnnotationAttributes propertySource : propertySources) {
            ...
            VaultPropertySource.Renewal renewal = (VaultPropertySource.Renewal)((Object)propertySource.getEnum("renewal"));
            ...
            PropertyTransformer propertyTransformer = StringUtils.hasText(propertyNamePrefix) ? PropertyTransformers.propertyNamePrefix(propertyNamePrefix) : PropertyTransformers.noop();
            block1: for (String propertyPath : paths) {
                if (!StringUtils.hasText(propertyPath)) continue;
