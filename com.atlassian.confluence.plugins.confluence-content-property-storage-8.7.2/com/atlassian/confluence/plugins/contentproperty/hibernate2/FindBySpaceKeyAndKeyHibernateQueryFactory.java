...
public class FindBySpaceKeyAndKeyHibernateQueryFactory
implements HibernateContentQueryFactory {
    public Query getQuery(EntityManager entityManager, Object ... parameters) {
        String spacekey = (String)Preconditions.checkNotNull((Object)((String)parameters[0]));
        String key = (String)Preconditions.checkNotNull((Object)((String)parameters[1]));
        Query query = entityManager.createQuery("select content from CustomContentEntityObject content where content.originalVersion is null and     content.contentStatus = 'current' and     content.pluginModuleKey = :pluginModuleKey and     content.space.key = :spaceKey and     content.title = :key and     content.containerContent.id is null");
