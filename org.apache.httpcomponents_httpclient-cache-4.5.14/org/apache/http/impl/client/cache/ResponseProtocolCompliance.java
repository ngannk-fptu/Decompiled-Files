...
class ResponseProtocolCompliance {
    ...
    public void ensureProtocolCompliance(HttpRequestWrapper request, HttpResponse response) throws IOException {
        if (this.backendResponseMustNotHaveBody((HttpRequest)request, response)) {
        ...
        this.ensurePartialContentIsNotSentToAClientThatDidNotRequestIt((HttpRequest)request, response);
        this.ensure200ForOPTIONSRequestWithNoBodyHasContentLengthZero((HttpRequest)request, response);
    ...
    private void transferEncodingIsNotReturnedTo1_0Client(HttpRequestWrapper request, HttpResponse response) {
        HttpRequest originalRequest = request.getOriginal();
        if (originalRequest.getProtocolVersion().compareToVersion((ProtocolVersion)HttpVersion.HTTP_1_1) >= 0) {
            return;
        }
