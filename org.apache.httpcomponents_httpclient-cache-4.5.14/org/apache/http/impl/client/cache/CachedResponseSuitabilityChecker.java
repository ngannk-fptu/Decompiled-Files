...
class CachedResponseSuitabilityChecker {
    ...
    public boolean canCachedResponseBeUsed(HttpHost host, HttpRequest request, HttpCacheEntry entry, Date now) {
        if (!this.isFreshEnough(entry, request, now)) {
            this.log.trace((Object)"Cache entry was not fresh enough");
            return false;
        }
        ...
        if (this.hasUnsupportedConditionalHeaders(request)) {
            this.log.debug((Object)"Request contained conditional headers we don't handle");
            return false;
        }
        ...
        if (this.hasUnsupportedCacheEntryForGet(request, entry)) {
            this.log.debug((Object)"HEAD response caching enabled but the cache entry does not contain a request method, entity or a 204 response");
            return false;
        }
        for (Header ccHdr : request.getHeaders("Cache-Control")) {
            for (HeaderElement elt : ccHdr.getElements()) {
                if ("no-cache".equals(elt.getName())) {
                    this.log.trace((Object)"Response contained NO CACHE directive, cache was not suitable");
                    return false;
                }
                if ("no-store".equals(elt.getName())) {
                    this.log.trace((Object)"Response contained NO STORE directive, cache was not suitable");
                    return false;
                }
                if ("max-age".equals(elt.getName())) {
                    try {
                        ...
                        if (this.validityStrategy.getCurrentAgeSecs(entry, now) > (long)maxage) {
                            this.log.trace((Object)"Response from cache was NOT suitable due to max age");
                            return false;
                        }
                    }
                ...
                if ("max-stale".equals(elt.getName())) {
                    try {
                        ...
                        if (this.validityStrategy.getFreshnessLifetimeSecs(entry) > (long)maxstale) {
                            this.log.trace((Object)"Response from cache was not suitable due to Max stale freshness");
                            return false;
                        }
                    }
                    catch (NumberFormatException ex) {
                ...
                try {
                    ...
                    this.log.trace((Object)"Response from cache was not suitable due to min fresh freshness requirement");
                    return false;
                }
                catch (NumberFormatException ex) {
                    this.log.debug((Object)("Response from cache was malformed: " + ex.getMessage()));
        ...
        this.log.trace((Object)"Response from cache was suitable");
        return true;
    }
    ...
    private boolean lastModifiedValidatorMatches(HttpRequest request, HttpCacheEntry entry, Date now) {
        ...
        for (Header h : request.getHeaders("If-Modified-Since")) {
            Date ifModifiedSince = DateUtils.parseDate((String)h.getValue());
    ...
    private boolean hasValidDateField(HttpRequest request, String headerName) {
        ...
        if (i$ < len$) {
            Header h = arr$[i$];
            Date date = DateUtils.parseDate((String)h.getValue());
            return date != null;
        }
        return false;
    }
