...
extends OperationDescriptorImpl {
    ...
    private static final Class[] paramClasses = new Class[]{array$I == null ? (array$I = XorConstDescriptor.class$("[I")) : array$I};
    ...
    public boolean validateArguments(String modeName, ParameterBlock args, StringBuffer message) {
        ...
        if (dtype != 0 && dtype != 1 && dtype != 2 && dtype != 3) {
            message.append(this.getName() + " " + JaiI18N.getString("XorConstDescriptor2"));
            return false;
        }
        int length = ((int[])args.getObjectParameter(0)).length;
        if (length < 1) {
            message.append(this.getName() + " " + JaiI18N.getString("XorConstDescriptor3"));
            return false;
        }
        return true;
    }
    ...
    public static RenderedOp create(RenderedImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockJAI pb = new ParameterBlockJAI("XorConst", "rendered");
    ...
    public static RenderableOp createRenderable(RenderableImage source0, int[] constants, RenderingHints hints) {
        ParameterBlockJAI pb = new ParameterBlockJAI("XorConst", "renderable");
        pb.setSource("source0", source0);
        pb.setParameter("constants", constants);
