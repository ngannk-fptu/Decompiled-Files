        ...
        switch (dstMA.getDataType()) {
            ...
            case 3: {
                for (int i = 0; i < dstMLI.length; ++i) {
                    ...
                    int[] mlConstants = dstMA.getIntParameters(i, this.constantsInt);
                    Image.Thresh5((mediaLibImage)dstMLI[i], (mediaLibImage)srcMLI[i], (int[])mlHigh, (int[])mlLow, (int[])mlConstants);
                }
                break;
            }
            case 4: 
            case 5: {
                for (int i = 0; i < dstMLI.length; ++i) {
                    ...
                    double[] mlConstants = dstMA.getDoubleParameters(i, this.constants);
                    Image.Thresh5_Fp((mediaLibImage)dstMLI[i], (mediaLibImage)srcMLI[i], (double[])mlHigh, (double[])mlLow, (double[])mlConstants);
                }
                break;
            }
            default: {
