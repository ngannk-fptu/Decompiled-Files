...
final class MlibAddConstOpImage
extends PointOpImage {
    private double[] constants;

    public MlibAddConstOpImage(RenderedImage source, Map config, ImageLayout layout, double[] constants) {
    ...
    protected void computeRect(Raster[] sources, WritableRaster dest, Rectangle destRect) {
        ...
        switch (dstAccessor.getDataType()) {
            ...
            case 3: {
                int i;
                int[] constInt = new int[this.constants.length];
                for (i = 0; i < this.constants.length; ++i) {
                    constInt[i] = ImageUtil.clampRoundInt(this.constants[i]);
                }
                for (i = 0; i < dstML.length; ++i) {
                    int[] mlconstants = dstAccessor.getIntParameters(i, constInt);
                    Image.ConstAdd((mediaLibImage)dstML[i], (mediaLibImage)srcML[i], (int[])mlconstants);
                }
                break;
            }
            case 4: 
            case 5: {
                for (int i = 0; i < dstML.length; ++i) {
                    ...
                    Image.ConstAdd_Fp((mediaLibImage)dstML[i], (mediaLibImage)srcML[i], (double[])mlconstants);
                }
                break;
            }
            default: {
