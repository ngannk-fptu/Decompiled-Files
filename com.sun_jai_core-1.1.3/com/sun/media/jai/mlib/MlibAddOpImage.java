...
extends PointOpImage {
    ...
    public MlibAddOpImage(RenderedImage source1, RenderedImage source2, Map config, ImageLayout layout) {
        super(source1, source2, MlibAddOpImage.layoutHelper(layout), config, true);
        ...
        switch (dstAccessor.getDataType()) {
            ...
            case 3: {
                for (int i = 0; i < dstML.length; ++i) {
                    Image.Add((mediaLibImage)dstML[i], (mediaLibImage)srcML1[i], (mediaLibImage)srcML2[i]);
                }
                break;
            }
            case 4: 
            case 5: {
                for (int i = 0; i < dstML.length; ++i) {
                    Image.Add_Fp((mediaLibImage)dstML[i], (mediaLibImage)srcML1[i], (mediaLibImage)srcML2[i]);
                }
                break;
            }
            default: {
