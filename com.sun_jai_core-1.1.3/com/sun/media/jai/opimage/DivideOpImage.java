...
extends PointOpImage {
    ...
    public DivideOpImage(RenderedImage source1, RenderedImage source2, Map config, ImageLayout layout) {
        ...
        if (layout != null && layout.isValid(256) && (numBandsDst = (sm = layout.getSampleModel(null)).getNumBands()) > 1 && (numBands1 > 1 && numBands2 == 1 || numBands1 == 1 && numBands2 > 1)) {
        ...
        if (this.sampleModel.getTransferType() == 0) {
            ...
            for (int j = 0; j < 256; ++j) {
                byte[] array = this.divideTableByte[j];
                array[0] = j > 0 ? -1 : 0;
                for (int i = 1; i < 256; ++i) {
        ...
        while (b < dstNumBands) {
            ...
            for (int h = 0; h < dstHeight; ++h) {
                ...
                for (int w = 0; w < dstWidth; ++w) {
                    d[dstPixelOffset] = this.divideTableByte[s1[src1PixelOffset] & 0xFF][s2[src2PixelOffset] & 0xFF];
    ...
    private void ushortLoop(int dstNumBands, int dstWidth, int dstHeight, int src1LineStride, int src1PixelStride, int[] src1BandOffsets, short[][] src1Data, int src2LineStride, int src2PixelStride, int[] src2BandOffsets, short[][] src2Data, int dstLineStride, int dstPixelStride, int[] dstBandOffsets, short[][] dstData) {
        ...
        while (b < dstNumBands) {
            ...
            for (int h = 0; h < dstHeight; ++h) {
                ...
                dstLineOffset += dstLineStride;
                for (int w = 0; w < dstWidth; ++w) {
                    float f1 = s1[src1PixelOffset] & 0xFFFF;
                    float f2 = s2[src2PixelOffset] & 0xFFFF;
                    d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? -1 : ImageUtil.clampRoundUShort(f1 / f2));
                    src1PixelOffset += src1PixelStride;
                    src2PixelOffset += src2PixelStride;
                    dstPixelOffset += dstPixelStride;
                }
    ...
    private void shortLoop(int dstNumBands, int dstWidth, int dstHeight, int src1LineStride, int src1PixelStride, int[] src1BandOffsets, short[][] src1Data, int src2LineStride, int src2PixelStride, int[] src2BandOffsets, short[][] src2Data, int dstLineStride, int dstPixelStride, int[] dstBandOffsets, short[][] dstData) {
        ...
        while (b < dstNumBands) {
            ...
            for (int h = 0; h < dstHeight; ++h) {
                ...
                dstLineOffset += dstLineStride;
                for (int w = 0; w < dstWidth; ++w) {
                    float f1 = s1[src1PixelOffset];
                    float f2 = s2[src2PixelOffset];
                    d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? (f1 < 0.0f ? Short.MIN_VALUE : Short.MAX_VALUE) : ImageUtil.clampRoundShort(f1 / f2));
                    src1PixelOffset += src1PixelStride;
                    src2PixelOffset += src2PixelStride;
                    dstPixelOffset += dstPixelStride;
                }
            ...
            case 0: {
                ...
                while (b < dstNumBands) {
                    ...
                    for (int h = 0; h < dstHeight; ++h) {
                        ...
                        dstLineOffset += dstLineStride;
                        for (int w = 0; w < dstWidth; ++w) {
                            float f1 = s1[src1PixelOffset] & 0xFF;
                            float f2 = s2[src2PixelOffset] & 0xFF;
                            d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? 255 : (int)ImageUtil.clampRoundByte(f1 / f2));
                            src1PixelOffset += src1PixelStride;
                            src2PixelOffset += src2PixelStride;
                            dstPixelOffset += dstPixelStride;
                        }
                ...
                while (b < dstNumBands) {
                    ...
                    for (int h = 0; h < dstHeight; ++h) {
                        ...
                        for (int w = 0; w < dstWidth; ++w) {
                            float f1 = s1[src1PixelOffset] & 0xFFFF;
                            float f2 = s2[src2PixelOffset] & 0xFFFF;
                            d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? -1 : (int)ImageUtil.clampRoundUShort(f1 / f2));
                            src1PixelOffset += src1PixelStride;
                            src2PixelOffset += src2PixelStride;
                            dstPixelOffset += dstPixelStride;
                        }
                ...
                while (b < dstNumBands) {
                    ...
                    for (int h = 0; h < dstHeight; ++h) {
                        ...
                        for (int w = 0; w < dstWidth; ++w) {
                            float f1 = s1[src1PixelOffset];
                            float f2 = s2[src2PixelOffset];
                            d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? (f1 < 0.0f ? Short.MIN_VALUE : Short.MAX_VALUE) : (int)ImageUtil.clampRoundShort(f1 / f2));
                            src1PixelOffset += src1PixelStride;
                            src2PixelOffset += src2PixelStride;
                            dstPixelOffset += dstPixelStride;
                        }
                ...
                while (b < dstNumBands) {
                    ...
                    for (int h = 0; h < dstHeight; ++h) {
                        ...
                        for (int w = 0; w < dstWidth; ++w) {
                            float f1 = s1[src1PixelOffset];
                            float f2 = s2[src2PixelOffset];
                            d[dstPixelOffset] = f1 == 0.0f ? 0 : (f2 == 0.0f ? (f1 < 0.0f ? Integer.MIN_VALUE : Integer.MAX_VALUE) : ImageUtil.clampRoundInt(f1 / f2));
                            src1PixelOffset += src1PixelStride;
                            src2PixelOffset += src2PixelStride;
                            dstPixelOffset += dstPixelStride;
                        }
