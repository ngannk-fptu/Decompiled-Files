...
public class NettyRequestMetrics {
    ...
    private static void writeHttp2RequestMetrics(MetricCollector metricCollector, Channel channel, Http2Connection http2Connection) {
        int streamId = ((Http2FrameStream)channel.attr(ChannelAttributeKey.HTTP2_FRAME_STREAM).get()).id();
        Http2Stream stream = http2Connection.stream(streamId);
        metricCollector.reportMetric(Http2Metric.LOCAL_STREAM_WINDOW_SIZE_IN_BYTES, (Object)((Http2LocalFlowController)http2Connection.local().flowController()).windowSize(stream));
        metricCollector.reportMetric(Http2Metric.REMOTE_STREAM_WINDOW_SIZE_IN_BYTES, (Object)((Http2RemoteFlowController)http2Connection.remote().flowController()).windowSize(stream));
    }
