...
public final class NettyUtils {
    ...
    public static String closedChannelMessage(Channel channel) {
        ChannelDiagnostics channelDiagnostics = channel != null && channel.attr(ChannelAttributeKey.CHANNEL_DIAGNOSTICS) != null ? (ChannelDiagnostics)channel.attr(ChannelAttributeKey.CHANNEL_DIAGNOSTICS).get() : null;
        ChannelDiagnostics parentChannelDiagnostics = channel != null && channel.parent() != null && channel.parent().attr(ChannelAttributeKey.CHANNEL_DIAGNOSTICS) != null ? (ChannelDiagnostics)channel.parent().attr(ChannelAttributeKey.CHANNEL_DIAGNOSTICS).get() : null;
    ...
    public static void warnIfNotInEventLoop(EventLoop loop) {
        ...
        if (!loop.inEventLoop()) {
            ...
            log.warn(() -> "Execution is happening outside of the expected event loop.", (Throwable)exception);
        }
    }

    public static <T> AttributeKey<T> getOrCreateAttributeKey(String attr) {
        if (AttributeKey.exists((String)attr)) {
            return AttributeKey.valueOf((String)attr);
        }
        return AttributeKey.newInstance((String)attr);
    }
