...
public final class SslContextProvider {
    ...
    public SslContext sslContext() {
        try {
            return SslContextBuilder.forClient().sslProvider(this.sslProvider).ciphers(this.getCiphers(), (CipherSuiteFilter)SupportedCipherSuiteFilter.INSTANCE).trustManager(this.trustManagerFactory).keyManager(this.keyManagerFactory).build();
        }
    ...
    private TrustManagerFactory getTrustManager(NettyConfiguration configuration) {
        ...
        Validate.isTrue((tlsTrustManagersProvider == null || !configuration.trustAllCertificates() ? 1 : 0) != 0, (String)"A TlsTrustManagerProvider can't be provided if TrustAllCertificates is also set", (Object[])new Object[0]);
