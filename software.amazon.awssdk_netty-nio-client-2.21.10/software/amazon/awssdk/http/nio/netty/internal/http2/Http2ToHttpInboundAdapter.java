...
extends SimpleChannelInboundHandler<Http2Frame> {
    ...
    private void onHeadersRead(Http2HeadersFrame headersFrame, ChannelHandlerContext ctx) throws Http2Exception {
        HttpResponse httpResponse = HttpConversionUtil.toHttpResponse((int)headersFrame.stream().id(), (Http2Headers)headersFrame.headers(), (boolean)true);
        ctx.fireChannelRead((Object)httpResponse);
        if (HttpStatusFamily.of((int)httpResponse.status().code()) == HttpStatusFamily.SERVER_ERROR) {
    ...
    private void fireConnectionExceptionForServerError(ChannelHandlerContext ctx) {
        if (ctx.channel().parent() != null) {
            ...
            parent.pipeline().fireExceptionCaught((Throwable)new Http2ConnectionTerminatingException("A 5xx server error occurred on an Http2 stream " + ctx.channel()));
        }
    }
    ...
    private void onDataRead(Http2DataFrame dataFrame, ChannelHandlerContext ctx) throws Http2Exception {
        ...
        if (!dataFrame.isEndStream()) {
            ctx.fireChannelRead((Object)new DefaultHttpContent(data));
        } else {
            ctx.fireChannelRead((Object)new DefaultLastHttpContent(data));
        }
    }

    private void onRstStreamRead(Http2ResetFrame resetFrame, ChannelHandlerContext ctx) throws Http2Exception {
        ctx.fireExceptionCaught((Throwable)new Http2ResetException(resetFrame.errorCode()));
    }

    public static final class Http2ResetException
    extends IOException {
        Http2ResetException(long errorCode) {
            super(String.format("Connection reset. Error - %s(%d)", Http2Error.valueOf((long)errorCode).name(), errorCode));
        }
    }
}
