...
implements SdkChannelPool {
    ...
    private void configureProtocol(Channel newChannel, Protocol protocol) {
        if (Protocol.HTTP1_1 == protocol) {
            IdleConnectionCountingChannelPool idleConnectionMetricChannelPool = new IdleConnectionCountingChannelPool((EventExecutor)this.eventLoop, this.delegatePool);
            this.protocolImpl = BetterFixedChannelPool.builder().channelPool(idleConnectionMetricChannelPool).executor((EventExecutor)this.eventLoop).acquireTimeoutAction(BetterFixedChannelPool.AcquireTimeoutAction.FAIL).acquireTimeoutMillis(this.configuration.connectionAcquireTimeoutMillis()).maxConnections(this.maxConcurrency).maxPendingAcquires(this.configuration.maxPendingConnectionAcquires()).build();
        } else {
        ...
        this.delegatePool.release(newChannel).addListener(NettyUtils.runOrPropagate(this.protocolImplPromise, () -> this.protocolImplPromise.trySuccess((Object)this.protocolImpl)));
    }
