...
extends SpelNodeImpl {
    ...
    private TypedValue getValueInternal(TypedValue contextObject, EvaluationContext evalContext, boolean isAutoGrowNullReferences) throws EvaluationException {
        ...
        if (result.getValue() == null && isAutoGrowNullReferences && this.nextChildIs(Indexer.class, PropertyOrFieldReference.class)) {
            ...
            Assert.state((resultDescriptor != null ? 1 : 0) != 0, (String)"No result type");
            if (List.class == resultDescriptor.getType()) {
                ...
                try {
                    if (this.isWritableProperty(this.name, contextObject, evalContext)) {
                        ...
                        Object newObject = ReflectionUtils.accessibleConstructor((Class)clazz, (Class[])new Class[0]).newInstance(new Object[0]);
    ...
    public TypedValue setValueInternal(ExpressionState state, Supplier<TypedValue> valueSupplier) throws EvaluationException {
        ...
        this.writeProperty(state.getActiveContextObject(), state.getEvaluationContext(), this.name, typedValue.getValue());
        return typedValue;
    }
