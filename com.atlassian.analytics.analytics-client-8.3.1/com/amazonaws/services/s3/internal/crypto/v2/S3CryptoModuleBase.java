...
extends S3CryptoModule<T> {
    ...
    private ContentCryptoMaterial buildContentCryptoMaterial(EncryptionMaterials materials, AmazonWebServiceRequest req) {
        ...
        if (materials.isKMSEnabled()) {
            ...
            Map<String, String> encryptionContext = KMSMaterialsHandler.createKMSContextMaterialsDescription(KMSMaterialsHandler.mergeMaterialsDescription((KMSEncryptionMaterials)materials, req), cekAlgo);
            ...
            return ContentCryptoMaterial.wrap(cek, iv, this.contentCryptoScheme, this.cryptoConfig.getCryptoProvider(), this.cryptoConfig.getAlwaysUseCryptoProvider(), new SecuredCEK(keyBlob, InternalKeyWrapAlgorithm.KMS, encryptionContext));
        }
    ...
    private ContentCryptoMaterial contentCryptoMaterialOf(Map<String, String> userMetadata, ObjectMetadata objectMetadata) {
        if (this.storesMetadataInObjectHeader(objectMetadata)) {
            return ContentCryptoMaterial.fromObjectMetadata(userMetadata, this.kekMaterialsProvider, this.cryptoConfig, false, this.kms);
        }
        return ContentCryptoMaterial.fromInstructionFile(userMetadata, this.kekMaterialsProvider, this.cryptoConfig, false, this.kms);
    }
    ...
    private Map<String, String> getUserMetadata(S3ObjectId objectId, ObjectMetadata objectMetadata) {
        ...
        return this.getInstructionFileMetadata(objectId);
    }
