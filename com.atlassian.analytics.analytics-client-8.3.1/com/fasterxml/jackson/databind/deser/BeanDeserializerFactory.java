...
implements Serializable {
    ...
    public JsonDeserializer<Object> buildThrowableDeserializer(DeserializationContext ctxt, JavaType type, BeanDescription beanDesc) throws JsonMappingException {
        ...
        while (it.hasNext()) {
            SettableBeanProperty prop2 = it.next();
            if (!"setCause".equals(prop2.getMember().getName())) continue;
            it.remove();
            break;
        }
        if ((am = beanDesc.findMethod("initCause", INIT_CAUSE_PARAMS)) != null && (prop = this.constructSettableProperty(ctxt, beanDesc, (BeanPropertyDefinition)(propDef = SimpleBeanPropertyDefinition.construct(ctxt.getConfig(), am, new PropertyName("cause"))), am.getParameterType(0))) != null) {
