...
extends IonWriterSystem {
    ...
    protected IonWriterSystemText(SymbolTable defaultSystemSymtab, PrivateIonTextWriterBuilder options, PrivateFastAppendable out) {
        super(defaultSystemSymtab, options.getInitialIvmHandling(), options.getIvmMinimizing());
        this._output = PrivateIonTextAppender.forFastAppendable(out, options.getCharset());
    ...
    private void writeSidLiteral(int sid) throws IOException {
        assert (sid > 0);
        boolean asString = this._options._symbol_as_string;
        if (asString) {
            this._output.appendAscii('\"');
        }
        this._output.appendAscii('$');
