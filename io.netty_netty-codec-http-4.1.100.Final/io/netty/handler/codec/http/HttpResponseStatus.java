...
implements Comparable<HttpResponseStatus> {
    ...
    public static final HttpResponseStatus EARLY_HINTS = HttpResponseStatus.newStatus(103, "Early Hints");
    ...
    public static HttpResponseStatus parseLine(CharSequence line) {
        return line instanceof AsciiString ? HttpResponseStatus.parseLine((AsciiString)line) : HttpResponseStatus.parseLine(line.toString());
    }
    ...
    public static HttpResponseStatus parseLine(AsciiString line) {
        try {
            ...
            return space == -1 ? HttpResponseStatus.valueOf(line.parseInt()) : HttpResponseStatus.valueOf(line.parseInt(0, space), line.toString(space + 1));
        }
        catch (Exception e) {
            throw new IllegalArgumentException("malformed status line: " + line, e);
        }
    ...
    private HttpResponseStatus(int code, String reasonPhrase, boolean bytes) {
        ObjectUtil.checkPositiveOrZero((int)code, (String)"code");
        ObjectUtil.checkNotNull((Object)reasonPhrase, (String)"reasonPhrase");
        ...
        this.codeAsText = new AsciiString((CharSequence)codeString);
        this.reasonPhrase = reasonPhrase;
        this.bytes = (byte[])(bytes ? (codeString + ' ' + reasonPhrase).getBytes(CharsetUtil.US_ASCII) : null);
    }
    ...
    void encode(ByteBuf buf) {
        if (this.bytes == null) {
            ...
            buf.writeByte(32);
            buf.writeCharSequence((CharSequence)this.reasonPhrase, CharsetUtil.US_ASCII);
        } else {
