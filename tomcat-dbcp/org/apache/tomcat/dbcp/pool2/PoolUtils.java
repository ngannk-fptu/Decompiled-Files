...
public final class PoolUtils {
    ...
    public static <K, V> Map<K, TimerTask> checkMinIdle(KeyedObjectPool<K, V> keyedPool, Collection<K> keys, int minIdle, long periodMillis) throws IllegalArgumentException {
        ...
        for (K key : keys) {
            TimerTask task = PoolUtils.checkMinIdle(keyedPool, key, minIdle, periodMillis);
            tasks.put(key, task);
        }
        return tasks;
    }
    ...
    public static <K, V> TimerTask checkMinIdle(KeyedObjectPool<K, V> keyedPool, K key, int minIdle, long periodMillis) throws IllegalArgumentException {
        ...
        PoolUtils.getMinIdleTimer().schedule(task, 0L, periodMillis);
        return task;
    }
    ...
    extends ErodingKeyedObjectPool<K, V> {
        ...
        ErodingPerKeyKeyedObjectPool(KeyedObjectPool<K, V> keyedPool, float factor) {
        ...
        protected ErodingFactor getErodingFactor(K key) {
            return this.factors.computeIfAbsent(key, k -> new ErodingFactor(this.factor));
        }
    ...
    implements KeyedObjectPool<K, V> {
        ...
        ErodingKeyedObjectPool(KeyedObjectPool<K, V> keyedPool, float factor) {
