    ...
    public boolean checkNotModified(@Nullable String etag, long lastModifiedTimestamp) {
        ...
        if (this.validateIfUnmodifiedSince(lastModifiedTimestamp)) {
            ...
            if (SAFE_METHODS.contains(this.getRequest().getMethod())) {
                if (StringUtils.hasLength((String)etag) && response.getHeader("ETag") == null) {
    ...
    private boolean validateIfNoneMatch(@Nullable String etag) {
        ...
        block2: while (ifNoneMatch.hasMoreElements()) {
            ...
            while (etagMatcher.find()) {
                if (!StringUtils.hasLength((String)etagMatcher.group()) || !etag.equals(etagMatcher.group(3))) continue;
