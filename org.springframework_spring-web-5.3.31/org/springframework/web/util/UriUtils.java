...
public abstract class UriUtils {
    ...
    public static MultiValueMap<String, String> encodeQueryParams(MultiValueMap<String, String> params) {
        ...
        LinkedMultiValueMap result = new LinkedMultiValueMap(params.size());
        for (Map.Entry entry : params.entrySet()) {
            for (String value : (List)entry.getValue()) {
                result.add((Object)UriUtils.encodeQueryParam((String)entry.getKey(), charset), (Object)UriUtils.encodeQueryParam(value, charset));
            }
        }
        return result;
    }
    ...
    public static Map<String, String> encodeUriVariables(Map<String, ?> uriVariables) {
        LinkedHashMap result = CollectionUtils.newLinkedHashMap((int)uriVariables.size());
        uriVariables.forEach((key, value) -> {
            String stringValue = value != null ? value.toString() : "";
            result.put(key, UriUtils.encode(stringValue, StandardCharsets.UTF_8));
        });
        return result;
    }
