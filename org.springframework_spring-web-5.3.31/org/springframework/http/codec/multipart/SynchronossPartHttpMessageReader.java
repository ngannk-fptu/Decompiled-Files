...
implements HttpMessageReader<Part> {
    ...
    public boolean canRead(ResolvableType elementType, @Nullable MediaType mediaType) {
        if (Part.class.equals((Object)elementType.toClass())) {
            if (mediaType == null) {
                return true;
            }
    ...
    public Flux<Part> read(ResolvableType elementType, ReactiveHttpInputMessage message, Map<String, Object> hints) {
        return this.getFileStorageDirectory().flatMapMany(directory -> Flux.create((Consumer)new SynchronossPartGenerator(message, (Path)directory)).doOnNext(part -> {
            if (!Hints.isLoggingSuppressed((Map)hints)) {
                LogFormatUtils.traceDebug((Log)this.logger, traceOn -> Hints.getLogPrefix((Map)hints) + "Parsed " + (this.isEnableLoggingRequestDetails() ? LogFormatUtils.formatValue((Object)part, (traceOn == false ? 1 : 0) != 0) : "parts '" + part.name() + "' (content masked)"));
            }
        }));
    }
    ...
    implements FormFieldPart {
        ...
        private Charset getCharset() {
            String name = MultipartUtils.getCharEncoding((Map)((Object)this.headers()));
    ...
    extends AbstractSynchronossPart {
        ...
        SynchronossPart(HttpHeaders headers, StreamStorage storage) {
            super(headers);
            Assert.notNull((Object)storage, (String)"StreamStorage is required");
            this.storage = storage;
        }

        @Override
        public Flux<DataBuffer> content() {
            return DataBufferUtils.readInputStream(() -> ((StreamStorage)this.getStorage()).getInputStream(), (DataBufferFactory)DefaultDataBufferFactory.sharedInstance, (int)4096);
        }
    ...
    implements Part {
        ...
        AbstractSynchronossPart(HttpHeaders headers) {
            ...
            this.name = MultipartUtils.getFieldName((Map)((Object)headers));
    ...
    implements NioMultipartParserListener {
        ...
        private Part createPart(StreamStorage storage, HttpHeaders httpHeaders) {
            String filename = MultipartUtils.getFileName((Map)((Object)httpHeaders));
            if (filename != null) {
                return new SynchronossFilePart(httpHeaders, filename, storage);
            }
            if (MultipartUtils.isFormField((Map)((Object)httpHeaders), (MultipartContext)this.context)) {
                String value = MultipartUtils.readFormParameterValue((StreamStorage)storage, (Map)((Object)httpHeaders));
    ...
    implements Consumer<FluxSink<Part>> {
        ...
        public void accept(FluxSink<Part> sink) {
            ...
            Assert.state((mediaType != null ? 1 : 0) != 0, (String)"No content type set");
        ...
        protected void hookOnNext(DataBuffer buffer) {
            Assert.state((this.parser != null && this.listener != null ? 1 : 0) != 0, (String)"Not initialized yet");
