...
extends AbstractDecoder<Object> {
    ...
    public Mono<Object> decodeToMono(Publisher<DataBuffer> input, ResolvableType elementType, @Nullable MimeType mimeType, @Nullable Map<String, Object> hints) {
        return DataBufferUtils.join(input, (int)this.maxInMemorySize).map(dataBuffer -> this.decode((DataBuffer)dataBuffer, elementType, mimeType, hints));
    }
    ...
    private Object unmarshal(List<XMLEvent> events, Class<?> outputClass) {
        try {
            ...
            throw new DecodingException("Could not unmarshal XML to " + outputClass, (Throwable)ex);
        }
