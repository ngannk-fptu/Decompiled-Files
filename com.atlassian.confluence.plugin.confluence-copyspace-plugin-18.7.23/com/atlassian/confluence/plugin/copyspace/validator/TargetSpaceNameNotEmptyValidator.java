...
implements Consumer<CopySpaceRequest> {
    ...
    @Autowired
    public TargetSpaceNameNotEmptyValidator(I18NBeanProvider i18NBeanProvider) {
    ...
    public void accept(CopySpaceRequest request) {
        if (StringUtils.isBlank((String)request.getNewName())) {
            throw new BadRequestException(this.i18NBeanProvider.getI18NBean().getText("copyspace.validation.space.name.empty"));
        }
    }
}
