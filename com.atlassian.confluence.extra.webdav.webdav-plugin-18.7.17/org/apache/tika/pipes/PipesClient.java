...
implements Closeable {
    ...
    private PipesResult readResults(FetchEmitTuple t, long start) throws IOException {
        ...
        switch (status) {
            ...
            case PARSE_SUCCESS: {
                LOG.info("pipesClientId={} parse success: {} in {} ms", new Object[]{this.pipesClientId, t.getId(), millis});
            ...
            case EMIT_SUCCESS: {
                LOG.info("pipesClientId={} emit success: {} in {} ms", new Object[]{this.pipesClientId, t.getId(), millis});
    ...
    private void restart() throws IOException, InterruptedException, TimeoutException {
        ...
        }
        {
            ...
            FutureTask<Integer> futureTask = new FutureTask<Integer>(() -> {
                ...
                if (bos.size() > 0) {
                    LOG.warn("pipesClientId={}: From forked process before start byte: {}", (Object)this.pipesClientId, (Object)new String(bos.toByteArray(), StandardCharsets.UTF_8));
                }
                return 1;
            });
            long start = System.currentTimeMillis();
            ...
            try {
                ...
                long elapsed = System.currentTimeMillis() - start;
                LOG.error("pipesClientId={} didn't receive ready byte from server within StartupTimeoutMillis {}; ms elapsed {}; did read >{}<", new Object[]{this.pipesClientId, this.pipesConfig.getStartupTimeoutMillis(), elapsed, new String(bos.toByteArray(), StandardCharsets.UTF_8)});
                this.destroyForcibly();
                throw e;
            }
            finally {
