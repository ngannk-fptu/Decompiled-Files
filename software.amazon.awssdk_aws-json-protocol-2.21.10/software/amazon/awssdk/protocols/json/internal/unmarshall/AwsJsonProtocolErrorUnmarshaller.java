...
implements HttpResponseHandler<AwsServiceException> {
    ...
    private AwsServiceException unmarshall(SdkHttpFullResponse response, ExecutionAttributes executionAttributes) {
        ...
        exception.requestId((String)response.firstMatchingHeader((Collection)X_AMZN_REQUEST_ID_HEADERS).orElse(null));
        exception.extendedRequestId((String)response.firstMatchingHeader("x-amz-id-2").orElse(null));
        return exception.build();
    }

    private String getEffectiveErrorCode(SdkHttpFullResponse response, String errorCode) {
        String compatibleErrorCode;
        if (this.hasAwsQueryCompatible && !StringUtils.isEmpty((CharSequence)(compatibleErrorCode = this.queryCompatibleErrorCodeFromResponse(response)))) {
