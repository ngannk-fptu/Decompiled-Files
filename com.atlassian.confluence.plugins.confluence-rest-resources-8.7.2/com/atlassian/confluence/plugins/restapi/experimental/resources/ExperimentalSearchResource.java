...
@Path(value="/search")
public class ExperimentalSearchResource {
    private final CQLSearchService searchService;

    public ExperimentalSearchResource(@ComponentImport CQLSearchService searchService) {
        this.searchService = searchService;
    }

    @GET
    public SearchPageResponse<SearchResult> search(@QueryParam(value="cql") String cql, @QueryParam(value="cqlcontext") String cqlcontext, @QueryParam(value="excerpt") String excerpt, @QueryParam(value="expand") @DefaultValue(value="") String expand, @QueryParam(value="start") int start, @QueryParam(value="limit") @DefaultValue(value="25") int limit, @QueryParam(value="includeArchivedSpaces") @DefaultValue(value="false") boolean includeArchivedSpaces, @Context UriInfo uriInfo) {
        ...
        SearchPageResponse results = this.searchService.search(cql, searchOptions, (PageRequest)new RestPageRequest(uriInfo, start, limit), ExpansionsParser.parseExperimental((String)expand));
        return results;
    }
}
