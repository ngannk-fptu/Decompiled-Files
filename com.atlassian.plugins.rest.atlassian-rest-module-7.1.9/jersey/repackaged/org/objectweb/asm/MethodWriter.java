...
import jersey.repackaged.org.objectweb.asm.AnnotationWriter;
...
import jersey.repackaged.org.objectweb.asm.Edge;
import jersey.repackaged.org.objectweb.asm.Frame;
import jersey.repackaged.org.objectweb.asm.Handle;
import jersey.repackaged.org.objectweb.asm.Handler;
...
extends MethodVisitor {
    ...
    MethodWriter(ClassWriter cw, int access, String name, String desc, String signature, String[] exceptions, boolean computeMaxs, boolean computeFrames) {
        ...
        int n = computeFrames ? 0 : (this.compute = computeMaxs ? 1 : 2);
    ...
    public void visitVarInsn(int opcode, int var) {
        ...
        if (this.compute != 2 && (n = opcode == 22 || opcode == 24 || opcode == 55 || opcode == 57 ? var + 2 : var + 1) > this.maxLocals) {
            this.maxLocals = n;
        }
        if (var < 4 && opcode != 169) {
    ...
    public void visitLocalVariable(String name, String desc, String signature, Label start, Label end, int index) {
        ...
        if (this.compute != 2 && (n = index + ((c = desc.charAt(0)) == 'J' || c == 'D' ? 2 : 1)) > this.maxLocals) {
            this.maxLocals = n;
        }
    }
    ...
    private void resizeInstructions() {
        ...
        if (this.compute == 0) {
            ...
            while (l != null) {
                ...
                MethodWriter.getNewOffset(allIndexes, allSizes, l);
                l = l.successor;
            }
            for (i = 0; i < this.cw.typeTable.length; ++i) {
                Item item = this.cw.typeTable[i];
                if (item == null || item.type != 31) continue;
                item.intVal = MethodWriter.getNewOffset(allIndexes, allSizes, 0, item.intVal);
            }
        ...
        while (attr != null) {
            ...
            if (labels != null) {
                for (i = labels.length - 1; i >= 0; --i) {
                    MethodWriter.getNewOffset(allIndexes, allSizes, labels[i]);
                }
            }
            attr = attr.next;
        }
