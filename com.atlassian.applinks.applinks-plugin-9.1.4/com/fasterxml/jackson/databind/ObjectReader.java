...
Serializable {
    ...
    protected Object _bind(JsonParser p, Object valueToUpdate) throws IOException {
        ...
        Object result = t == JsonToken.VALUE_NULL ? (valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate));
        p.clearCurrentToken();
    ...
    protected Object _bindAndClose(JsonParser p0) throws IOException {
        try (JsonParser p = p0;){
            DefaultDeserializationContext ctxt = this.createDeserializationContext(p);
            JsonToken t = this._initForReading(ctxt, p);
            Object result = t == JsonToken.VALUE_NULL ? (this._valueToUpdate == null ? this._findRootDeserializer(ctxt).getNullValue(ctxt) : this._valueToUpdate) : (t == JsonToken.END_ARRAY || t == JsonToken.END_OBJECT ? this._valueToUpdate : ctxt.readRootValue(p, this._valueType, this._findRootDeserializer(ctxt), this._valueToUpdate));
            if (this._config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
                this._verifyNoTrailingTokens(p, ctxt, this._valueType);
            }
    ...
    protected final JsonNode _bindAsTree(JsonParser p) throws IOException {
        ...
        if ((t = p.currentToken()) == null && (t = p.nextToken()) == null) {
            return this._config.getNodeFactory().missingNode();
        }
        DefaultDeserializationContext ctxt = this.createDeserializationContext(p);
        JsonNode resultNode = t == JsonToken.VALUE_NULL ? this._config.getNodeFactory().nullNode() : (JsonNode)ctxt.readRootValue(p, this._jsonNodeType(), this._findTreeDeserializer(ctxt), null);
        p.clearCurrentToken();
        if (this._config.isEnabled(DeserializationFeature.FAIL_ON_TRAILING_TOKENS)) {
    ...
    protected JsonDeserializer<Object> _prefetchRootDeserializer(JavaType valueType) {
        ...
        if (deser == null) {
            try {
                DefaultDeserializationContext ctxt = this.createDummyDeserializationContext();
