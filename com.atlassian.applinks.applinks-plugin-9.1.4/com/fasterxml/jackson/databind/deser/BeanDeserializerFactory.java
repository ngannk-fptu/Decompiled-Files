...
implements Serializable {
    ...
    public JsonDeserializer<Object> buildThrowableDeserializer(DeserializationContext ctxt, JavaType type, BeanDescription beanDesc) throws JsonMappingException {
        ...
        if ((am = beanDesc.findMethod("initCause", INIT_CAUSE_PARAMS)) != null) {
            ...
            if ((prop = this.constructSettableProperty(ctxt, beanDesc, propDef = SimpleBeanPropertyDefinition.construct(ctxt.getConfig(), am, new PropertyName((String)name)), am.getParameterType(0))) != null) {
    ...
    protected SettableAnyProperty constructAnySetter(DeserializationContext ctxt, BeanDescription beanDesc, AnnotatedMember mutator) throws JsonMappingException {
        ...
        if (mutator instanceof AnnotatedMethod) {
            ...
            if (!isField) return (SettableAnyProperty)ctxt.reportBadDefinition(beanDesc.getType(), String.format("Unrecognized mutator type for any-setter: %s", ClassUtil.nameOf(mutator.getClass())));
            ...
            if (fieldType.isMapLikeType()) {
                ...
                if (!fieldType.hasRawClass(JsonNode.class) && !fieldType.hasRawClass(ObjectNode.class)) return (SettableAnyProperty)ctxt.reportBadDefinition(beanDesc.getType(), String.format("Unsupported type for any-setter: %s -- only support `Map`s, `JsonNode` and `ObjectNode` ", ClassUtil.getTypeDescription(fieldType)));
                fieldType = this.resolveMemberAndTypeAnnotations(ctxt, mutator, fieldType);
        ...
        if (!isField) return SettableAnyProperty.constructForMethod(ctxt, prop, mutator, valueType, keyDeser, deser, typeDeser);
