...
extends AbstractAppLinksProtectedServlet {
    ...
    protected final boolean checkAccess(HttpServletRequest request) {
        return this.adminUIAuthenticator.checkAdminUIAccessBySessionOrCurrentUser(request);
    }

    @Override
    protected void handleUnauthorizedAccess(HttpServletRequest request) {
        if (request.getUserPrincipal() != null) {
            throw new AbstractApplinksServlet.UnauthorizedException(this.messageFactory.newI18nMessage("applinks.error.only.admin", new Serializable[0]));
        }
