...
implements Serializable {
    ...
    public ArrayNode(JsonNodeFactory nf, List<JsonNode> children) {
        super(nf);
        this._children = Objects.requireNonNull(children, "Must not pass `null` for 'children' argument");
    }
    ...
    protected void _withXxxSetArrayElement(int index, JsonNode value) {
        if (index >= this.size()) {
            ...
            if (index > max) {
                this._reportWrongNodeOperation("Too big Array index (%d; max %d) to use for insert with `JsonPointer`", index, max);
            }
    ...
    public void serialize(JsonGenerator g, SerializerProvider provider) throws IOException {
        ...
        int size = c.size();
        g.writeStartArray((Object)this, size);
