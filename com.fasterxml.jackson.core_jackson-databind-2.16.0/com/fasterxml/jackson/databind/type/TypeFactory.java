...
implements Serializable {
    ...
    private static final Class<?> CLS_DOUBLE = Double.TYPE;
    ...
    protected TypeFactory(LookupCache<Object, JavaType> typeCache) {
        this._typeCache = Objects.requireNonNull(typeCache);
    ...
    public static Class<?> rawClass(Type t) {
        ...
        if (t instanceof GenericArrayType) {
            return Array.newInstance(TypeFactory.rawClass(((GenericArrayType)t).getGenericComponentType()), 0).getClass();
        }
        if (t instanceof ParameterizedType) {
            return TypeFactory.rawClass(((ParameterizedType)t).getRawType());
        }
        if (t instanceof TypeVariable) {
            return TypeFactory.rawClass(((TypeVariable)t).getBounds()[0]);
        }
        if (t instanceof WildcardType) {
            return TypeFactory.rawClass(((WildcardType)t).getUpperBounds()[0]);
        }
...
lbl22:
    ...
    public JavaType findFirstTypeParameter(JavaType type, Class<?> expType) {
        JavaType t;
        JavaType match = type.findSuperType(expType);
        if (match != null && (t = match.getBindings().getBoundType(0)) != null) {
            return t;
        }
    ...
    public JavaType resolveMemberType(Type type, TypeBindings contextBindings) {
        return this._fromAny(null, type, contextBindings);
    }
    ...
    public JavaType constructParametricType(Class<?> rawType, TypeBindings parameterTypes) {
        JavaType resultType = this._fromClass(null, rawType, parameterTypes);
    ...
    protected JavaType _fromClass(ClassStack context, Class<?> rawType, TypeBindings bindings) {
        ...
        JavaType javaType = result = key == null ? null : this._typeCache.get(key);
        if (result != null) {
            return result;
        }
        if (context == null) {
