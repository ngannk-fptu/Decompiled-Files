...
extends AbstractFileStoreTlsKeyManagersProvider {
    ...
    public KeyManager[] keyManagers() {
        return SystemPropertyTlsKeyManagersProvider.getKeyStore().map(p -> {
            ...
            try {
                ...
                log.warn(() -> String.format("Unable to create KeyManagers from %s property value '%s'", JavaSystemSetting.SSL_KEY_STORE.property(), p), (Throwable)e);
                return null;
            }
        }).orElse(null);
    }
    ...
    private static Optional<String> getKeyStore() {
        return SystemSettingUtils.resolveSetting((SystemSetting)JavaSystemSetting.SSL_KEY_STORE);
    }

    private static String getKeyStoreType() {
        return SystemSettingUtils.resolveSetting((SystemSetting)JavaSystemSetting.SSL_KEY_STORE_TYPE).orElseGet(KeyStore::getDefaultType);
    }

    private static Optional<String> getKeyStorePassword() {
        return SystemSettingUtils.resolveSetting((SystemSetting)JavaSystemSetting.SSL_KEY_STORE_PASSWORD);
    }
}
